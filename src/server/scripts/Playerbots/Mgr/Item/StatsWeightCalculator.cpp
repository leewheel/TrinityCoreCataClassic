...
namespace
{
//By leewheel 2026-09-04 对齐上游: 移植 AC GenerateEnchSuffixFactor(itemId) 移植适配 TC 框架——
//TC343 的等价实现 GetRandomPropertyPoints 是 game 层 static 函数, 模块侧不可调用,
//按其 RandPropPointsStore 数据在模块内等价重写(装备栏位->列索引映射 + 品质列选择),
//修复原"AllocationPct * 1"占位——随机后缀物品的附魔量此前恒按万分之一计, 评分严重低估
uint32 GetEnchSuffixFactor(uint32 itemId)
{
    ItemTemplate const* proto = sObjectMgr->GetItemTemplate(itemId);
    if (!proto)
        return 0;
    // 无随机后缀组的物品没有随机属性点数
    if (!proto->GetRandomSuffixGroupID())
        return 0;
    ...
    RandPropPointsEntry const* randPropPointsEntry = sRandPropPointsStore.LookupEntry(proto->GetItemLevel());
...
{
    //By leewheel 2026-07-10: GetItemTemplateStore()返回引用，用.而非->
    ...
    if (enable_quality_blend_)
    {
        ...
        if (proto->GetQuality() == ITEM_QUALITY_HEIRLOOM)
            ...
            weight_ *= PlayerbotFactory::CalcMixedGearScore(proto->GetItemLevel(), proto->GetQuality());
    }
...
//By leewheel 2026-09-04: itemId 恢复参与计算——随机后缀附魔量需按物品等级/品质查点数系数(AC GenerateEnchSuffixFactor 语义)
    ...
    else
    {
        ...
        for (uint32 i = PROP_ENCHANTMENT_SLOT_0; i < MAX_ENCHANTMENT_SLOT; ++i)
        {
            ...
            for (int k = 0; k < MAX_ITEM_ENCHANTMENT_EFFECTS; ++k)
            {
                ...
                {
                    //By leewheel 2026-09-04 对齐上游: 附魔量 = AllocationPct × 点数系数 / 10000——
                    //原"(AllocationPct * 1)/10000"占位使随机后缀物品属性评分低估约5000倍
                    enchant_amount = uint32((item_rand->AllocationPct[k] * GetEnchSuffixFactor(itemId)) / 10000);
                    break;
                }
            }
...
int32 StatsWeightCalculator::PickBestRandomPropertyId(uint32 itemId)
{
    ...
    bool isSuffix = false;
    uint32 poolEntry = proto->GetRandomSelect();
    if (!poolEntry)
    {
        poolEntry = proto->GetRandomSuffixGroupID();
        isSuffix = true;
    }
    if (!poolEntry)
        return 0;
...
{
    uint32 itemSet = proto->GetItemSet();
    if (!itemSet)
        return;
    ...
    for (i = 0; i < player->ItemSetEff.size(); i++)
    {
        if (player->ItemSetEff[i])
        {
            ...
            uint32 setId = eff->ItemSetID; // By leewheel 2026-07-09: TC使用ItemSetID而不是setid
            if (itemSet != setId)
                continue;

            uint32 itemCount = (uint32)eff->EquippedItems.size(); // By leewheel 2026-07-09: TC使用EquippedItems而不是item_count
            uint32 max_items = 0;
            // By leewheel 2026-07-09: TC使用SetBonuses中的Threshold而不是items_to_triggerspell
            for (ItemSetSpellEntry const* setSpell : eff->SetBonuses)
                max_items = std::max(max_items, (uint32)setSpell->Threshold);
            // End By leewheel
...
void StatsWeightCalculator::CalculateSocketBonus(Player* /*player*/, ItemTemplate const* proto)
{
    ...
    for (uint32 enchant_slot = SOCK_ENCHANTMENT_SLOT; enchant_slot < SOCK_ENCHANTMENT_SLOT + MAX_GEM_SOCKETS;
         ++enchant_slot)
    {
        uint8 socketColor = (uint8)proto->GetSocketColor(enchant_slot - SOCK_ENCHANTMENT_SLOT);
...
void StatsWeightCalculator::CalculateItemTypePenalty(ItemTemplate const* proto)
{
    ...
    if (proto->GetClass() == ITEM_CLASS_WEAPON)
    {
        ...
        if (isDoubleHand)
        {
            ...
            {
                weight_ *= 0.1f; //By leewheel 2026-09-04: 修 C4305——乘数字面量补 float 后缀
            }
        }
        ...
        if (!isDoubleHand)
        {
            ...
            {
                weight_ *= 0.1f; //By leewheel 2026-09-04: 修 C4305
            }
            ...
            if ((cls == CLASS_MAGE || cls == CLASS_PRIEST || cls == CLASS_WARLOCK || cls == CLASS_DRUID ||
                ...
                !(cls == CLASS_PALADIN && tab == PALADIN_TAB_HOLY))
            {
                weight_ *= 0.65f; //By leewheel 2026-09-04: 修 C4305
            }
            if (cls == CLASS_PALADIN && tab == PALADIN_TAB_HOLY)
            {
                weight_ *= 0.8f; //By leewheel 2026-09-04: 修 C4305
            }
        }
...
proto->GetSubClass() == ITEM_SUBCLASS_WEAPON_STAFF) &&
        ...
        {
            weight_ *= 1.5f; //By leewheel 2026-09-04: 修 C4305
        }
        ...
        {
            weight_ *= 1.1f; //By leewheel 2026-09-04: 修 C4305
        }
        ...
        {
            weight_ *= 1.3f; //By leewheel 2026-09-04: 修 C4305
        }
...
float StatsWeightCalculator::ApplyPreferredSpecWeapons(ItemTemplate const* proto, int32 slot)
{
    ...
    uint32 delay = proto->GetDelay();  // 毫秒
