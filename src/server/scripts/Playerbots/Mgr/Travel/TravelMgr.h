...
public:
    ...
    //By leewheel 2026-07-10: TC中ObjectGuid和WorldLocation都有GetMapId()，需要显式指定使用WorldPosition的版本
    uint32 GetMapId() const { return WorldPosition::GetMapId(); }
    //End By leewheel

    operator bool() const { return !IsEmpty(); }
    bool operator==(ObjectGuid const& guid) const { return static_cast<ObjectGuid const&>(*this) == guid; }
    bool operator!=(ObjectGuid const& guid) const { return static_cast<ObjectGuid const&>(*this) != guid; }
    bool operator<(ObjectGuid const& guid) const { return static_cast<ObjectGuid const&>(*this) < guid; }
