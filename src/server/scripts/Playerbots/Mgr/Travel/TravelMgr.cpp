...
//By leewheel 2026-07-11: 添加RaceMask.h以获取RACEMASK_ALLIANCE
#include "RaceMask.h"
//End By leewheel
//By leewheel 2026-07-11: 添加CharacterPackets.h以使用CharacterCreateInfo完整定义
...
WorldPosition::WorldPosition(std::string const str)
{
    ...
    if (tokens.size() == 5)
    {
        try
        {
            ...
            SetOrientation(std::stof(tokens[4]));
        }
    ...
    else if (conType == WP_CENTROID)
    {
        set(std::accumulate(list.begin(), list.end(), WorldLocation(list[0]->GetMapId(), 0, 0, 0, 0),
                            [size](WorldLocation i, WorldPosition* j)
                            {
                                ...
                                i.SetOrientation(i.GetOrientation() + j->GetOrientation() / size);
                                return i;
                            }));
    }
...
void WorldPosition::setO(float o) { SetOrientation(o); }
...
{
    return getMap() ? getMap()->IsUnderWater(PhaseShift(), GetPositionX(), GetPositionY(), GetPositionZ())
                    : false;
};
...
{
    return getMap()->GetHeight(PhaseShift(), GetPositionX(), GetPositionY(), GetPositionZ());
}
...
{
    //By leewheel 2026-07-11: TC使用map->GetAreaId, WorldPosition没有GetInstanceId/GetPhaseShift
    Map* map = sMapMgr->FindMap(GetMapId(), 0);
    if (map)
        return map->GetAreaId(PhaseShift(), GetPositionX(), GetPositionY(), GetPositionZ());
    return 0;
    //End By leewheel
}
...
/*
    ...
    {
        //By leewheel 2026-07-10: TC使用不同的MMap API
            ...
            if (!TravelMgr::instance().isBadVmap(mapId, x, y))
            {
                ...
                switch (vmapLoadResult)
                {
                    case VMAP::VMAP_LOAD_RESULT_OK:
                        // TC_LOG_ERROR("playerbots", "VMAP loaded name:{}, id:{}, x:{}, y:{} (vmap rep.: x:{}, y:{})",
                        // mapName, mapId, x, y, x, y);
                        break;
                    case VMAP::VMAP_LOAD_RESULT_ERROR:
                        // TC_LOG_ERROR("playerbots", "Could not load VMAP name:{}, id:{}, x:{}, y:{} (vmap rep.: x:{},
                        // y:{})", mapName, mapId, x, y, x, y);
                        TravelMgr::instance().addBadVmap(mapId, x, y);
                        break;
                    case VMAP::VMAP_LOAD_RESULT_IGNORED:
                        TravelMgr::instance().addBadVmap(mapId, x, y);
                        // TC_LOG_INFO("playerbots", "Ignored VMAP name:{}, id:{}, x:{}, y:{} (vmap rep.: x:{}, y:{})",
                        // mapName, mapId, x, y, x, y);
                        break;
                }
...
bool WorldPosition::GetReachableRandomPointOnGround(Player* bot, float radius, bool randomRange)
{
    ...
    //By leewheel 2026-07-10: TC没有CanReachPositionAndGetValidCoords
    bool canReach = Map_CanReachPositionAndGetValidCoords(getMap(), bot, x, y, z);
    //End By leewheel
    setX(x);
    setY(y);
    setZ(z);
...
WorldObject* GuidPosition::GetWorldObject()
{
    ...
    switch (GetHigh())
    {
        ...
        // By leewheel 2026-07-08: TC没有HighGuid::Mo_Transport，已合并到HighGuid::Transport
        case HighGuid::GameObject:
            return GetGameObject();
        case HighGuid::Vehicle:
        case HighGuid::Creature: // By leewheel 2026-07-08: TC使用HighGuid::Creature替代HighGuid::Unit
            return GetCreature();
        case HighGuid::Pet:
...
GameObject* GuidPosition::GetGameObject()
{
    ...
    if (loadedFromDB)
        //By leewheel 2026-07-10: TC没有GetSpawnedGameObjectByDBGUID
    return ObjectAccessor_GetSpawnedGameObjectByDBGUID(getMap(), GetCounter());
    //End By leewheel
...
Creature* GuidPosition::GetCreature()
{
    ...
    if (loadedFromDB)
        //By leewheel 2026-07-10: TC没有GetSpawnedCreatureByDBGUID
    return ObjectAccessor_GetSpawnedCreatureByDBGUID(getMap(), GetCounter());
    //End By leewheel
...
std::string const QuestObjectiveTravelDestination::getTitle()
{
    ...
    if (itemId)
        //By leewheel 2026-08-01: 玩家可见文本中文化
        out << " 从以下处获取 " << ChatHelper::FormatItem(sObjectMgr->GetItemTemplate(itemId), 0, 0);
        //End By leewheel
    else
        //By leewheel 2026-08-01: 玩家可见文本中文化
        out << " 击杀目标";
...
bool BossTravelDestination::isActive(Player* bot)
{
    ...
    */
    ...
    FactionTemplateEntry const* factionEntry = sFactionTemplateStore.LookupEntry(cInfo->faction);
    //By leewheel 2026-07-11: TC的GetFactionReactionTo参数顺序: (FactionTemplateEntry*, WorldObject*)
    ReputationRank reaction = Unit::GetFactionReactionTo(factionEntry, bot);
    //End By leewheel
...
void TravelMgr::logQuestError(uint32 errorNr, Quest* quest, uint32 objective, uint32 unitId, uint32 itemId)
{
    ...
    if (errorNr == 1)
    {
        TC_LOG_ERROR("playerbots", "Quest {} [{}] has {} {} [{}] but none is found in the world.",
                  quest->GetLogTitle(), quest->GetQuestId(), objective == 0 ? "quest giver" : "quest taker",
                  unitName.c_str(), unitId);
    }
    else if (errorNr == 2)
    {
        TC_LOG_ERROR("playerbots", "Quest {} [{}] needs {} [{}] for objective {} but none is found in the world.",
                  quest->GetLogTitle(), quest->GetQuestId(), unitName.c_str(), unitId, objective);
    }
    else if (errorNr == 3)
    {
        TC_LOG_ERROR("playerbots", "Quest {} [{}] needs itemId {} but no such item exists.", quest->GetLogTitle(),
    ...
    else if (errorNr == 4)
    {
        TC_LOG_ERROR(
            "playerbots",
            //By leewheel 2026-07-11: TC的GetLogTitle()和GetName()返回const char*, 不需要.c_str()
            "Quest {} [{}] needs {} [{}] for loot of item {} [{}] for objective {} but none is found in the world.",
            quest->GetLogTitle(), quest->GetQuestId(), unitName.c_str(), unitId, proto->GetName(DEFAULT_LOCALE), itemId,
            objective);
            //End By leewheel
    }
    else if (errorNr == 5)
    {
        //By leewheel 2026-07-11: TC的GetLogTitle()和GetName()返回const char*
        TC_LOG_ERROR("playerbots", "Quest {} [{}] needs item {} [{}] for objective {} but none is found in the world.",
                  quest->GetLogTitle(), quest->GetQuestId(), proto->GetName(DEFAULT_LOCALE), itemId, objective);
        //End By leewheel
    }
    else if (errorNr == 6)
    {
        TC_LOG_ERROR("playerbots", "Quest {} [{}] has no quest giver.", quest->GetLogTitle(), quest->GetQuestId());
    }
    else if (errorNr == 7)
    {
        TC_LOG_ERROR("playerbots", "Quest {} [{}] has no quest taker.", quest->GetLogTitle(), quest->GetQuestId());
    }
    else if (errorNr == 8)
    {
        TC_LOG_ERROR("playerbots", "Quest {} [{}] has no quest viable quest objective.", quest->GetLogTitle(),
                  quest->GetQuestId());
    }
}
...
void TravelMgr::LoadQuestTravelTable()
{
    ...
    {
        TC_LOG_INFO("playerbots", "[Quest id: {}]", cQuest.first);

        for (auto cObj : cQuest.second)
        {
            TC_LOG_INFO("playerbots", " [Objective type: {}]", cObj.first);

            for (auto cCre : cObj.second)
            {
                TC_LOG_INFO("playerbots", " {} {}", cCre.GetTypeName().c_str(), cCre.GetEntry());
            }
        }
    }
    */
    ...
    TC_LOG_INFO("playerbots", "Loading units locations.");
...
//By leewheel 2026-09-09: TC-Cata的SpawnData使用mapId字段和spawnPoint成员
    ...
    TC_LOG_INFO("playerbots", "Loading game object locations.");
...
//By leewheel 2026-09-09: TC-Cata的SpawnData使用mapId字段和spawnPoint成员
    ...
    if (result)
    {
        ...
        TC_LOG_INFO("playerbots", ">> Loaded {} units locations.", units.size());
    }
    else
    {
        TC_LOG_ERROR("playerbots", ">> Error loading units locations.");
    }
    ...
    if (result)
    {
        ...
        TC_LOG_INFO("playerbots", ">> Loaded {} relations.", relations.size());
    }
    else
    {
        TC_LOG_ERROR("playerbots", ">> Error loading relations.");
    }
    ...
    if (result)
    {
        ...
        TC_LOG_INFO("playerbots", ">> Loaded {} loot lists.", loots.size());
    }
    else
    {
        TC_LOG_ERROR("playerbots", ">> Error loading loot lists.");
    }
    */

    TC_LOG_INFO("playerbots", "Loading quest data.");
    ...
    sPlayerbotAIConfig.sightDistance); loc->setExpireDelay(5 * 60 * 1000); loc->setMaxVisitors(15, 0);
            ...
            for (QuestObjective const& objective : quest->GetObjectives())
            {
                ...
                uint32 reqEntry = objective.ObjectID;

                loc = new QuestObjectiveTravelDestination(questId, reqEntry, objective.StorageIndex, sPlayerbotAIConfig.tooCloseDistance,
    sPlayerbotAIConfig.sightDistance); loc->setExpireDelay(1 * 60 * 1000); loc->setMaxVisitors(100, 1);
                ...
                if (loc->getPoints(0).empty())
                {
                    logQuestError(2, quest, objective.StorageIndex, reqEntry);
            ...
            for (QuestObjective const& objective : quest->GetObjectives())
            {
                ...
                ItemTemplate const* proto = sObjectMgr->GetItemTemplate(objective.ObjectID);
                if (!proto)
                {
                    logQuestError(3, quest, objective.StorageIndex, 0, objective.ObjectID);
                    hasError = true;
                    continue;
                }
                ...
                for (auto& l : loots)
                {
                    if (l.item != objective.ObjectID)
                        continue;

                    int32 entry = l.type == 0 ? l.entry : l.entry * -1;

                    loc = new QuestObjectiveTravelDestination(questId, entry, objective.StorageIndex, sPlayerbotAIConfig.tooCloseDistance,
    sPlayerbotAIConfig.sightDistance, l.item); loc->setExpireDelay(1 * 60 * 1000); loc->setMaxVisitors(100, 1);
                    ...
                    if (loc->getPoints(0).empty())
                    {
                        logQuestError(4, quest, objective.StorageIndex, entry, objective.ObjectID);
                        delete loc;
                        continue;
                    }
                ...
                {
                    hasError = true;
                    logQuestError(5, quest, objective.StorageIndex, 0, objective.ObjectID);
                }
            }
        ...
        TC_LOG_INFO("playerbots", ">> Loaded {} quest details.", questIds.size());
    }
    */

    WorldPosition point;

    TC_LOG_INFO("playerbots", "Loading Rpg, Grind and Boss locations.");
    ...
    TC_LOG_INFO("playerbots", "Loading Explore locations.");
   ...
   if (!fullNavPointReload && true)
    ...
    {
        TC_LOG_INFO("playerbots", "Loading navigation points");
        ...
        for (uint32 i = 0; i < MAX_RACES; i++)
        {
            for (uint32 j = 0; j < MAX_CLASSES; j++)
            {
                ...
                pos = WorldPosition(info->createPosition.Loc.GetMapId(), info->createPosition.Loc.m_positionX, info->createPosition.Loc.m_positionY, info->createPosition.Loc.m_positionZ, info->createPosition.Loc.m_orientation);
   ...
   true); travelPath.setPathAndCost(ppath, moveSpeed); node->setPathTo(prevNode, travelPath);
        ...
        TC_LOG_INFO("playerbots", ">> Loaded {} navigation points.", TravelNodeMap::instance().getNodes().size());
    }
    ...
    */
    ...
    if (preloadNodePaths)
    {
        ...
        TC_LOG_INFO("playerbots", ">> Loaded paths for {} nodes.", TravelNodeMap::instance().getNodes().size());
    }
    ...
    if (removeLowLinkNodes)
    {
        ...
        TC_LOG_INFO("playerbots", ">> Checked {} nodes.", TravelNodeMap::instance().getNodes().size());
    }
    ...
    if (cleanUpNodeLinks)
    {
        ...
        TC_LOG_INFO("playerbots", ">> Cleaned paths for {} nodes.", TravelNodeMap::instance().getNodes().size());
    }
    ...
    if (reCalculateCost)
    {
        ...
        TC_LOG_INFO("playerbots", ">> Calculated pathcost for {} nodes.", TravelNodeMap::instance().getNodes().size());
    }
    ...
    if (mirrorMissingPaths)
    {
        ...
        TC_LOG_INFO("playerbots", ">> Reversed missing paths for {} nodes.", TravelNodeMap::instance().getNodes().size());
    }
    */
...
//By leewheel 2026-09-09: TC-Cata的WorldSession构造函数需要14个参数(新增build和clientBuildVariant)
            ...
            //By leewheel 2026-07-11: TC使用MAX_RACES替代sRaceMgr->GetMaxRaces()
...
uint32 TravelMgr::getDialogStatus(Player* pPlayer, int32 questgiver, Quest const* pQuest)
{
    ...
    // TC中使用QuestRelationResult,不使用.first/.second,用范围for循环
    QuestRelationResult irboundsResult = (questgiver > 0) ?
        sObjectMgr->GetCreatureQuestInvolvedRelations(questgiver) :
        sObjectMgr->GetGOQuestInvolvedRelations(questgiver * -1);
    //End By leewheel 2025-01-16

    // Check markings for quest-finisher
    for (uint32 itr : irboundsResult)
    {
        if (itr != questId)
            continue;
    ...
    //By leewheel 2025-01-16
    // TC中使用QuestRelationResult,不使用.first/.second,用范围for循环
    QuestRelationResult rboundsResult = (questgiver > 0) ?
        sObjectMgr->GetCreatureQuestRelations(questgiver) :
        sObjectMgr->GetGOQuestRelations(questgiver * -1);
    //End By leewheel 2025-01-16

    // check markings for quest-giver
    for (uint32 itr : rboundsResult)
    {
        if (itr != questId)
            continue;
                                                    ...
                                                    uint32 /*amount*/)
{
    ...
    Trinity::Containers::RandomShuffle(retVec);
...
void TravelMgr::Init()
{
    ...
    TC_LOG_INFO("playerbots", "Playerbots Taxi graph and destination cache built.");
}
...
std::vector<std::vector<uint32>> TravelMgr::GetOptimalFlightDestinations(Player* bot)
{
    ...
    if (AreaTableEntry const* area = sAreaTableStore.LookupEntry(bot->GetZoneId()))
        botInCapital = (area->flags() & AREA_FLAG_CAPITAL) != 0; //By leewheel 2026-07-10: TC中flags是方法
...
void TravelMgr::PrepareZone2LevelBracket()
{
    ...
    //By leewheel 2025-01-16
    //By leewheel 2026-09-04 对齐上游: PlayerbotAIConfig 已补 zoneBrackets 字段与
    //ZoneBracket.<zoneId> 配置加载, 恢复配置层覆盖——配置指定的区域等级区间覆盖硬编码表
...
void TravelMgr::PrepareDestinationCache()
{
    ...
    TC_LOG_INFO("playerbots", "Preparing destination caches for {} levels...", maxLevel);
    ...
    for (auto const& [guid, creatureData] : sObjectMgr->GetAllCreatureData())
    {
        ...
        //By leewheel 2026-07-14: sTravelMgr.Init()已移至OnStartup中调用
        //此时PreloadContinents已完成，sMapMgr->FindMap能正常返回Map对象
        Map* map = sMapMgr->FindMap(mapId, 0);
        if (!map)
            continue;

        //By leewheel 2026-07-11: TC使用PhaseShift()而不是GetPhaseShift()
        AreaTableEntry const* area = sAreaTableStore.LookupEntry(map->GetAreaId(PhaseShift(), x, y, z));
        //End By leewheel
        if (!area)
            continue;
    ...
    //End By leewheel
        ...
        {
            //By leewheel 2026-07-11: TC的FactionTemplateEntry使用EnemyGroup替代hostileMask
            FactionTemplateEntry const* factionEntry = sFactionTemplateStore.LookupEntry(creatureTemplate->faction);
            bool forHorde = !(factionEntry->EnemyGroup & 4);
            bool forAlliance = !(factionEntry->EnemyGroup & 2);
            //End By leewheel
    ...
    //End By leewheel
    {
        for (uint32 j = 1; j < MAX_CLASSES; j++)
        {
            ...
            //By leewheel 2026-07-11: TC的Position::m_orientation是私有的, 使用GetOrientation()
            WorldPosition pos(info->createPosition.Loc.GetMapId(), info->createPosition.Loc.m_positionX, info->createPosition.Loc.m_positionY, info->createPosition.Loc.m_positionZ, info->createPosition.Loc.GetOrientation());
            //End By leewheel

            for (int32 l = 1; l <= 5; l++)
            {
                //By leewheel 2026-07-11: TC使用RACEMASK_ALLIANCE常量替代RaceMgr
                //By leewheel 2026-09-03 修复C4334警告：RawValue为uint64，1<<(i-1)是32位int移位(最高位race=11时溢出)，改用1ULL
                if ((1ULL << (i - 1)) & RACEMASK_ALLIANCE.RawValue)
                //End By leewheel
    ...
    TC_LOG_INFO("playerbots", ">> {} flight masters and {} innkeepers and {} banker locations for level collected.", flightMastersCount, innkeepersCount, bankerCount);

    //By leewheel 2026-07-14: 验证locsPerLevelCache是否成功填充
    uint32 totalLocs = 0;
    for (uint8 l = 1; l <= maxLevel; l++)
        totalLocs += locsPerLevelCache[l].size();
    TC_LOG_INFO("playerbots", ">> locsPerLevelCache total: {} locations across {} levels.", totalLocs, maxLevel);
    //End By leewheel
}
