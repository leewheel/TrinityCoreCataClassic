        ...
        if (!distanceOnly)
        {
            for (CreatureData const* cData : point.getCreaturesNear(50))  // Agro radius + 5
            {
                ...
                if (cInfo)
                {
                    ...
                    //By leewheel 2026-09-04 对齐上游: 修复阵营判定硬编码 false——
                    //TC 的 Unit::GetFactionReactionTo 第二参是 WorldObject*, 无法像 AC 那样直接传阵营模板,
                    //等价改用 FactionTemplateEntry::IsFriendlyTo 对联盟(1)/部落(2)模板求值;
                    //原写法两值恒 false: maxLevelCreature[1]/[2] 恒 0(敌对卫兵避让权重 factionAnnoyance 失效),
                    //且 !aFriend&&!hFriend 恒真把友方卫兵也计入怪物等级(mobAnnoyance 在城内虚高)
                    if (aReact.find(factionEntry) == aReact.end())
                        aReact.insert(std::make_pair(
                            factionEntry, factionEntry->IsFriendlyTo(sFactionTemplateStore.LookupEntry(1))));
...
float TravelNodePath::getCost(Player* bot, uint32 cGold)
{
    ...
    if (bot)
    {
        ...
        //By leewheel 2026-07-09: TC使用bot->GetReactionTo而非Unit::GetFactionReactionTo
        //判断是否联盟：通过玩家种族判断
...
{
    TC_LOG_INFO("playerbots", "-Generating Start nodes");
    generateStartNodes();
    TC_LOG_INFO("playerbots", "-Generating npc nodes");
    generateNpcNodes();
    TC_LOG_INFO("playerbots", "-Generating area trigger nodes");
    generateAreaTriggerNodes();
    TC_LOG_INFO("playerbots", "-Generating transport nodes");
    generateTransportNodes();
    TC_LOG_INFO("playerbots", "-Generating zone mean nodes");
...
void TravelNodeMap::generateWalkPaths()
{
    ...
    TC_LOG_INFO("playerbots", ">> Generated paths for {} nodes.", TravelNodeMap::instance().getNodes().size());
}
...
void TravelNodeMap::removeUselessPaths()
{
    ...
    while (true)
    {
        ...
        TC_LOG_INFO("playerbots", "Iteration {}, removed {}", it, rem);
    }
}
...
{
    TC_LOG_INFO("playerbots", "-Calculating walkable paths");
    generateWalkPaths();
    TC_LOG_INFO("playerbots", "-Removing useless nodes");
    removeLowNodes();
    TC_LOG_INFO("playerbots", "-Removing useless paths");
    removeUselessPaths();
    TC_LOG_INFO("playerbots", "-Calculating path costs");
    calculatePathCosts();
    TC_LOG_INFO("playerbots", "-Generating taxi paths");
    generateTaxiPaths();
}
...
void TravelNodeMap::generateAll()
{
    ...
    TC_LOG_INFO("playerbots", "-Calculating mapoffset");
    calcMapOffset();

    TC_LOG_INFO("playerbots", "-Generating maptransfers");
...
void TravelNodeMap::saveNodeStore()
{
    ...
    TC_LOG_INFO("playerbots", ">> Saved {} travelNodes.", anodes.size());

    {
        ...
        TC_LOG_INFO("playerbots", ">> Saved {} travelNode Paths, {} points.", paths, points);
    }
...
void TravelNodeMap::loadNodeStore()
{
...

    {
                ...
                PlayerbotsDatabase.Query(PlayerbotsDatabase.GetPreparedStatement(PLAYERBOTS_SEL_TRAVELNODE)))
        {
            ...
            TC_LOG_INFO("playerbots", ">> Loaded {} travelNodes.", saveNodes.size());
        }
        else
        {
            hasToFullGen = true;
            TC_LOG_ERROR("playerbots", ">> Error loading travelNodes.");
        }
    }

    {
                ...
                PlayerbotsDatabase.Query(PlayerbotsDatabase.GetPreparedStatement(PLAYERBOTS_SEL_TRAVELNODE_LINK)))
        {
            ...
            TC_LOG_INFO("playerbots", ">> Loaded {} travelNode paths.", result->GetRowCount());
        }
        else
        {
            TC_LOG_ERROR("playerbots", ">> Error loading travelNode links.");
        }
    }

    {
                ...
                PlayerbotsDatabase.Query(PlayerbotsDatabase.GetPreparedStatement(PLAYERBOTS_SEL_TRAVELNODE_PATH)))
        {
            ...
            TC_LOG_INFO("playerbots", ">> Loaded {} travelNode paths points.", result->GetRowCount());
        }
        else
        {
            TC_LOG_ERROR("playerbots", ">> Error loading travelNode paths.");
        }
    }
}
...
void TravelNodeMap::BuildTaxiGraph()
{
    ...
    for (uint32 i = 0; i < sTaxiPathStore.GetNumRows(); ++i)
    {
        ...
        //By leewheel 2026-09-05: 上游a1252c6d——飞行路径为单向图,不再反向插入边(避免单向航线被反向利用)
...
void TravelNodeMap::ComputeAllPaths()
{
    ...
    for (auto const& [source, neighbors] : taxiGraph)
    {
        ...
        //By leewheel 2026-09-05: 上游a1252c6d——邻接目标也纳入全节点集(单向边后必须包含目标才能搜到)
...
std::unordered_map<uint32, uint32> TravelNodeMap::BFS(uint32 fromNode)
{
    ...
    while (!workQueue.empty())
    {
        ...
        //By leewheel 2026-09-05: 上游a1252c6d——起点可能没有出边,用find避免at()抛异常崩溃
