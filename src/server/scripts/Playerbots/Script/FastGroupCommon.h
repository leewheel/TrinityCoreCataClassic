...
public:
    ...
    //By leewheel 2026-07-22: 增加group参数，返回true表示处理了bot（group可能已销毁）
    bool LogoutFastGroupBots(Player* master, Group* group = nullptr);

    //By leewheel 2026-07-22: 防递归标志，LogoutFastGroupBots内部RemoveMember时跳过OnRemoveMember
    bool IsInCleanup() const { return m_inCleanup; }
...
// By leewheel 2026-07-08
// 为队伍中所有成员分配LFG角色（坦克/治疗/输出），并发送角色选择包
// 使客户端显示角色图标
