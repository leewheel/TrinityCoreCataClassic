...
//By leewheel 2026-08-01: 日志清理——注释掉组队/竞技场操作过程DEBUG日志，仅保留错误与关键反馈
//End By leewheel
...
//By leewheel 2026-08-23: 合并 the-lab(#2637 Arena refactor) —— ArenaTeamAssignOperation 调用 RandomPlayerbotFactory
...
public:
    ...
    bool Execute() override
    {
        ...
        if (!bot || !target)
        {
            // TC_LOG_DEBUG("playerbots", "GroupInviteOperation: Bot or target not found");
            return false;
        }

        //By leewheel 2026-07-14: 组队操作日志
        ...
        if (target->GetGroup())
        {
            // TC_LOG_DEBUG("playerbots", "GroupInviteOperation: Target {} is already in a group", target->GetName());
            return false;
        }
        ...
        if (!group)
        {
            ...
            {
                delete group;
                TC_LOG_ERROR("playerbots", "GroupInviteOperation: Failed to create group for bot {}", bot->GetName());
                return false;
            }
            sGroupMgr->AddGroup(group);
            // TC_LOG_DEBUG("playerbots", "GroupInviteOperation: Created new group for bot {}", bot->GetName());
        }
        ...
        {
            group->ConvertToRaid();
            // TC_LOG_DEBUG("playerbots", "GroupInviteOperation: Converted group to raid");
        }

        // Add member to group
        if (group->AddMember(target))
        {
            // TC_LOG_DEBUG("playerbots", "GroupInviteOperation: Successfully added {} to group", target->GetName());
        ...
        {
            TC_LOG_ERROR("playerbots", "GroupInviteOperation: Failed to add {} to group", target->GetName());
            return false;
        }
    }
...
public:
    ...
    bool Execute() override
    {
        ...
        if (!group)
        {
            // TC_LOG_DEBUG("playerbots", "GroupRemoveMemberOperation: Bot is not in a group");
            return false;
        }

        if (!group->IsMember(target->GetGUID()))
        {
            // TC_LOG_DEBUG("playerbots", "GroupRemoveMemberOperation: Target is not in bot's group");
            return false;
        }

        group->RemoveMember(target->GetGUID());
        // TC_LOG_DEBUG("playerbots", "GroupRemoveMemberOperation: Removed {} from group", target->GetName());
        return true;
    }
...
public:
    ...
    bool Execute() override
    {
        ...
        if (!group)
        {
            // TC_LOG_DEBUG("playerbots", "GroupConvertToRaidOperation: Bot is not in a group");
            return false;
        }

        if (group->isRaidGroup())
        {
            // TC_LOG_DEBUG("playerbots", "GroupConvertToRaidOperation: Group is already a raid");
            return true;  // Success - already in desired state
        }

        group->ConvertToRaid();
        // TC_LOG_DEBUG("playerbots", "GroupConvertToRaidOperation: Converted group to raid");
        return true;
    }
...
public:
    ...
    bool Execute() override
    {
        ...
        if (!group)
        {
            // TC_LOG_DEBUG("playerbots", "GroupSetLeaderOperation: Bot is not in a group");
            return false;
        }

        if (!group->IsMember(newLeader->GetGUID()))
        {
            // TC_LOG_DEBUG("playerbots", "GroupSetLeaderOperation: New leader is not in the group");
            return false;
        }

        group->ChangeLeader(newLeader->GetGUID());
        group->SendUpdate();
        // TC_LOG_DEBUG("playerbots", "GroupSetLeaderOperation: Changed leader to {}", newLeader->GetName());
        return true;
    }
...
public:
    ...
    bool Execute() override
    {
        ...
        if (!leader)
        {
            TC_LOG_ERROR("playerbots", "ArenaGroupFormationOperation: Leader not found");
            return false;
        }
        ...
        for (const ObjectGuid& memberGuid : m_memberGuids)
        {
            ...
            if (memberGroup)
            {
                ...
                // TC_LOG_DEBUG("playerbots", "ArenaGroupFormationOperation: Removed {} from their existing group",
                //          member->GetName());
            }
        }
        ...
        if (leaderGroup)
        {
            ...
            TC_LOG_DEBUG("playerbots", "ArenaGroupFormationOperation: Disbanded leader's existing group");
        }
        ...
        {
            delete newGroup;
            TC_LOG_ERROR("playerbots", "ArenaGroupFormationOperation: Failed to create arena group for leader {}",
                     leader->GetName());
            return false;
        }

        sGroupMgr->AddGroup(newGroup);
        // TC_LOG_DEBUG("playerbots", "ArenaGroupFormationOperation: Created new arena group with leader {}",
        //          leader->GetName());
        ...
        for (const ObjectGuid& memberGuid : m_memberGuids)
        {
            ...
            if (!member)
            {
                // TC_LOG_DEBUG("playerbots", "ArenaGroupFormationOperation: Member {} not found, skipping",
                //          memberGuid.ToString());
                continue;
            }
            ...
            {
                // TC_LOG_DEBUG("playerbots", "ArenaGroupFormationOperation: Member {} is below level 70, skipping",
                //          member->GetName());
                continue;
            }
            ...
            {
                addedMembers++;
                // TC_LOG_DEBUG("playerbots", "ArenaGroupFormationOperation: Added {} to arena group",
                //          member->GetName());
            }
            else
                TC_LOG_ERROR("playerbots", "ArenaGroupFormationOperation: Failed to add {} to arena group",
                         member->GetName());
        }

        if (addedMembers == 0)
        {
            TC_LOG_ERROR("playerbots", "ArenaGroupFormationOperation: No members were added to the arena group");
            newGroup->Disband();
            return false;
        }
        ...
        for (const ObjectGuid& memberGuid : m_memberGuids)
        {
            ...
            // TC_LOG_DEBUG("playerbots", "ArenaGroupFormationOperation: Teleported {} to leader", member->GetName());
        }

        // Check if we have enough members
        if (newGroup->GetMembersCount() < m_requiredSize)
        {
            TC_LOG_INFO("playerbots", "Team #{} <{}> Group is not ready for match (not enough members: {}/{})",
                    m_arenaTeamId, m_arenaTeamName, newGroup->GetMembersCount(), m_requiredSize);
            newGroup->Disband();
            return false;
        }

        TC_LOG_INFO("playerbots", "Team #{} <{}> Group is ready for match with {} members",
