    ...
    else
    {
        ...
        switch (bot->getClass())
        {
            ...
            case CLASS_WARLOCK:
                ...
                break;
            //By leewheel 2026-07-23: DK无天赋时默认返回Frost(DPS)而非Blood(Tank)
            //原代码缺DK的case，默认返回tab=0即DEATH_KNIGHT_TAB_BLOOD(坦克)，
            //导致无天赋的DK bot被误判为坦克职责进入LFG随机本
            case CLASS_DEATH_KNIGHT:
...
void AiFactory::AddDefaultCombatStrategies(Player* player, PlayerbotAI* const facade, Engine* engine)
{
    ...
    //By leewheel 2026-08-29: 引入 NPCBots PVP 战斗策略——控制/打断/爆发/走位/保命/逃跑。
    //触发条件均为 PVP 专属(敌方是玩家), PVE 战斗中不会误触发。
    engine->addStrategy("pvp combat", false);
    //End By leewheel
    ...
    if (player->InBattleground() && player->GetBattleground())
    {
        ...
        if (bgType == BATTLEGROUND_RB)
            bgType = player->GetBattleground()->GetTypeID();
...
void AiFactory::AddDefaultNonCombatStrategies(Player* player, PlayerbotAI* const facade, Engine* nonCombatEngine)
{
    ...
    {
        case CLASS_PRIEST:
            //By leewheel 2026-08-02: 对齐来源项目AC——治疗NON_COMBAT不加heal策略
            //之前为"治疗慢"把heal/holy heal加进NON_COMBAT, 引入治疗施法占AI(超范围反复施法打断移动)
            //导致治疗bot不跟随队伍卡在副本内。AC原版治疗NON_COMBAT只加dps assist/cure(战斗才治疗),
            //非战斗不施法→跟随正常, 战斗治疗(COMBAT引擎heal策略)不受影响。
        ...
        case CLASS_PALADIN:
            if (tab == PALADIN_TAB_PROTECTION)
            {
                ...
                //By leewheel 2026-08-02: 对齐来源项目AC——奶骑NON_COMBAT不加heal策略(防施法占AI不跟随)
        ...
        case CLASS_HUNTER:
            ...
            break;
        case CLASS_SHAMAN:
            //By leewheel 2026-08-02: 对齐来源项目AC——恢复萨NON_COMBAT不加resto策略(防施法占AI不跟随)
        ...
        case CLASS_DRUID:
            ...
            //By leewheel 2026-08-02: 对齐来源项目AC——恢复德NON_COMBAT不加resto策略(防施法占AI不跟随)
            else
                nonCombatEngine->addStrategiesNoInit("dps assist", "cure", nullptr);
            //End By leewheel
            break;
        ...
        case CLASS_WARLOCK:
            ...
            //By leewheel 2026-07-22: 灵魂石应绑定给坦克而非术士自己
            nonCombatEngine->addStrategiesNoInit("dps assist", "ss tank", nullptr);
            break;
        case CLASS_DEATH_KNIGHT:
            if (tab == DEATH_KNIGHT_TAB_BLOOD)
                nonCombatEngine->addStrategiesNoInit("tank assist", "pull", nullptr);
    ...
    if ((sRandomPlayerbotMgr.IsRandomBot(player)) && !player->InBattleground())
    {
        ...
        {
            //By leewheel 2026-07-14: 单机模式诊断日志已清理
            //End By leewheel
        ...
        {
            // By leewheel 2026-07-17: 诊断日志已清理 - else分支(机器人已在队伍)
            //End By leewheel
            ...
            if (facade)
            {
                if (master)
                {
                    ...
                    // By leewheel 2026-07-17: 显式移除单机模式的idle策略，防止在组队时残留new rpg
                    // 问题：机器人在solo时AddDefaultNonCombatStrategies会添加new rpg/grind/rpg/move random�?                    //      但else分支(已组�?从未显式移除这些策略，导致组队后机器人仍执行new rpg动作
                    ...
                    nonCombatEngine->removeStrategy("move random", false);
                    nonCombatEngine->removeStrategy("bg", false);
                    nonCombatEngine->removeStrategy("lfg", false);
