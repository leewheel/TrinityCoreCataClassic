...
void PerfMonitor::PrintStats(bool perTick, bool fullStack)
{
    ...
    if (!perTick)
    {
             ...
             i != data.end(); ++i)
        {
            ...
            {
                if (key == "总计" && j->first.find("PlayerbotAI::UpdateAIInternal") == std::string::npos)
                    continue;
            ...
            TC_LOG_INFO("playerbots", "{:7.3f}% {:10.3f}s | {:7.1f} .. {:7.1f} ({:10.3f} of {:10d}) - {:6}    : {}", tPerc,
                     tTime, tMinTime, tMaxTime, tAvg, typeCount, key.c_str(), "总计");
            TC_LOG_INFO("playerbots", " ");
        }
    }
    else
    {
             ...
             i != data.end(); ++i)
        {
            ...
            if (i->first != PERF_MON_TOTAL)
            {
                ...
                TC_LOG_INFO("playerbots", "{:7.3f}% {:9.3f}ms | {:7.1f} .. {:7.1f} ({:10.3f} of {:10.2f}) - {:6}    : {}",
                         tPerc, tTime, tMinTime, tMaxTime, tAvg, tAmount, key.c_str(), "总计");
            }
            TC_LOG_INFO("playerbots", " ");
        }
    }
...
PerfMonitorOperation::PerfMonitorOperation(PerformanceData* data, std::string const name, PerformanceStack* stack)
