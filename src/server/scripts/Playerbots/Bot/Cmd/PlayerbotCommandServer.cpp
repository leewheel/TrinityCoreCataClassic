...
//By leewheel 2026-07-12: 使用std::thread替代boost::thread，避免链接boost thread库
...
void session(socket_ptr sock)
{
    ...
    {
        TC_LOG_ERROR("playerbots", "{}", e.what());
    }
}
...
void server(Trinity::Asio::IoContext& io_service, short port)
{
    ...
    for (;;)
    {
        ...
        a.accept(*sock);
        //By leewheel 2026-07-12: 使用std::thread和lambda替代boost::thread和boost::bind
        std::thread t([sock]() { session(sock); });
        t.detach();
        //End By leewheel
    }
}
...
void Run()
{
    ...
    TC_LOG_INFO("playerbots", "{}", s.str().c_str());
