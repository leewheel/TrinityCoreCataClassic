...
bool RpgEmoteAction::Execute(Event /*event*/)
{
    ...
    WPPCompat::DoGossipHello(bot->GetSession(), rpg->guid());

    //By leewheel 2025-07-10
    // TC使用多参数版本HandleEmoteCommand
    bot->HandleEmoteCommand(Emote(type), nullptr, {}, 0);
    //End By leewheel
...
bool RpgTaxiAction::Execute(Event /*event*/)
{
    ...
    if (nodes.empty())
    {
        TC_LOG_ERROR("playerbots", "Bot {} - No flight paths available", bot->GetName());
        return false;
    }
    ...
    //By leewheel 2026-09-03 修复C4189警告：nodeFrom/nodeTo在诊断日志恢复后才有意义，恢复log(log中文)
    ...
    Creature* flightMaster = bot->GetNPCIfCanInteractWith(guidP, UNIT_NPC_FLAG_FLIGHTMASTER, UNIT_NPC_FLAG_2_NONE); // By leewheel 2026-07-08
    if (!flightMaster)
    {
        TC_LOG_ERROR("playerbots", "Bot {} cannot talk to flightmaster ({} location available)", bot->GetName(),
                  nodes.size());
        return false;
    }
    ...
    {
        TC_LOG_ERROR("playerbots", "Bot {} cannot fly {} ({} location available)", bot->GetName(), path, nodes.size());
        return false;
    }

    //By leewheel 2025-07-10
    // TC使用TC_LOG_INFO，log字符串使用中文
    TC_LOG_INFO("playerbots", "Bot {} <{}> 正在从 {} 飞往 {} ({} 个位置可用)",
             bot->GetGUID().ToString().c_str(), bot->GetName(), nodeFrom->Name[LOCALE_enUS], nodeTo->Name[LOCALE_enUS], nodes.size());
    //End By leewheel
...
bool RpgDuelAction::isUseful()
{
    ...
    if (casterAreaEntry && !(casterAreaEntry->flags() & AREA_FLAG_ALLOW_DUELS)) //By leewheel 2026-07-10: TC中flags是方法
    {
...
{
    WPPCompat::MountSpecialAnim(bot->GetSession());
