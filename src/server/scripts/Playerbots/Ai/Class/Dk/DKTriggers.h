...
#define PLAYERBOTS_DKTRIGGERS_H
...
public:
    ...
    std::vector<uint32> GetFallbackSpellIds() const override { return { 51123, 51124, 51127, 51128, 51129, 51130 }; }
};
...
public:
    ...
    std::vector<uint32> GetFallbackSpellIds() const override { return { 59052 }; }
};
...
//By leewheel 2026-07-30: 三个姿态检测 trigger,用于在坦克/DPS 失去正确姿态时强制切回
//原 DKPresenceTrigger 名字叫 "blood presence" 但 IsActive 检测所有 presence 都没有,语义错乱
//重构为三个独立 trigger, BloodDKStrategy 监听 "no blood presence", FrostDKStrategy 监听 "no frost presence", UnholyDKStrategy 监听 "no unholy presence"
class NoBloodPresenceTrigger : public BuffTrigger
{
public:
    NoBloodPresenceTrigger(PlayerbotAI* botAI) : BuffTrigger(botAI, "blood presence") {}
    bool IsActive() override;
};

class NoFrostPresenceTrigger : public BuffTrigger
{
public:
    NoFrostPresenceTrigger(PlayerbotAI* botAI) : BuffTrigger(botAI, "frost presence") {}
    bool IsActive() override;
};

class NoUnholyPresenceTrigger : public BuffTrigger
{
public:
    NoUnholyPresenceTrigger(PlayerbotAI* botAI) : BuffTrigger(botAI, "unholy presence") {}
    bool IsActive() override;
};
//End By leewheel
