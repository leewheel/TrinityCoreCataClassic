...
// By leewheel 2026-08-07: Shaman Shock 系列 spell ID (依据 classic_db2.spelllevels 玩家可学版本)
// 用于 ShockTrigger 的 locale 匹配失败 fallback, 防止误判 shock CD 导致乱放技能
namespace {
// Flame Shock 全等级
constexpr uint32 FLAME_SHOCK_SPELL_IDS[] = {
    8050, 8052, 8053, 10447, 10448, 25457, 49232, 49233
};
// Frost Shock 全等级
constexpr uint32 FROST_SHOCK_SPELL_IDS[] = {
...
{
    // By leewheel 2026-08-07: 名称检查 + spell ID fallback, 防止 locale 匹配失败导致误判 shock 状态
    Unit* target = GetTarget();
    if (!target)
        return false;
    bool hasFlameShock = botAI->HasAura("flame shock", target, false, true);
    if (!hasFlameShock)
        hasFlameShock = HasAuraFromList(target, FLAME_SHOCK_SPELL_IDS, std::size(FLAME_SHOCK_SPELL_IDS));
    bool hasFrostShock = botAI->HasAura("frost shock", target, false, true);
    if (!hasFrostShock)
        hasFrostShock = HasAuraFromList(target, FROST_SHOCK_SPELL_IDS, std::size(FROST_SHOCK_SPELL_IDS));
    return SpellTrigger::IsActive() && !hasFlameShock && !hasFrostShock;
}
