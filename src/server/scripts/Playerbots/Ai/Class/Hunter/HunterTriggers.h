...
public:
    ...
    bool IsActive() override
    {
        ...
        if (botAI->HasAura("lock and load", botAI->GetBot()))
            return true;
        for (uint32 spellId : GetFallbackSpellIds())
            if (botAI->GetBot()->HasAura(spellId)) return true;
        return false;
    }
    std::vector<uint32> GetFallbackSpellIds() const override { return { 56342, 56343, 56344, 56453 }; }
};
