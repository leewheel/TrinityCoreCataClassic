...
//By leewheel 2026-08-06: 添加spell ID fallback检查,防止locale名称匹配失败导致重复加BUFF
    ...
    PlayerbotAI* botAI, Unit* target, std::initializer_list<char const*> auraNames)
{
    ...
    // 2. 名称匹配失败时,用spell ID检测(可靠,不受locale影响)
...
// By leewheel 2026-08-07: Paladin Actions spell ID fallback, 防止 locale 匹配失败导致技能判断错误
namespace paladin_actions {
// Blessing of Sanctuary 全版本
constexpr uint32 BLESSING_OF_SANCTUARY_SPELL_IDS[] = { 20911, 57319, 57320, 57321, 67480 };
// Greater Blessing of Sanctuary (玩家唯一版本)
constexpr uint32 GREATER_BLESSING_OF_SANCTUARY_SPELL_IDS[] = { 25899 };
// Divine Guardian 全版本
constexpr uint32 DIVINE_GUARDIAN_SPELL_IDS[] = { 53527, 53530, 70940 };
// Divine Sacrifice (玩家唯一版本)
constexpr uint32 DIVINE_SACRIFICE_SPELL_IDS[] = { 64205 };
...
static inline bool IsTankRole(Player* player)
{
    if (!player)
        ...
        //By leewheel 2026-07-10: TC的Player没有HasTankSpec，使用PlayerbotAI::IsTank替代
...
inline std::string const GetActualBlessingOfSanctuary(Unit* target, Player* bot)
{
    ...
    if (auto* botAI = GET_PLAYERBOT_AI(bot))
    {
        ...
        //By leewheel 2026-07-10: TC的Player没有HasTankSpec，使用PlayerbotAI::IsTank替代
        if (PlayerbotAI::IsTank(targetPlayer, true))
        //End By leewheel
        return "blessing of sanctuary";
...
bool CastBlessingOfSanctuaryOnPartyAction::Execute(Event /*event*/)
{
    ...
    // By leewheel 2026-08-07: 名称检查 + spell ID fallback, 防止 locale 匹配失败导致重复加庇护祝福
    const auto HasSanctAura = [&](Unit* unit) -> bool {
        return botAI->HasAura("blessing of sanctuary", unit) ||
               ...
               paladin_actions::HasAuraFromList(unit, paladin_actions::BLESSING_OF_SANCTUARY_SPELL_IDS, std::size(paladin_actions::BLESSING_OF_SANCTUARY_SPELL_IDS)) ||
               paladin_actions::HasAuraFromList(unit, paladin_actions::GREATER_BLESSING_OF_SANCTUARY_SPELL_IDS, std::size(paladin_actions::GREATER_BLESSING_OF_SANCTUARY_SPELL_IDS));
    };
...
bool CastBlessingOfKingsOnPartyAction::Execute(Event /*event*/)
{
    ...
    if (targetPlayer)
    {
        ...
        const bool hasSanctAny =
            ...
            paladin_actions::HasAuraFromList(target, paladin_actions::BLESSING_OF_SANCTUARY_SPELL_IDS, std::size(paladin_actions::BLESSING_OF_SANCTUARY_SPELL_IDS)) ||
            paladin_actions::HasAuraFromList(target, paladin_actions::GREATER_BLESSING_OF_SANCTUARY_SPELL_IDS, std::size(paladin_actions::GREATER_BLESSING_OF_SANCTUARY_SPELL_IDS));
...
{
    // By leewheel 2026-08-07: 名称检查 + spell ID fallback, 防止 locale 匹配失败导致重复施放神圣牺牲
    Unit* target = GetTarget();
    if (!target || !CastSpellAction::isUseful())
        return false;
    return !botAI->HasAura("divine guardian", target, false, false, -1, true) &&
           !paladin_actions::HasAuraFromList(target, paladin_actions::DIVINE_GUARDIAN_SPELL_IDS, std::size(paladin_actions::DIVINE_GUARDIAN_SPELL_IDS));
}
...
{
    // By leewheel 2026-08-07: 名称检查 + spell ID fallback, 防止 locale 匹配失败导致无法识别神圣牺牲状态
    Unit* target = GetTarget();
    if (!target) return false;
    return botAI->HasAura("divine sacrifice", target, false, true, -1, true) ||
           paladin_actions::HasAuraFromList(target, paladin_actions::DIVINE_SACRIFICE_SPELL_IDS, std::size(paladin_actions::DIVINE_SACRIFICE_SPELL_IDS));
}
