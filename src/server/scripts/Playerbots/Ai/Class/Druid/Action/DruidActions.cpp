...
// By leewheel 2026-08-07: Druid Actions spell ID fallback, 防止 locale 匹配失败导致技能判断错误
namespace druid_actions {
// Innervate (玩家唯一版本)
constexpr uint32 INNERVATE_SPELL_IDS[] = { 29166 };
// Rejuvenation 全等级 (玩家可学版本)
constexpr uint32 REJUVENATION_SPELL_IDS[] = {
    774, 1058, 1430, 2090, 2091, 3627, 8910, 9839, 9840, 9841,
    25299, 26981, 48440, 48441
};
...
bool CastStarfallAction::isUseful()
{
    ...
    for (ObjectGuid const& guid : nearbyNpcs)
    {
        ...
        if (unit->HasUnitState(UNIT_STATE_UNATTACKABLE) ||
            unit->HasUnitFlag(UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NON_ATTACKABLE_2 |
                              UNIT_FLAG_NOT_ATTACKABLE_1 | UNIT_FLAG_ON_TAXI) ||
            unit->IsUninteractible() ||
            unit->IsImmuneToPC())
            continue;
        //End By leewheel
...
bool CastInnervateOnHealerAction::isPossible()
{
    ...
    // By leewheel 2026-08-07: 名称检查 + spell ID fallback, 防止 locale 匹配失败导致重复施放激活
    if (botAI->HasAura("innervate", target) ||
        druid_actions::HasAuraFromList(target, druid_actions::INNERVATE_SPELL_IDS, std::size(druid_actions::INNERVATE_SPELL_IDS)))
        return false;
...
Unit* CastRejuvenationOnNotFullAction::GetTarget()
{
    ...
    for (GroupReference* gref = group->GetFirstMember(); gref; gref = gref->next())
    {
        ...
        // By leewheel 2026-08-07: 名称检查 + spell ID fallback, 防止 locale 匹配失败导致重复施放回春
        if (botAI->HasAura("rejuvenation", player) ||
            druid_actions::HasAuraFromList(player, druid_actions::REJUVENATION_SPELL_IDS, std::size(druid_actions::REJUVENATION_SPELL_IDS)))
        {
            continue;
        }
