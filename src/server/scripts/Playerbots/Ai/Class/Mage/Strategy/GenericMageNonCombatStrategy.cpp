...
void GenericMageNonCombatStrategy::InitTriggers(std::vector<TriggerNode*>& triggers)
{
    ...
    // By leewheel 2026-07-22: 法师没水/没食物时自动造水造食，否则DrinkAction因背包无饮料永远无法执行
    triggers.push_back(new TriggerNode("no drink", { NextAction("conjure water", 20.0f) }));
    triggers.push_back(new TriggerNode("no food", { NextAction("conjure food", 19.0f) }));
}
