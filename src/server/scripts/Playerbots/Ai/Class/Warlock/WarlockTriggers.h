...
// By leewheel 2026-08-07: 移除内联 IsActive, 改在 cpp 中实现以使用 spell ID fallback
// 防止 locale 匹配失败导致 HasAura("seed of corruption") 永远返回 false, 进而重复施放腐蚀术
...
public:
    ...
    std::vector<uint32> GetFallbackSpellIds() const override { return { 17941 }; }
};
...
public:
    ...
    std::vector<uint32> GetFallbackSpellIds() const override { return { 34935, 34936, 34938, 34939, 37537, 51439, 69746, 69770 }; }
};
...
public:
    ...
    bool IsActive() override;
    std::vector<uint32> GetFallbackSpellIds() const override { return { 63156, 63158, 63165, 63167 }; }
};
...
public:
    ...
    std::vector<uint32> GetFallbackSpellIds() const override { return { 47245, 47246, 47247, 47383, 71162, 71165 }; }
};
