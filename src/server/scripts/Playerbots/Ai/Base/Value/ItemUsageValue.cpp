...
namespace
{
    //By leewheel 2026-09-04: 修复配方"已学"判定槽位错位——
    //TC343 的 ItemTemplate::Effects 是带 LegacySlotIndex 的紧凑向量, 与 AC Spells[0..4] 固定槽位
    //不做对齐(配方常只有 1 条效果, Effects.size()>2 恒假 → HasSpell(0) → 已学配方被反复判"未学"),
    //改为按 AC 语义匹配: 先取遗留槽 2(LegacySlotIndex==2) 的法术, 找不到再回退扫 ON_LEARN 触发效果
    uint32 GetRecipeLearnSpellId(ItemTemplate const* proto)
    {
        if (!proto)
            return 0;

        for (ItemEffectEntry const* effect : proto->Effects)
            if (effect && effect->LegacySlotIndex == 2)
                return effect->SpellID;

        for (ItemEffectEntry const* effect : proto->Effects)
            if (effect && effect->TriggerType == ITEM_SPELLTRIGGER_ON_LEARN)
                return effect->SpellID;
...
ItemUsage ItemUsageValue::Calculate()
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (proto->GetClass() == ITEM_CLASS_TRADE_GOODS || proto->GetClass() == ITEM_CLASS_MISC ||
                proto->GetClass() == ITEM_CLASS_REAGENT)
                needItem = IsItemNeededForUsefullSpell(proto, lowBagSpace);
            else if (proto->GetClass() == ITEM_CLASS_RECIPE)
            {
                if (bot->HasSpell(GetRecipeLearnSpellId(proto)))
                    needItem = false;
                else
    ...
    if (proto->GetClass() == ITEM_CLASS_KEY)
        return ITEM_USAGE_USE;

    const uint32_t maxCount = proto->GetMaxCount();
    ...
    Item* item = bot->GetItemByEntry(proto->GetId());
    ...
    if ((proto->GetClass() == ITEM_CLASS_ARMOR || proto->GetClass() == ITEM_CLASS_WEAPON) &&
        ...
        proto->GetQuality() >= ITEM_QUALITY_UNCOMMON)
    {
    ...
    if (proto->GetClass() == ITEM_CLASS_PROJECTILE && bot->CanUseItem(proto) == EQUIP_ERR_OK)
    {
    ...
    {
        if (proto->GetQuality() >= ITEM_QUALITY_NORMAL && !isSoulbound && proto->GetBonding() != BIND_WHEN_PICKED_UP)
...
ItemUsage ItemUsageValue::QueryItemUsageForEquip(ItemTemplate const* itemProto, int32 randomPropertyId)
{
    ...
    if (itemProto->GetInventoryType() == INVTYPE_NON_EQUIP)
        return ITEM_USAGE_NONE;

    Item* pItem = Item::CreateItem(itemProto->GetId(), 1, ItemContext::NONE, bot);
    if (!pItem)
        return ITEM_USAGE_NONE;
    ...
    if (needToCheckUnique)
    {
        ...
        uint32 totalItemCount = bot->GetItemCount(itemProto->GetId(), true);

        // Count the number of the item in bags only
        uint32 bagItemCount = bot->GetItemCount(itemProto->GetId(), false);
    ...
    if (itemProto->GetClass() == ITEM_CLASS_QUIVER && bot->getClass() != CLASS_HUNTER)
        return ITEM_USAGE_NONE;

    if (itemProto->GetClass() == ITEM_CLASS_CONTAINER)
    {
        if (itemProto->GetSubClass() != ITEM_SUBCLASS_CONTAINER)
        ...
        if (GetSmallestBagSize() >= itemProto->GetContainerSlots())
            return ITEM_USAGE_NONE;

        return ITEM_USAGE_EQUIP;
    }

    if (itemProto->GetClass() == ITEM_CLASS_WEAPON && itemProto->GetSubClass() == ITEM_SUBCLASS_WEAPON_MISC)
        return ITEM_USAGE_NONE;

    bool shouldEquip = false;
    // uint32 statWeight = sRandomItemMgr.GetLiveStatWeight(bot, itemProto->GetId());
    ...
    float itemScore = calculator.CalculateItem(itemProto->GetId(), randomPropertyId);

    if (itemScore)
        shouldEquip = true;

    if (itemProto->GetClass() == ITEM_CLASS_WEAPON && !sRandomItemMgr.CanEquipWeapon(itemProto, bot->getClass()))
        shouldEquip = false;
    if (itemProto->GetClass() == ITEM_CLASS_ARMOR &&
    ...
    if (dstSlot == EQUIPMENT_SLOT_MAINHAND)
    {
        ...
        have2HWeapon = currentWeapon && currentWeapon->GetTemplate()->GetInventoryType() == INVTYPE_2HWEAPON;
        ...
        if (bot->CanTitanGrip() && itemProto->GetInventoryType() == INVTYPE_2HWEAPON && !isValidTGWeapon)
            return ITEM_USAGE_NONE;
        ...
        if (bot->CanDualWield() &&
            ((itemProto->GetInventoryType() != INVTYPE_2HWEAPON && !have2HWeapon) ||
    ...
    for (uint8 i = 0; i < possibleSlots; i++)
    {
        ...
        float oldScore = calculator.CalculateItem(oldItemProto->GetId(), oldItem->GetItemRandomPropertyId());
        if (oldItem)
        {
            // uint32 oldStatWeight = sRandomItemMgr.GetLiveStatWeight(bot, oldItemProto->GetId());
            if (itemScore || oldScore)
                shouldEquipInSlot = itemScore > oldScore * sPlayerbotAIConfig.equipUpgradeThreshold;
        }

        // Bigger quiver
        if (itemProto->GetClass() == ITEM_CLASS_QUIVER)
        {
            if (!oldItem || oldItemProto->GetContainerSlots() < itemProto->GetContainerSlots())
                return ITEM_USAGE_EQUIP;
            else
                return ITEM_USAGE_NONE;
        }
        ...
        if (oldItemProto->GetClass() == ITEM_CLASS_WEAPON && !sRandomItemMgr.CanEquipWeapon(oldItemProto, bot->getClass()))
            existingShouldEquip = false;

        if (oldItemProto->GetClass() == ITEM_CLASS_ARMOR &&
            !sRandomItemMgr.CanEquipArmor(oldItemProto, bot->getClass(), bot->GetLevel()))
            existingShouldEquip = false;

        // uint32 oldItemPower = sRandomItemMgr.GetLiveStatWeight(bot, oldItemProto->GetId());
        // uint32 newItemPower = sRandomItemMgr.GetLiveStatWeight(bot, itemProto->GetId());

        // Compare items based on item level, quality or itemId.
        bool isBetter = false;
        if (itemScore > oldScore)
            isBetter = true;
        // else if (newItemPower == oldScore && itemProto->GetQuality() > oldItemProto->GetQuality())
        //     isBetter = true;
        // else if (newItemPower == oldScore && itemProto->GetQuality() == oldItemProto->GetQuality() && itemProto->GetId() >
        // oldItemProto->GetId())
        //     isBetter = true;

        Item* item = CurrentItem(itemProto);
        bool itemIsBroken =
            item && item->GetDurability() == 0 && item->GetMaxDurability() > 0;
        bool oldItemIsBroken =
            oldItem->GetDurability() == 0 && oldItem->GetMaxDurability() > 0;

        if (itemProto->GetId() != oldItemProto->GetId() && (shouldEquipInSlot || !existingShouldEquip) && isBetter)
        {
            switch (itemProto->GetClass())
            {
                case ITEM_CLASS_ARMOR:
                    if (oldItemProto->GetSubClass() <= itemProto->GetSubClass())
                    {
...
ItemUsage ItemUsageValue::QueryItemUsageForAmmo(ItemTemplate const* proto)
{
    ...
    if (rangedWeapon)
    {
        switch (rangedWeapon->GetTemplate()->GetSubClass())
        {
    ...
    if (proto->GetSubClass() == requiredSubClass)
    {
        ...
        if (currentAmmoProto)
        {
            uint32 currentAmmoDPS = (currentAmmoProto->GetMinDamage(0) + currentAmmoProto->GetMaxDamage(0)) * 1000 / 2;
            uint32 newAmmoDPS = (proto->GetMinDamage(0) + proto->GetMaxDamage(0)) * 1000 / 2;
...
bool ItemUsageValue::IsItemUsefulForQuest(Player* player, ItemTemplate const* proto)
{
    ...
    for (uint8 slot = 0; slot < MAX_QUEST_LOG_SIZE; ++slot)
    {
        ...
        for (uint8 i = 0; i < 4; i++)
        {
            if (quest->RequiredItemId[i] == proto->GetId())
            {
                if (player->GetItemCount(proto->GetId(), false) >= quest->RequiredItemCount[i])
                    continue;
        ...
        for (ItemEffectEntry const* effect : proto->Effects)
        {
            ...
            {
                if (spellInfo->GetEffects()[effectIndex].Effect == SPELL_EFFECT_CREATE_ITEM)
                {
                    uint32 createdItemId = spellInfo->GetEffects()[effectIndex].ItemType;
...
{
    for (auto spellId : SpellsUsingItem(proto->GetId(), bot))
    {
        SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spellId);
        if (!spellInfo)
            continue;
        ...
        uint32 newItemId = spellInfo->GetEffects()[EFFECT_0].ItemType;
        if (newItemId && newItemId != proto->GetId())
        {
...
bool ItemUsageValue::HasItemsNeededForSpell(uint32 spellId, ItemTemplate const* proto)
{
    ...
    for (uint8 i = 0; i < MAX_SPELL_REAGENTS; i++)
        ...
        {
            if (proto && proto->GetId() == uint32(spellInfo->Reagent[i]) &&
...
Item* ItemUsageValue::CurrentItem(ItemTemplate const* proto)
{
    ...
    for (auto item : found)
    {
        if (bestItem && item->GetDurability() < bestItem->GetDurability())
            continue;
...
float ItemUsageValue::BetterStacks(ItemTemplate const* proto, std::string const itemType)
{
    ...
    for (auto& otherItem : items)
    {
        ...
        if (otherProto->GetClass() != proto->GetClass() || otherProto->GetSubClass() != proto->GetSubClass())
            continue;

        if (otherProto->GetItemLevel() < proto->GetItemLevel())
            continue;

        if (otherProto->GetId() == proto->GetId())
            continue;
...
std::vector<uint32> ItemUsageValue::SpellsUsingItem(uint32 itemId, Player* bot)
{
    ...
    for (auto& spell : spellMap)
    {
        ...
        if (spellInfo->GetEffects()[EFFECT_0].Effect != SPELL_EFFECT_CREATE_ITEM)
            continue;
...
std::string const ItemUsageValue::GetConsumableType(ItemTemplate const* proto, bool hasMana)
{
    ...
    {
        if ((proto->Effects.empty() ? 0 : proto->Effects[0]->SpellCategoryID) == 11)
            return "food";
        else if ((proto->Effects.empty() ? 0 : proto->Effects[0]->SpellCategoryID) == 59 && hasMana)
            return "drink";
    }
    ...
    if (proto->GetSubClass() == ITEM_SUBCLASS_POTION || proto->GetSubClass() == ITEM_SUBCLASS_FLASK)
    {
        for (ItemEffectEntry const* effect : proto->Effects)
        {
            ...
            if (spellInfo)
                for (int i = 0; i < 3; i++)
                {
                    if (spellInfo->GetEffects()[i].Effect == SPELL_EFFECT_ENERGIZE && hasMana)
                        return "mana potion";
    ...
    if (proto->GetSubClass() == ITEM_SUBCLASS_BANDAGE)
    {
        return "bandage";
    }
