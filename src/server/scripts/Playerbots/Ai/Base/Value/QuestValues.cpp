...
// quest map.
//By leewheel 2026-08-18: 移植 brighton-chi the-lab 6fb08816(索引掉落者修正#2626)——上游按 creature_multispawn
//的 id/id2/id3 三个候选刷怪条目索引任务目标;TC 343 无 creature_multispawn 表(CreatureData 只有 id),
//故只处理基础条目,并保留上游的防御性查找(无任务关联时不再向 relationMap 插入空项)
        ...
        for (auto& entry : qPair.second[(int)QuestRelationFlag::questGiver])
        {
            for (auto& guidp : entry.second)
            {
                ...
                if (hasQualifier)
                {
                    ...
                    //By leewheel 2026-09-03 修复C4018警告：level为uint32，GetQuestMinLevel返回int32，显式转换
                    if (quest && (static_cast<int32>(level) < quest->GetQuestMinLevel() || (int)level > quest->GetQuestLevel() + 10))
                        continue;
                    //End By leewheel
                }
    ...
    for (auto& qGiver : qGivers)
    {
        ...
        for (auto& guidp : qGiver.second)
        {
            ...
            if (creatureTemplate)
            {
                ...
                FactionTemplateEntry const* creatureFaction = sFactionTemplateStore.LookupEntry(creatureTemplate->faction);
                if (WorldObject::GetFactionReactionTo(creatureFaction, bot) < REP_FRIENDLY)
    ...
    for (auto& questStatus : questStatusMap)
    {
        ...
        for (auto& entry : qt->second)
        {
            if (entry.first > 0)
            {
                ...
                {
                    //By leewheel 2025-07-10
                    // TC中GetFactionReactionTo是静态方法,需要两个参数
                    FactionTemplateEntry const* creatureFaction = sFactionTemplateStore.LookupEntry(info->faction);
                    if (WorldObject::GetFactionReactionTo(creatureFaction, bot) < REP_FRIENDLY)
                    //End By leewheel 2025-07-10
                        continue;
                }
...
std::vector<GuidPosition> ActiveQuestObjectivesValue::Calculate()
{
    ...
    for (auto& questStatus : questStatusMap)
    {
        ...
        for (uint32 objective = 0; objective < QUEST_OBJECTIVES_COUNT; objective++)
        {
            ...
            //By leewheel 2026-07-24: 修复杀怪/GO目标判断用错字段 - 原代码用RequiredItemId(恒为0)导致杀怪目标永远被跳过
...
uint32 DialogStatusValue::getDialogStatus(Player* bot, int32 questgiver, uint32 questId)
{
    ...
    //By leewheel 2025-07-10
    // TC中QuestRelationResult使用范围for循环,迭代器解引用直接返回questId
    for (uint32 qId : irbounds)
    {
        if (questId && qId != questId)
            continue;
