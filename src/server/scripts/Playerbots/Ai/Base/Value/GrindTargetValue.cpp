...
Unit* GrindTargetValue::FindTargetForGrinding(uint32 assistCount)
{
    ...
    if (targets.empty())
    {
        static std::map<ObjectGuid, uint32> logCount;
        auto& cnt = logCount[bot->GetGUID()];
        if (cnt < 3)
        {
            ++cnt;
            //By leewheel 2026-07-17: GrindDiag日志已禁用
            //End By leewheel
        }
        return nullptr;
    }
    ...
    for (ObjectGuid const guid : targets)
    {
            ...
            ServerFacade::instance().GetDistance2d(master, unit) > sPlayerbotAIConfig.lootDistance)
        {
            if (botAI->HasStrategy("debug grind", BotState::BOT_STATE_NON_COMBAT))
                ...
                botAI->TellMaster(chat->FormatWorldobject(unit) + " 已忽略(离主人太远)。");
                //End By leewheel
            continue;
        }
        ...
        if (Creature* creature = unit->ToCreature())
            if (CreatureTemplate const* CreatureTemplate = creature->GetCreatureTemplate())
                ...
                if (CreatureTemplate->Classification > CREATURE_ELITE_NORMAL && !AI_VALUE(bool, "can fight elite"))
                    continue;
