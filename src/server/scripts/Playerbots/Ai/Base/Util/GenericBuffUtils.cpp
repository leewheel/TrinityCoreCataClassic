...
// 添加PlayerbotTextMgr.h头文件
#include "PlayerbotTextMgr.h"
//End By leewheel 2025-01-16
//By leewheel 2026-08-23: 合并 the-lab 引入 SpellAuras.h(BuffBelowRefreshTarget 使用 Aura 完整类型)
...
namespace ai::buff
{
    namespace
    {
        ...
        bool IsWithinPostLoginBuffGrace(Player* player)
        {
            ...
            //By leewheel 2026-07-10: TC中GameTime::GetGameTimeMS()返回uint32而非duration类型，不需要.count()
        ...
        PlayerbotAI* botAI, std::string const& baseName, std::string const& groupName)
    {
        ...
        //By leewheel 2026-07-10 TC中GetGameTimeMS()返回uint32，不需要.count()
        auto const now = GameTime::GetGameTimeMS();
        //End By leewheel
...
namespace ai::spell
{
    bool HasSpellOrCategoryCooldown(Player* bot, uint32 spellId)
    {
        ...
        // TC使用GetSpellHistory()->GetRemainingCategoryCooldown替代GetSpellCooldownMap遍历
        // 原AC代码遍历GetSpellCooldownMap()检查category冷却，TC中直接用GetRemainingCategoryCooldown
        if (bot->GetSpellHistory()->GetRemainingCategoryCooldown(category).count() > 0)
            return true;
        //End By leewheel
