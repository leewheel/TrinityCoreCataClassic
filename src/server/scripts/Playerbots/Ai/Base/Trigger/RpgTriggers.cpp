...
bool RpgTaxiTrigger::IsActive()
{
    ...
    //By leewheel 2026-07-10: TC中ObjectGuid没有静态GetMapId方法，使用GuidPosition自身的GetMapId()
    uint32 node =
...
bool RpgDuelTrigger::IsActive()
{
    ...
    //By leewheel 2026-07-10: TC的HasIgnore需要两个参数(guid, accountGuid)
    if (bot->duel || player->duel || !player->GetSocial() || player->GetSocial()->HasIgnore(bot->GetGUID(), bot->GetSession()->GetAccountGUID()))
    //End By leewheel 2026-07-10
        return false;

    AreaTableEntry const* targetAreaEntry = sAreaTableStore.LookupEntry(player->GetAreaId());
    if (targetAreaEntry && !(targetAreaEntry->flags() & AREA_FLAG_ALLOW_DUELS)) //By leewheel 2026-07-10: TC中flags是方法
    {
