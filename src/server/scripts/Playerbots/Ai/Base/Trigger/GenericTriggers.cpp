...
//By leewheel 2026-08-06: 修复逗号分隔的法术名导致GetAura永远无法匹配的问题
//Paladin的BlessingOfMightOnPartyTrigger等trigger的spell是"blessing of might,blessing of wisdom"
//原代码直接用整个逗号字符串调用GetAura,Utf8toWStr转换后长度永远不匹配,GetAura永远返回nullptr
//导致IsActive永远返回true,bot一直尝试加buff
//修复: 拆分逗号分隔的法术名后逐个检查GetAura,任一aura存在且未过期就返回false
...
//By leewheel 2026-08-23: 合并 the-lab(#2571) —— 保留拆分+fallback 修复的同时,
    // 用 ai::buff::BuffBelowRefreshTarget 替代简单 beforeDuration 比较(支持 forceRebuff 刷新线)
    std::vector<std::string> auraNames = split(spell, ',');
    for (std::string const& auraName : auraNames)
    {
        Aura* aura = botAI->GetAura(auraName, target, checkIsOwner, checkDuration);
        if (aura && !ai::buff::BuffBelowRefreshTarget(botAI, aura, beforeDuration))
            return false;
    }

    // By leewheel 2026-08-07: 名称匹配认为没有 aura 时, 用 spell ID fallback 确认
    // 防止 locale 匹配失败导致重复施放 buff
    std::vector<uint32> fallbackIds = GetFallbackSpellIds();
    for (uint32 spellId : fallbackIds)
    {
        if (Aura* aura = target->GetAura(spellId))
        {
            if (!checkIsOwner || aura->GetCasterGUID() == bot->GetGUID())
            {
                if (!checkDuration || aura->GetDuration() > 0)
                {
...
bool DeflectSpellTrigger::IsActive()
{
    ...
    SpellSchoolMask deflectSchool = SpellSchoolMask(deflectSpell->GetEffects()[EFFECT_0].MiscValue);
...
// By leewheel 2026-08-07: 添加 spell ID fallback, 防止 locale 匹配失败导致 proc trigger 永远不触发
bool HasAuraTrigger::IsActive()
{
    Unit* target = GetTarget();
    if (botAI->HasAura(getName(), target, false, false, -1, true))
        return true;

    // 名称匹配失败时, 用 spell ID fallback 检查
    std::vector<uint32> fallbackIds = GetFallbackSpellIds();
    for (uint32 spellId : fallbackIds)
    {
...
{
    Unit* target = GetTarget();
    if (botAI->GetAura(getName(), target, false, true, stack))
        return true;

    // 名称匹配失败时, 用 spell ID fallback 检查 (检查 stack 数量)
    std::vector<uint32> fallbackIds = GetFallbackSpellIds();
    for (uint32 spellId : fallbackIds)
    {
        if (target)
        {
            if (Aura* aura = target->GetAura(spellId))
            {
                if (aura->GetStackAmount() >= static_cast<uint32>(stack))
                    return true;
            }
        }
    }
...
{
    //By leewheel 2026-08-07: 开怪瞬间怪物尚未攻击任何人(attacker count=0)，
    //原逻辑直接 return false 导致坦克发呆数秒。此时优先响应队伍的RTI标记目标；
    //无RTI时回退到真人队长的选中目标(玩家直接点怪拉怪也常见)，坦克立即接怪，不等怪物进入攻击列表。
    //By leewheel 2026-08-09: 严重修复——玩家仅"选中"目标(未攻击/未进战斗)时, 坦克绝对不得接怪。
    //必须玩家实际攻击(进战斗且玩家在目标攻击者列表)才允许坦克立即接怪, 否则坦克只响应RTI标记(明确指令)。
    if (!AI_VALUE(uint8, "attacker count"))
    {
        Unit* rtiTarget = AI_VALUE(Unit*, "rti target");
        if (rtiTarget && rtiTarget->IsAlive() &&
            AttackersValue::IsPossibleTarget(rtiTarget, botAI->GetBot()))
            return true;
        ...
        if (Player* master = botAI->GetMaster())
        {
            if (master->IsInCombat() && !master->GetTarget().IsEmpty())
            {
                ...
                if (masterTarget && masterTarget->IsAlive() && masterTarget != bot &&
                    AttackersValue::IsPossibleTarget(masterTarget, botAI->GetBot()))
                {
...
{
    //By leewheel 2026-07-15: 诊断日志 - 确认触发器是否被调用
    static uint32 lastTriggerLogTime = 0;
    bool shouldLog = (getMSTime() - lastTriggerLogTime > 5000);
    if (shouldLog)
    {
        lastTriggerLogTime = getMSTime();
        // TC_LOG_INFO("playerbots", "[TriggerDiag] NotDpsTargetActive::IsActive called for bot={}", 
        //     botAI ? (botAI->GetBot() ? botAI->GetBot()->GetName() : std::string("null")) : std::string("null"));
    }
    //End By leewheel
...
// TC的ObjectGuid不能隐式转换为bool，需要显式检查IsEmpty()
bool NewPlayerNearbyTrigger::IsActive() { return !AI_VALUE(ObjectGuid, "new player nearby").IsEmpty(); }
//End By leewheel
