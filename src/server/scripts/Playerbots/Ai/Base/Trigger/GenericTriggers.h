...
public:
    ...
    // By leewheel 2026-08-07: 子类可重写此方法返回 spell ID 列表, 用于 locale 匹配失败时的 fallback
    virtual std::vector<uint32> GetFallbackSpellIds() const { return {}; }
...
public:
    ...
    bool IsActive() override { return !bot->IsAlive(); }  // TC使用SelfResSpells数组，这里简化为检查死亡状态
};
