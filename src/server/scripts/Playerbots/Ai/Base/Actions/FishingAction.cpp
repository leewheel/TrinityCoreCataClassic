...
float HasFishableWaterOrLand(float x, float y, float z,  Map* map, PhaseShift const& phaseShift, bool checkForLand=false)
{
    if (!map)
        return INVALID_HEIGHT;

    LiquidData liq;
    ZLiquidStatus status = map->GetLiquidStatus(phaseShift, x, y, z+HEIGHT_ABOVE_WATER_TOLERANCE, map_liquidHeaderTypeFlags::AllLiquids, &liq, DEFAULT_COLLISION_HEIGHT);
    float ground = map->GetHeight(phaseShift, x, y, z + HEIGHT_SEARCH_BUFFER, true);
...
WorldPosition FindWaterRadial(Player* bot, float x, float y, float z, Map* map, PhaseShift const& phaseShift, float minDistance, float maxDistance, float increment, bool checkLOS = true, int numDirections = 16)
{
    std::vector<WorldPosition> boundaryPoints;
    ...
    while (dist <= maxDistance)
    {
        for (int i = 0; i < numDirections; ++i)
        {
            ...
            // 修复变量名:函数参数是phaseShift不是phaseMask
            float waterZ = HasFishableWaterOrLand(checkX, checkY, z, map, phaseShift);
...
bool MoveNearWaterAction::isPossible()
{
    ...
    WorldPosition waterAtCurrentPos =
        FindWaterRadial(bot, bot->GetPositionX(), bot->GetPositionY(), bot->GetPositionZ(),         bot->GetMap(), bot->GetPhaseShift(), MIN_DISTANCE_TO_WATER, MAX_DISTANCE_TO_WATER, SEARCH_INCREMENT, true);
...
bool EquipFishingPoleAction::Execute(Event /*event*/)
{
    ...
    WPPCompat::AutoEquipItemSlot(bot->GetSession(), _pole->GetGUID(), EQUIPMENT_SLOT_MAINHAND);
    //End By leewheel
    return true;
}
...
bool EquipFishingPoleAction::isUseful()
{
    ...
    {
        bot->StoreNewItemInBestSlots(FISHING_POLE, 1, ItemContext::NONE);  // 尝试获取鱼竿
        return true;
    }
