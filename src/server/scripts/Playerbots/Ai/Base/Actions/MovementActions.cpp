                            ...
                            bool exact_waypoint, MovementPriority priority, bool lessDelay, bool backwards)
{
    ...
    else
    {
        ...
        {
            //By leewheel 2026-08-02: 性能优化——A*寻路失败时设置移动冷却(100-600ms)，
            //避免MoveTo调用方(移动策略/战斗接近)在不可达目标下每tick重复最多3-4次A*搜索
            AI_VALUE(LastMovement&, "last movement").Set(mapId, x, y, z, bot->GetOrientation(),
                sPlayerbotAIConfig.reactDelay + bot->GetGUID().GetCounter() % 500, priority);
            return false;
        }
...
bool MovementAction::ReachCombatTo(Unit* target, float distance)
{
    ...
    //By leewheel 2026-09-03 修复C4305警告：M_PI/M_PI_2为double字面量，deltaAngle/behind为float算术，用float常量
    ...
    bool behind = fabs(deltaAngle) > float(M_PI_2);
    //End By leewheel
    ...
    if (!(type & typeOk))
    {
        //By leewheel 2026-08-02: 性能优化——寻路失败时设置战斗移动冷却(100-600ms)，
        //避免战斗中bot在不可达目标(悬崖/水对岸)下每tick重复A*搜索风暴
        AI_VALUE(LastMovement&, "last movement").Set(bot->GetMapId(), tx, ty, tz, bot->GetOrientation(),
            sPlayerbotAIConfig.reactDelay + bot->GetGUID().GetCounter() % 500, MovementPriority::MOVEMENT_COMBAT);
        return false;
    }
...
void MovementAction::UpdateMovementState()
{
    ...
    if (!isCurrentlyRestricted && bot->IsAlive())
    {
        ...
        // TC使用Object::GetLiquidStatus()返回ZLiquidStatus，替代AC的GetLiquidData().Status
        const auto liquidState = bot->GetLiquidStatus();
        //End By leewheel
...
bool MovementAction::Follow(Unit* target, float distance, float angle)
{
    ...
    */
                                                    ...
                                                    sPlayerbotAIConfig.sightDistance))
    {
        if (target->GetGUID().IsPlayer())
        {
            ...
            else
            {
                ...
                {
                    //By leewheel 2026-07-11
                    // TC使用GetCurrentMovementGenerator()替代top()，需包含FlightPathMovementGenerator.h
                    FlightPathMovementGenerator* flightGen =
                        static_cast<FlightPathMovementGenerator*>(pTarget->GetMotionMaster()->GetCurrentMovementGenerator());
                    if (flightGen)
                    {
                        std::vector<TaxiPathNodeEntry const*> const& tMap = flightGen->GetPath();
                        if (!tMap.empty())
                        {
                            auto tEnd = tMap.back();
                            if (tEnd)
    ...
    //By leewheel 2026-07-11
    // TC的HandleEmoteCommand参数是Emote枚举类型，不能直接传int 0
    bot->HandleEmoteCommand(Emote(0));
    //End By leewheel
...
bool MovementAction::MoveAway(Unit* target, float distance, bool backwards)
{
    ...
    //By leewheel 2026-09-03 修复C4305警告：M_PI为double字面量，delta为float，用float常量避免double算术截断
    for (float delta = 0; delta <= float(M_PI) / 2; delta += float(M_PI) / 8)
    //End By leewheel
    {
...
void MovementAction::DoMovePoint(Unit* unit, float x, float y, float z, bool generatePath, bool backwards)
{
    ...
    //By leewheel 2026-07-14: 移动诊断日志已清理
    //End By leewheel
    ...
    mm->Clear();
    //By leewheel 2026-07-11
    // TC没有MovePointBackwards，backwards时用MovePoint加finalOrient朝反方向
    // TC的MovePoint签名: MovePoint(id, x, y, z, generatePath, finalOrient, speed, ...)
    if (backwards)
    {
        float orient = unit->GetOrientation() + float(M_PI);
        mm->MovePoint(0, x, y, z, generatePath, orient);
    }
    else
    {
...
{
    //By leewheel 2026-08-04: 非战斗时清除残留引导法术, 否则MoveAway会被channel阻断
...
{
    //By leewheel 2026-08-04: 修复残留channel死循环——非战斗时允许Execute执行CastStop后逃跑
...
bool AvoidAoeAction::AvoidAuraWithDynamicObj()
{
    ...
    name << GetSpellNameBestLocaleWithCache(spellInfo->Id, spellInfo->SpellName);  // << "] (aura)";
    if (FleePosition(dynOwner->GetPosition(), radius))
    {
        if (sPlayerbotAIConfig.tellWhenAvoidAoe && lastTellTimer < time(NULL) - 10)
        {
            ...
            std::ostringstream out;
            //By leewheel 2026-08-01: 玩家可见文本中文化
            out << "我正在躲避 " << name.str() << " (" << spellInfo->Id << ")" << " 半径 " << radius << " - [光环]";
...
bool AvoidAoeAction::AvoidGameObjectWithDamage()
{
    ...
    for (ObjectGuid& guid : traps)
    {
        ...
        // 0 trap with no despawn after cast. 1 trap despawns after cast. 2 bomb casts on spawn.
                //By leewheel 2026-07-10: TC中trap.type改名为trap.charges
        if (goInfo->trap.charges != 0)
        //End By leewheel
            continue;

                //By leewheel 2026-07-10: TC中trap.spellId改名为trap.spell
        ...
        if (!spellInfo || spellInfo->IsPositive())
        {
            continue;
                ...
                //By leewheel 2026-07-10: TC中trap.diameter改名为trap.radius
        float radius = (float)goInfo->trap.radius / 2 + go->GetCombatReach();
        //End By leewheel
        if (!radius || radius > sPlayerbotAIConfig.maxAoeAvoidRadius)
            continue;
        ...
        std::ostringstream name;
        name << GetSpellNameBestLocaleWithCache(spellInfo->Id, spellInfo->SpellName);  // << "] (object)";
        if (FleePosition(go->GetPosition(), radius))
        {
            if (sPlayerbotAIConfig.tellWhenAvoidAoe && lastTellTimer < time(NULL) - 10)
            {
                ...
                std::ostringstream out;
                //By leewheel 2026-08-01: 玩家可见文本中文化
                out << "我正在躲避 " << name.str() << " (" << spellInfo->Id << ")" << " 半径 " << radius
                    << " - [陷阱]";
                bot->Say(out.str(), LANG_UNIVERSAL);
                //End By leewheel
            }
        ...
        for (const Unit::AuraEffectList& list : {aurasPeriodicTriggerSpell, aurasPeriodicTriggerWithValueSpell})
        {
            for (auto i = list.begin(); i != list.end(); ++i)
            {
                ...
                const SpellInfo* triggerSpellInfo =
                    sSpellMgr->GetSpellInfo(spellInfo->GetEffects()[aurEff->GetEffIndex()].TriggerSpell);
                if (!triggerSpellInfo)
                    continue;
                ...
                for (int j = 0; j < MAX_SPELL_EFFECTS; j++)
                {
                    if (triggerSpellInfo->GetEffects()[j].Effect == SPELL_EFFECT_SCHOOL_DAMAGE)
                    {
                        ...
                        if (!radius || radius > sPlayerbotAIConfig.maxAoeAvoidRadius)
                            continue;
                        std::ostringstream name;
                        name << GetSpellNameBestLocaleWithCache(triggerSpellInfo->Id, triggerSpellInfo->SpellName);  //<< "] (unit)";
...
bool CombatFormationMoveAction::isUseful()
{
    ...
    //By leewheel 2026-08-04: 修复残留channel死循环——非战斗时允许Execute执行CastStop后阵型移动
    if (bot->GetCurrentSpell(CURRENT_CHANNELED_SPELL) != nullptr && bot->IsInCombat())
        return false;
    //End By leewheel
...
{
    //By leewheel 2026-08-04: 非战斗时清除残留引导法术, 否则FleePosition会被channel阻断
    if (!bot->IsInCombat() && bot->GetCurrentSpell(CURRENT_CHANNELED_SPELL))
    {
        bot->CastStop();
        botAI->RequestSpellInterrupt();
...
Position CombatFormationMoveAction::AverageGroupPos(float dis, bool ranged, bool self)
{
    ...
    //By leewheel 2026-09-03 修复C4723潜在除0警告：空组(所有成员被过滤后cnt=0)时提前返回，
    //防止下方averageX /= cnt除以0产生未定义结果
    //By leewheel 2026-09-04 修C4723二次确认：上方只拦空组, 但成员可能全部被 IsRanged/距离 过滤,
    //cnt 仍可能为 0——把除法改为 cnt>0 判定, 彻底消除除零路径
    ...
    //By leewheel 2026-09-04: 修 C4723——成员可能全部被过滤, cnt=0 时直接返回原点, 消除除零路径
    if (cnt == 0)
        return Position();
    //End By leewheel
    averageX /= cnt;
...
bool TankFaceAction::Execute(Event /*event*/)
{
    ...
    float tolerable = float(M_PI_2);

    if (fabs(deltaAngle) > tolerable)
        return false;

    float goodAngle1 = Position::NormalizeOrientation(averageAngle + float(M_PI) * 3 / 5);
    float goodAngle2 = Position::NormalizeOrientation(averageAngle - float(M_PI) * 3 / 5);
    //End By leewheel
    ...
    float x, y, z;
    //By leewheel 2026-07-11
    // TC的GetNearPoint签名: GetNearPoint(searcher, x, y, z, distance2d, absAngle) - 6参数
    // AC的7参数版本多了一个dist_to_searcher参数(0.0f)，TC不需要
    //By leewheel 2026-09-03 修复C4189警告并修复功能bug：原注释行尾乱码把GetNearPoint(...goodAngle1)调用吞进注释，
    //导致第一个侧翼坐标从未计算、x/y/z未初始化就被检查(未定义行为)，按AC原版恢复调用
...
bool RearFlankAction::isUseful()
{
    ...
    //By leewheel 2026-09-03 修复C4305警告：M_PI为double字面量，float算术改float常量
    bool inRear = !target->HasInArc((2.f * float(M_PI)) - maxAngle, bot);
    //End By leewheel
...
bool DisperseSetAction::Execute(Event event)
{
    ...
    if (text == "increase")
    {
        ...
        //By leewheel 2026-08-01: 玩家可见文本中文化
        out << "分散距离已增加到 " << std::setprecision(2) << dis;
        botAI->TellMasterNoFacing(out.str());
        //End By leewheel
        return true;
    }
    if (text == "decrease")
    {
        ...
        std::ostringstream out;
        //By leewheel 2026-08-01: 玩家可见文本中文化
        out << "分散距离已减少到 " << std::setprecision(2) << dis;
        botAI->TellMasterNoFacing(out.str());
        //End By leewheel
        return true;
    }
    if (text.starts_with("set"))
    {
        ...
        {
            SET_AI_VALUE(float, "disperse distance", dis);
            out << "分散距离已设置为 " << std::setprecision(2) << dis;
        }
        botAI->TellMasterNoFacing(out.str());
        //End By leewheel
        return true;
    ...
    std::ostringstream out;
    //By leewheel 2026-08-01: 玩家可见文本中文化
    out << "用法: disperse [enable | disable | increase | decrease | set {distance}]";
    float dis = AI_VALUE(float, "disperse distance");
    if (dis > 0.0f)
    {
...
bool SetBehindTargetAction::Execute(Event /*event*/)
{
    ...
    //By leewheel 2026-09-04: 修 C4189——goodAngle1 计算后未引用(goodAngle2 才参与后续逻辑), 暂时归档避免警告
    //如后续绕背逻辑需要双翼角度可再启用
    (void)Position::NormalizeOrientation(target->GetOrientation() + float(M_PI) * 3 / 5);
    float goodAngle2 = Position::NormalizeOrientation(target->GetOrientation() - float(M_PI) * 3 / 5);
    //End By leewheel
