...
//By leewheel 2026-09-06: 移植到TrinityCore-Cata，Cata核心移除BattlegroundEY.h/BattlegroundWS.h
//旗帜光环法术ID(BG_WS_SPELL_*/BG_EY_NETHERSTORM_FLAG_SPELL)在Compat/BattlegroundAVCompat.h中补齐
//End By leewheel
#include "DBCStores.h"
#include "Event.h"
...
MountData CollectMountData(const Player* bot)
{
    ...
    for (auto& entry : bot->GetSpellMap())
    {
        ...
        if (!spellInfo || spellInfo->GetEffects()[0].ApplyAuraName != SPELL_AURA_MOUNTED)
            continue;

        if (entry.second.state == PLAYERSPELL_REMOVED || !entry.second.active || spellInfo->IsPassive())
            continue;

        int32 effect1 = spellInfo->GetEffects()[1].BasePoints;
        int32 effect2 = spellInfo->GetEffects()[2].BasePoints;
        ...
        uint32 index = (spellInfo->GetEffects()[1].ApplyAuraName == SPELL_AURA_MOD_INCREASE_MOUNTED_FLIGHT_SPEED ||
...
bool CheckMountStateAction::isUseful()
{
    ...
    //By leewheel 2026-08-07: 择优移植 brighton-chi/the-lab——selfbot 不自动上马。
    //已手动上马(骑乘中)则放行，允许后续下马逻辑正常执行。
...
bool CheckMountStateAction::TryForms(Player* master, int32 masterMountType, int32 masterSpeed) const
{
        ...
        (masterMountType == 1 && masterSpeed >= 279)))
    {
        ...
        // Compensate speedbuff
        //By leewheel 2026-09-03 修复C4305警告：3.8为double字面量，SetSpeed参数为float，补f后缀
        bot->SetSpeed(MOVE_RUN, 3.8f);
        //End By leewheel
        return true;
    }
...
{
    uint32 botGUID = (uint32)bot->GetGUID().GetCounter();
    ...
    if (!preferredMountTableChecked)
    {
        ...
        if (checkTable && checkTable->Fetch()[0].Get<uint32>() == 1)
        {
            ...
            if (result)
            {
                ...
                TC_LOG_INFO("playerbots", "Preferred mounts initialized | Total records: {}", totalResults);
            }
        }
        else // If the SQL table is missing, log an error and return false
        {
            ...
            // TC_LOG_DEBUG("playerbots", "Preferred mounts SQL table playerbots_preferred_mounts does not exist!");
    ...
    {
        TC_LOG_ERROR("playerbots", "Preferred mount failed: Invalid spell {} | Bot Guid: {}", chosenMountId, botGUID);
        return false;
    }
    ...
    // TC_LOG_DEBUG("playerbots", "Preferred mount failed! | Bot Guid: {}", botGUID);
    return false;
}
...
static bool BotCanUseFlyingMount(Player const* bot)
{
    ...
    //By leewheel 2026-07-10: TC的AreaTableEntry没有IsFlyable()方法，使用AreaTableEntry_IsFlyable兼容函数
    AreaTableEntry const* area = sAreaTableStore.LookupEntry(bot->GetAreaId());
    if (!area || !AreaTableEntry_IsFlyable(area))
        return false;
...
int32 CheckMountStateAction::CalculateMasterMountSpeed(Player* master) const
{
    ...
    if (!noRealMaster && !bot->InBattleground())
    {
        ...
        if (!auraEffects.empty())
        {
            ...
            int32 effect1 = masterSpell->GetEffects()[1].BasePoints;
            int32 effect2 = masterSpell->GetEffects()[2].BasePoints;
...
uint32 CheckMountStateAction::GetMountType(Player* master) const
{
    ...
    {
        SpellInfo const* masterSpell = auraEffects.front()->GetSpellInfo();
        return (masterSpell->GetEffects()[1].ApplyAuraName == SPELL_AURA_MOD_INCREASE_MOUNTED_FLIGHT_SPEED ||
                masterSpell->GetEffects()[2].ApplyAuraName == SPELL_AURA_MOD_INCREASE_MOUNTED_FLIGHT_SPEED) ? 1 : 0;
    }
