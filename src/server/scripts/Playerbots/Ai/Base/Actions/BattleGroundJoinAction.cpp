...
//By leewheel 2026-08-01: 日志清理——注释掉排队状态轮询INFO与DEBUG日志，仅保留错误与关键状态变更
//End By leewheel
...
bool BGJoinAction::Execute(Event /*event*/)
{
    ...
    if (!queueType)  // force join to fill bg
    {
        ...
        BattlegroundQueueTypeId queueTypeId = bgList[urand(0, bgList.size() - 1)];
        BattlegroundTypeId bgTypeId = BattlegroundMgr::BGTemplateId(queueTypeId);
        bool isRated = false;

        BattlegroundTemplate const* bg = sBattlegroundMgr->GetBattlegroundTemplate(bgTypeId);
        if (!bg)
            return false;

        //By leewheel 2026-09-06: 移植到TrinityCore-Cata，BattlemasterListEntry在Cata无MapID字段，
        //战场地图取自BattlegroundTemplate::MapIDs(经Playerbots.h的BattlegroundTemplate_GetMapId兼容)
        uint32 mapId = BattlegroundTemplate_GetMapId(bg);
        //End By leewheel
        PVPDifficultyEntry const* pvpDiff = DB2Manager::GetBattlegroundBracketByLevel(mapId, bot->GetLevel());
        if (!pvpDiff)
            return false;
        ...
        //By leewheel 2026-09-06: 移植到TrinityCore-Cata，"bg type"存战场类型ID(结构化队列ID转回类型ID)
        botAI->GetAiObjectContext()->GetValue<uint32>("bg type")->Set(uint32(BattlegroundMgr::BGTemplateId(queueTypeId)));
        queueType = uint32(BattlegroundMgr::BGTemplateId(queueTypeId));
    }
...
bool BGJoinAction::gatherArenaTeam(ArenaType type)
{
    ...
    {
        TC_LOG_INFO("playerbots", "Team #{} <{}> has not enough members for match", arenateam->GetId(),
    ...
    if (!group->Create(bot))
    {
        TC_LOG_INFO("playerbots", "Team #{} <{}>: Can't create group for arena queue", arenateam->GetId(),
                 arenateam->GetName());
        return false;
    }
    else
    ...
    TC_LOG_INFO("playerbots", "Bot {} <{}>: Leader of <{}>", bot->GetGUID().ToString().c_str(), bot->GetName(),
             arenateam->GetName());
    ...
    for (auto i = begin(members); i != end(members); ++i)
    {
        ...
        TC_LOG_INFO("playerbots", "Bot {} <{}>: Member of <{}>", member->GetGUID().ToString().c_str(),
                 member->GetName().c_str(), arenateam->GetName().c_str());
    }

    if (group && group->GetMembersCount() >= (uint32)arenateam->GetType())
    {
        TC_LOG_INFO("playerbots", "Team #{} <{}> Group is ready for match", arenateam->GetId(),
                 arenateam->GetName().c_str());
        return true;
    }
    else
    {
        TC_LOG_INFO("playerbots", "Team #{} <{}> Group is not ready for match (not enough members)", arenateam->GetId(),
                 arenateam->GetName().c_str());
        group->Disband();
    }
...
bool BGJoinAction::canJoinBg(BattlegroundQueueTypeId queueTypeId, BattlegroundBracketId bracketId)
{
    ...
    //By leewheel 2026-09-06: 移植到TrinityCore-Cata，Cata无BattlemasterEntry->MapID，用BattlegroundTemplate_GetMapId兼容
    uint32 mapId = BattlegroundTemplate_GetMapId(bg);
    PVPDifficultyEntry const* pvpDiff = DB2Manager::GetBattlegroundBracketByLevel(mapId, bot->GetLevel());
    if (!pvpDiff)
        return false;
...
bool BGJoinAction::isUseful()
{
    ...
    //By leewheel 2026-09-01: 缩短机器人上线后不排战场的延迟时间，从120秒改为30秒。
    //原120秒意味着新上线的机器人2分钟内不参与战场填充，严重影响排战场速度。
    //缩短后机器人上线后更快参与战场排队，降低玩家排队等待时间。
    ...
    //WotLK的BATTLEGROUND_QUEUE_AV..MAX_BATTLEGROUND_QUEUE_TYPES枚举在Cata已移除，
    //改为遍历战场管理器的全部战场模板构造队列ID(普通战场队列：非评级、无队伍规模)
    for (auto const& [bgTypeId, bgTemplate] : sBattlegroundMgr->GetBattlegroundTemplates())
    {
        BattlegroundQueueTypeId queueTypeId =
            sBattlegroundMgr->BGQueueTypeId(uint16(bgTypeId), BattlegroundQueueIdType::Battleground, false, 0);
...
bool BGJoinAction::JoinQueue(uint32 type)
{
    ...
    if (!unit && isArena)
    {
        ...
        // TC_LOG_DEBUG("playerbots", "Bot {} could not find Battlemaster to join", bot->GetGUID().ToString().c_str());
        return false;
    }
    ...
    TC_LOG_INFO("playerbots", "Bot {} {}:{} <{}> queued {} {}", bot->GetGUID().ToString().c_str(),
...
bool BGLeaveAction::Execute(Event /*event*/)
{
    ...
    TC_LOG_INFO("playerbots", "Bot {} {}:{} <{}> leaves {} queue", bot->GetGUID().ToString().c_str(),
...
bool BGStatusAction::LeaveBG(PlayerbotAI* botAI)
{
    ...
    TC_LOG_INFO("playerbots", "Bot {} {}:{} <{}> leaves {}", bot->GetGUID().ToString().c_str(),
             bot->GetTeamId() == TEAM_ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName(),
             isArena ? "Arena" : "BG");

    WorldPacket leaveData(CMSG_BATTLEFIELD_LEAVE, 0);
    WorldPackets::Battleground::BattlefieldLeave leavePacket(std::move(leaveData));
    bot->GetSession()->HandleBattlefieldLeaveOpcode(leavePacket);
...
bool BGStatusAction::Execute(Event event)
{
    uint32 QueueSlot;
    uint32 instanceId;
    //By leewheel 2026-09-05: 上游8cc9024b——mapId/statusid 初始化,避免解析包前被未定义值使用
    uint32 mapId = 0;
    uint32 statusid = 0;
    //End By leewheel
    uint32 Time1 = 0;
    ...
    //By leewheel 2026-08-04: 修复——TC的SMSG_BATTLEFIELD_STATUS(=SMSG_BATTLEFIELD_STATUS_QUEUED)格式与AC完全不同
    //AC格式: QueueSlot(4)+arenaByte(8)+minlevel(1)+maxlevel(1)+instanceId(4)+isRated(1)+statusid(4)+[分支字段]
    //TC格式(BattlefieldStatusQueued): Hdr{Ticket{RequesterGuid(8)+Id(4)+Type(4)+Time(8)+bit}
    //  +QueueID.size(4)+RangeMin(1)+RangeMax(1)+TeamSize(1)+InstanceID(4)+QueueID[](8*count)+2bits}
    //  +AverageWaitTime(4)+WaitTime(4)+Unused920(4)+3bits
    //原代码按AC格式读,所有字段全部错位,QueueSlot读到的是Ticket.RequesterGuid的前4字节(垃圾值)
    //修复: 按TC格式解析,statusid恒为STATUS_WAIT_QUEUE(因为只注册了SMSG_BATTLEFIELD_STATUS_QUEUED trigger)
    //  STATUS_WAIT_JOIN/STATUS_IN_PROGRESS有独立opcode,需额外注册trigger才能处理(留待后续)
    //QueueSlot从bot的队列槽位反查: 遍历0..2找非空槽位(PLAYER_MAX_BATTLEGROUND_QUEUES=3)
    //包体最小(无QueueID): 8+4+4+8+1bit + 4+1+1+1+4 + 2bits + 4+4+4 + 3bits ≈ 50字节
    if (p.wpos() < p.rpos() || (p.wpos() - p.rpos()) < 50)
        return false;

    // 解析 RideTicket
    ObjectGuid requesterGuid;
    uint32 ticketId, ticketType;
    int64 ticketTime;
    p >> requesterGuid >> ticketId >> ticketType >> ticketTime;
    p.ReadBit();  // Unknown925
    p.ResetBitPos();

    // 解析 BattlefieldStatusHeader 其余字段
    uint32 queueIdCount;
    p >> queueIdCount;
    //By leewheel 2026-08-04: 限制queueIdCount上界防止伪造大值导致越界读取或OOM
    if (queueIdCount > 10)
        return false;
    // 剩余数据至少需要: QueueID[](8*count) + RangeMin(1) + RangeMax(1) + TeamSize(1) + InstanceID(4) + 2bits(对齐1) + 12 + 3bits(对齐1)
    if ((p.wpos() - p.rpos()) < (uint32)(queueIdCount * 8 + 1 + 1 + 1 + 4 + 1 + 12 + 1))
        return false;

    p >> minlevel >> maxlevel;  // RangeMin, RangeMax (借用原变量名)
    uint8 teamSize;
    p >> teamSize;
    p >> instanceId;

    // 跳过 QueueID 数组
    for (uint32 i = 0; i < queueIdCount; ++i)
    {
        uint64 queueId;
        p >> queueId;
    }

    isRated = p.ReadBit();      // RegisteredMatch (对应AC的isRated语义)
    p.ReadBit();                // TournamentRules, 业务不需要
    p.ResetBitPos();

    // 解析 BattlefieldStatusQueued 尾部字段
    p >> Time1;  // AverageWaitTime
    p >> Time2;  // WaitTime
    int32 unused920;
    p >> unused920;
    p.ReadBit();  // AsGroup
    p.ReadBit();  // EligibleForMatchmaking
    p.ReadBit();  // SuspendedQueue
    p.ResetBitPos();

    statusid = STATUS_WAIT_QUEUE;  // TC只触发此opcode, statusid恒为STATUS_WAIT_QUEUE
    arenaTeam = 0;  // BattlefieldStatusQueued无此字段, STATUS_IN_PROGRESS分支才用
    mapId = 0;      // BattlefieldStatusQueued无此字段, STATUS_WAIT_JOIN/IN_PROGRESS分支才用
    //End By leewheel

    //By leewheel 2026-08-04: QueueSlot从bot队列槽位反查
    //TC的BattlefieldStatusQueued无QueueSlot字段,需遍历bot的队列槽位找非空项
    QueueSlot = 0;
    ...
    switch (statusid)
    {
        case STATUS_WAIT_QUEUE:  // status_in_queue
            // Time1/Time2 已在上方按TC格式读取
            break;
        case STATUS_WAIT_JOIN:  // status_invite
            // TC中此状态由SMSG_BATTLEFIELD_STATUS_NEED_CONFIRMATION触发, 当前trigger不会命中
            mapId = 0;
            Time1 = 0;
            break;
        case STATUS_IN_PROGRESS:  // status_in_progress
            // TC中此状态由SMSG_BATTLEFIELD_STATUS_ACTIVE触发, 当前trigger不会命中
            mapId = 0;
            Time1 = 0;
            Time2 = 0;
            arenaTeam = 0;
            break;
        default:
            TC_LOG_ERROR("playerbots", "Unknown BG status!");
            break;
    }
    ...
    //End By leewheel
    //By leewheel 2026-09-06: 移植到TrinityCore-Cata，Cata无BattlemasterEntry->MapID，用BattlegroundTemplate_GetMapId兼容
    mapId = BattlegroundTemplate_GetMapId(bg);
    PVPDifficultyEntry const* pvpDiff = DB2Manager::GetBattlegroundBracketByLevel(mapId, bot->GetLevel());
    //By leewheel 2026-09-05: 上游8cc9024b——等级无匹配段位时直接退出(原先bracketId默认0会错误加入最老段位队列)
    if (!pvpDiff)
        return false;
    //End By leewheel
    ...
    {
        TC_LOG_INFO("playerbots", "Bot {} <{}> ({} {}): Received BG status TIME_TO_AUTOREMOVE for {} {}",
    ...
    {
        TC_LOG_INFO("playerbots", "Bot {} {}:{} <{}>: Received BG status WAIT_QUEUE (wait time: {}) for {} {}",
        ...
        if (bgQueue.GetPlayerGroupInfoData(bot->GetGUID(), &ginfo))
        {
            if (ginfo.IsInvitedToBGInstanceGUID && !bot->InBattleground())
            {
                ...
                if (bg)
                {
                    ...
                    TC_LOG_INFO("playerbots", "Bot {} {}:{} <{}>: Force join {} {}", bot->GetGUID().ToString().c_str(),
                             bot->GetTeamId() == TEAM_ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName(),
        ...
        BattlegroundTemplate const* bgTemplate = sBattlegroundMgr->GetBattlegroundTemplate(_bgTypeId);
        if (!bgTemplate)
            return false;
        ...
        if (isArena)
            ...
            timer = TIME_TO_AUTOREMOVE + 1000 * (bgTemplate->GetMaxPlayersPerTeam() * 8);
                       ...
                       !(bot->GetGroup() || botAI->GetMaster())))
        {
            ...
            TC_LOG_INFO("playerbots", "Bot {} {}:{} <{}> waited too long and leaves queue ({} {}).",
                     bot->GetGUID().ToString().c_str(), bot->GetTeamId() == TEAM_ALLIANCE ? "A" : "H", bot->GetLevel(),
    ...
    {
        TC_LOG_INFO("playerbots", "Bot {} {}:{} <{}>: Received BG status IN_PROGRESS for {} {}",
                 bot->GetGUID().ToString().c_str(), bot->GetTeamId() == TEAM_ALLIANCE ? "A" : "H", bot->GetLevel(),
                 bot->GetName(), isArena ? "Arena" : "BG", _bgType);
        return false;
    }
    ...
    {
        TC_LOG_INFO("playerbots", "Bot {} {}:{} <{}>: Received BG status WAIT_JOIN for {} {}",
                 bot->GetGUID().ToString().c_str(), bot->GetTeamId() == TEAM_ALLIANCE ? "A" : "H", bot->GetLevel(),
                 bot->GetName(), isArena ? "Arena" : "BG", _bgType);
        ...
        if (isArena)
        {
            ...
            if (!bgQueue.GetPlayerGroupInfoData(bot->GetGUID(), &ginfo))
            {
                TC_LOG_ERROR("playerbots", "Bot {} {}:{} <{}>: Missing QueueInfo for {} {}",
        ...
        TC_LOG_INFO("playerbots", "Bot {} {}:{} <{}> joined {} - {}", bot->GetGUID().ToString().c_str(),
                 bot->GetTeamId() == TEAM_ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName(),
                 isArena ? "Arena" : "BG", _bgType);
...
bool BGStatusCheckAction::Execute(Event /*event*/)
{
    ...
    WorldPacket statusData(CMSG_REQUEST_BATTLEFIELD_STATUS, 0);
    WorldPackets::Battleground::RequestBattlefieldStatus statusPacket(std::move(statusData));
    bot->GetSession()->HandleRequestBattlefieldStatusOpcode(statusPacket);

    // TC_LOG_INFO("playerbots", "Bot {} <{}> ({} {}) : Checking BG invite status", bot->GetGUID().ToString().c_str(),
