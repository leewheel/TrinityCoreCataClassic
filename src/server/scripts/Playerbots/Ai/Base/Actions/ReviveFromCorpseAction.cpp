...
bool ReviveFromCorpseAction::Execute(Event event)
{
    ...
    // TC_LOG_DEBUG("playerbots", "Bot {} {}:{} <{}> revives at body", bot->GetGUID().ToString().c_str(),
    ...
    packet << bot->GetGUID();
    WPPCompat::ReclaimCorpse(bot->GetSession(), packet);
    // End By leewheel
...
bool FindCorpseAction::Execute(Event /*event*/)
{
    ...
    if (!botAI->HasRealPlayerMaster())
    {
        if (dCount >= 5)
        {
            // TC_LOG_INFO("playerbots", "Bot {} {}:{} <{}>: died too many times, was revived and teleported",
...
GraveyardStruct SpiritHealerAction::GetGrave(bool startZone)
{
    //By leewheel 2026-08-15: GameGraveyard改按值返回(修static别名bug)——种族循环内直接按值比较，
    //返回最近墓地副本，调用方持有安全
    GraveyardStruct ClosestGrave = sGraveyard->GetClosestGraveyard(bot, bot->GetTeamId());

    if (!startZone && ClosestGrave.ID)
        return ClosestGrave;
    ...
    else if (startZone && AI_VALUE(uint8, "durability"))
    {
        ...
        if (travelTarget->getPosition())
        {
            ...
            if (travelPos.GetMapId() != uint32(-1))
            {
                uint32 areaId = 0;
                uint32 zoneId = 0;
                bot->GetMap()->GetZoneAndAreaId(bot->GetPhaseShift(), zoneId, areaId, travelPos.GetPositionX(),
    ...
    for (auto race : races)
    {
        for (uint32 cls = 0; cls < MAX_CLASSES; cls++)
        {
            ...
            //By leewheel 2026-07-09: TC的PlayerInfo使用createPosition.Loc而非positionX/Y/Z
            uint32 areaId = 0;
            uint32 zoneId = 0;
            bot->GetMap()->GetZoneAndAreaId(bot->GetPhaseShift(), zoneId, areaId,
                                          info->createPosition.Loc.m_positionX,
                                          info->createPosition.Loc.m_positionY,
                                          info->createPosition.Loc.m_positionZ);

            GraveyardStruct NewGrave = sGraveyard->GetClosestGraveyard(info->createPosition.Loc.GetMapId(),
                                                       info->createPosition.Loc.m_positionX,
                                                       info->createPosition.Loc.m_positionY,
                                                       info->createPosition.Loc.m_positionZ,
...
bool SpiritHealerAction::Execute(Event /*event*/)
{
    ...
    if (bot->GetDistance2d(ClosestGrave.x, ClosestGrave.y) < sPlayerbotAIConfig.sightDistance)
    {
        ...
        for (GuidVector::iterator i = npcs.begin(); i != npcs.end(); i++)
        {
            ...
            {
                // TC_LOG_DEBUG("playerbots", "Bot {} {}:{} <{}> revives at spirit healer", bot->GetGUID().ToString().c_str(),
                //           bot->GetTeamId() == TEAM_ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName());
                PlayerbotChatHandler ch(bot);
                bot->ResurrectPlayer(0.5f);
                bot->SpawnCorpseBones();
                context->GetValue<Unit*>("current target")->Set(nullptr);
                //By leewheel 2026-07-10: TC的SetTarget需要ObjectGuid参数
    ...
    // TC_LOG_INFO("playerbots", "Bot {} {}:{} <{}> can't find a spirit healer", bot->GetGUID().ToString().c_str(),
    //          bot->GetTeamId() == TEAM_ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName().c_str());
