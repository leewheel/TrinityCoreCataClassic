...
void ChooseTravelTargetAction::ReportTravelTarget(TravelTarget* newTarget, TravelTarget* oldTarget)
{
    ...
    else if (destination->getName() == "NullTravelDestination")
    {
        ...
        {
            //By leewheel 2026-08-01: 玩家可见文本中文化
            botAI->TellMaster("没有可去的地方，暂时原地待一会。");
            //End By leewheel
        }
    }
}
                                                ...
                                                std::string const name, std::vector<uint32> items)
{
    ...
    for (auto& d : TravelMgr::instance().getRpgTravelDestinations(bot, true, true))
    {
        ...
        //TC的GetFactionReactionTo第二参数是WorldObject const*，不是FactionTemplateEntry const*
        //传入bot作为target，计算bot对此阵营模板的反应值
...
//By leewheel 20260709: 移除函数结尾多余的;分号，避免IntelliScope解析器混淆作用域
//End By leewheel
...
bool ChooseTravelTargetAction::needForQuest(Unit* target)
{
    ...
    for (auto& quest : questMap)
    {
        ...
        else if (status == QUEST_STATUS_INCOMPLETE)
        {
            ...
            {
                //By leewheel 20260709
                //QuestObjective没有ObjectId成员（正确名称是ObjectID大写D）
                //使用项目中通用的AC兼容数组RequiredNpcOrGo，与GrindTargetValue.cpp等保持一致
                ...
                {
                    //By leewheel 20260709
                    //QuestObjective没有ItemId成员，且被宏#define ItemId GetId()替换会编译失败
                    //使用项目中通用的AC兼容数组RequiredItemId，与DropQuestAction.cpp等保持一致
