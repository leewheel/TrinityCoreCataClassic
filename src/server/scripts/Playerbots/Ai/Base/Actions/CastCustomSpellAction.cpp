...
//By leewheel 2026-07-12: ltrim已在Util/Helpers.h中定义，删除重复的局部定义避免C2556冲突
//End By leewheel
...
bool CastCustomSpellAction::Execute(Event event)
{
    ...
    if (!target)
        if (master && !master->GetTarget().IsEmpty())
...
{
    bool isTradeSkill = spellInfo->GetEffects()[EFFECT_0].Effect == SPELL_EFFECT_CREATE_ITEM &&
...
bool CastRandomSpellAction::Execute(Event event)
{
    ...
    if (!got && !target && !bot->GetTarget().IsEmpty())
    {
...
{
    return spellInfo->GetEffects()[EFFECT_0].Effect == SPELL_EFFECT_CREATE_ITEM && spellInfo->ReagentCount[EFFECT_0] > 0 &&
...
{
    return spellInfo->GetEffects()[EFFECT_0].Effect == SPELL_EFFECT_ENCHANT_ITEM && spellInfo->ReagentCount[EFFECT_0] > 0;
}

uint32 EnchantRandomItemAction::GetSpellPriority(SpellInfo const* spellInfo)
{
    if (spellInfo->GetEffects()[EFFECT_0].Effect == SPELL_EFFECT_ENCHANT_ITEM)
    {
