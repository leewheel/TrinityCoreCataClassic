...
bool BuyAction::Execute(Event event)
{
    ...
    for (GuidVector::iterator i = vendors.begin(); i != vendors.end(); ++i)
    {
        ...
        Creature* pCreature = bot->GetNPCIfCanInteractWith(vendorguid, UNIT_NPC_FLAG_VENDOR, UNIT_NPC_FLAG2_NONE);
        if (!pCreature)
            continue;
        ...
        if (buyUseful)
        {
            ...
            std::vector<VendorItem> m_items_sorted = tItems->m_items;
            ...
            std::sort(m_items_sorted.begin(), m_items_sorted.end(),
                [&calculator](VendorItem const& i, VendorItem const& j)
                {
                    ...
                    {
                        score1 = item1->GetItemLevel();
                        score2 = item2->GetItemLevel();
                    }
                    return score1 > score2; // 降序排序（评分最高的优先）
                });
        ...
        else
        {
            ...
            for (ItemIds::iterator i = itemIds.begin(); i != itemIds.end(); i++)
            {
                ...
                if (!result)
                {
                    ...
                    out << "附近没人出售 " << ChatHelper::FormatItem(proto);
                    //End By leewheel
