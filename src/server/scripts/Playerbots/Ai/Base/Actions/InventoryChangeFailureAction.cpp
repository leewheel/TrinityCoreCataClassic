...
bool InventoryChangeFailureAction::Execute(Event event)
{
    ...
    //By leewheel 2026-08-04: 修复——TC的SMSG_INVENTORY_CHANGE_FAILURE首字段是int32 BagResult,非uint8
    //TC格式: int32 BagResult + Item[2](2*ObjectGuid=16) + uint8 ContainerBSlot + 根据BagResult分支字段
    //AC格式: uint8 err + ... (扁平1字节)
    //原代码按AC读uint8只取1字节,在TC下只读到BagResult最低字节,错误码判断错误
    //修复: 按TC读int32(4字节),长度检查改为>=4
