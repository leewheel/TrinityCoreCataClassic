...
bool DropQuestAction::Execute(Event event)
{
    ...
    for (uint8 slot = 0; slot < MAX_QUEST_LOG_SIZE; ++slot)
    {
        ...
        if (logQuest == entry || link.find(quest->GetLogTitle()) != std::string::npos)
        {
    ...
    if (botAI->HasStrategy("debug quest", BotState::BOT_STATE_NON_COMBAT) || botAI->HasStrategy("debug rpg", BotState::BOT_STATE_COMBAT))
    {
        ...
        TC_LOG_INFO("playerbots", "{} => Quest [ {} ] removed", bot->GetName(), pQuest->GetTitle());
...
{
    //By leewheel 2026-08-01: 移植brighton-chi(5ae21f70)——altbot不清除任务日志，
    //否则刚接的灰色任务会被当过期任务移除
    if (botAI->IsAltBot())
        return false;
    //End By leewheel
    ...
    for (uint8 slot = 0; slot < MAX_QUEST_LOG_SIZE; ++slot)
    {
        ...
        if ((botLevel - questLevel) > trivialLevel)
        {
            ...
            if (botAI->HasStrategy("debug rpg", BotState::BOT_STATE_COMBAT))
            {
                ...
                TC_LOG_INFO("playerbots", "{} => Quest [ {} ] removed", bot->GetName(), quest->GetLogTitle());
                std::string text = PlayerbotTextMgr::instance().GetBotTextOrDefault(
                    "quest_removed_debug",
                    "Quest [%quest] removed",
                    {{"%quest", text_quest}});
