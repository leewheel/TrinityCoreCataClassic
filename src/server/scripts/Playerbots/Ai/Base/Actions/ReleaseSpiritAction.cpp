...
bool ReleaseSpiritAction::Execute(Event event)
{
    ...
    {
        botAI->TellMasterNoFacing(PlayerbotTextMgr::instance().GetBotTextOrDefault(
            "release_spirit_already_spirit", "我已经是灵魂状态了", {}));
        return false;
    }
    ...
    const std::string message = !packet.empty() && packet.GetOpcode() == CMSG_REPOP_REQUEST
        ...
        : PlayerbotTextMgr::instance().GetBotTextOrDefault("release_spirit_meet_graveyard", "在墓园等我吧", {});
        //End By leewheel
    ...
    WPPCompat::RepopRequest(bot->GetSession(), releasePacket);
    // End By leewheel
...
void ReleaseSpiritAction::LogRelease(const std::string& releaseMsg) const
{
    ...
    //By leewheel 2026-09-03 修复C4100警告：按AC原版恢复释放灵魂日志(log中文)
    TC_LOG_DEBUG("playerbots", "Bot {} {}:{} <{}> {}",
...
bool AutoReleaseSpiritAction::Execute(Event /*event*/)
{
    ...
    packet << uint8(0);
    WPPCompat::RepopRequest(bot->GetSession(), packet);
    // End By leewheel
...
bool AutoReleaseSpiritAction::HandleBattlegroundSpiritHealer()
{
    ...
    else if (!botAI->IsRealPlayer())
    {
        m_bgGossipTime = now;
        // By leewheel 2026-07-09: 使用WPPCompat兼容层替代直接WorldPacket调用
        WPPCompat::DoGossipHello(bot->GetSession(), spiritHealer->GetGUID());
        // End By leewheel
    }
...
{
    //By leewheel 2026-08-15: GetGrave改按值返回(继承SpiritHealerAction，随GameGraveyard
    //static别名修复)，这里用值+指针包装保持原语义
    GraveyardStruct graveyardValue = GetGrave(
        AI_VALUE(uint32, "death count") > 10 ||
        CalculateDeadTime() > 30 * MINUTE
    );

    if (!graveyardValue.ID)
        return false;

    PerformGraveyardTeleport(&graveyardValue);
    return true;
    //End By leewheel
}
...
//By leewheel 2026-07-14: 修复空包崩溃 - TC的SelfRes::Read()需要从包中读取int32 SpellID，
// 但原代码创建空包后直接调用WPPCompat::SelfRes导致ByteBuffer::read越界崩溃
bool SelfResurrectAction::Execute(Event /*event*/)
{
    if (!bot->IsAlive() && bot->GetUInt32Value(PLAYER_SELF_RES_SPELL))
    {
        ...
        WorldPacket selfResPacket(CMSG_SELF_RES);
        selfResPacket << uint32(selfResSpell);  // 写入SpellID，供SelfRes::Read()读取
        WPPCompat::SelfRes(bot->GetSession(), selfResPacket);
        return true;
    }
    return false;
}
