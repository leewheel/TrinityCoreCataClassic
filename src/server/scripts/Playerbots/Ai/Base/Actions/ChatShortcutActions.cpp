...
bool FollowChatShortcutAction::Execute(Event /*event*/)
{
    ...
    //By leewheel 2026-08-09: LFG随机本匹配的bot无玩家master, 用队伍leader(玩家队长)代替, 使 /p 集合 能响应
    if (!master && botAI->GetGroupLeader() && botAI->GetGroupLeader()->IsPlayer())
        master = botAI->GetGroupLeader()->ToPlayer();
    // End By leewheel
    if (!master)
        return false;
