...
bool ChangeTalentsAction::Execute(Event event)
{
    ...
    {
        flag->Set(false);
        TC_LOG_INFO("playerbots", "Custom Glyph Flag set to OFF");
    }
    ...
    if (!param.empty())
    {
        ...
        else if (param.find("switch") != std::string::npos)
        {
            ...
            {
                if (bot->GetBonusTalentGroupCount() == 0 && bot->GetLevel() >= sWorld->getIntConfig(CONFIG_MIN_DUALSPEC_LEVEL))
                {
                    bot->CastSpell(bot, 63680, CastSpellExtraArgs(TRIGGERED_FULL_MASK).SetOriginalCaster(bot->GetGUID()));
                    bot->CastSpell(bot, 63624, CastSpellExtraArgs(TRIGGERED_FULL_MASK).SetOriginalCaster(bot->GetGUID()));
                }
...
std::string ChangeTalentsAction::SpecPick(std::string param)
{
    ...
    for (int specNo = 0; specNo < MAX_SPECNO; ++specNo)
    {
        ...
        if (sPlayerbotAIConfig.premadeSpecName[cls][specNo] == param)
        {
            ...
            std::ostringstream out;
            //By leewheel 2026-08-01: 玩家可见文本中文化
            out << "正在选择 " << sPlayerbotAIConfig.premadeSpecName[cls][specNo];
            //End By leewheel
            return out.str();
        }
    }
