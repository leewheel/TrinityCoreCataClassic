...
bool RevealGatheringItemAction::Execute(Event /*event*/)
{
    ...
    Cell::VisitGridObjects(bot, searcher, sPlayerbotAIConfig.reactDistance);
        ...
        if (LockEntry const* lockInfo = sLockStore.LookupEntry(go->GetGOInfo()->GetLockId()))
        {
            for (uint8 i = 0; i < 8; ++i)
            {
                if (lockInfo->Type[i] == LOCK_KEY_SKILL)
                {
                    ...
                    uint32 reqSkillValue = std::max(2u, (uint32)lockInfo->Skill[i]); // By leewheel 2026-07-09
                    //By leewheel 2026-08-14: 揭示采集物也要求已学技能且有工具
