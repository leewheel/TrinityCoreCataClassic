...
bool InviteToGroupAction::Invite(Player* inviter, Player* player)
{
    ...
    //By leewheel 2026-08-15: 修复——原扁平包(名字\0+uint32)喂给TC bit-packed的PartyInviteClient::Read()必然抛
    //ByteBufferException(被Engine吞掉)导致机器人发起的组队邀请永远发不出去(附近邀请/公会邀请/JoinGroup/LFG全失效)。
    //改用WPPCompat::GroupInvite(直接Group::AddInvite, 与TC语义一致, 空安全)
    WPPCompat::GroupInvite(inviter->GetSession(), player->GetName());
    //End By leewheel

    //By leewheel 2026-07-14: 记录邀请已发送日志已清理
    //End By leewheel
...
bool InviteNearbyToGroupAction::Execute(Event /*event*/)
{
    ...
    for (auto& i : nearGuids)
    {
        ...
        //By leewheel 2026-08-15: 修复空指针崩溃——真实玩家(无PlayerbotAI)GET_PLAYERBOT_AI返回nullptr，
        //直接解引用IsRealPlayer()会崩溃(world线程)；判空后仅跳过selfbot(语义对齐the-lab IsSelfBot)
        if (!PlayerbotAIConfig::instance().randomBotInvitePlayer)
        {
...
bool LfgAction::Execute(Event event)
{
    ...
    if (invite)
    {
        ...
        placeholders["%role"] = (role & BOT_ROLE_TANK ? "坦克" : (role & BOT_ROLE_HEALER ? "治疗" : "输出"));
        placeholders["%spotsleft"] = std::to_string(allowedRoles[role] - 1);

        std::ostringstream out;
        if (allowedRoles[role] > 1)
        {
            out << "以" << placeholders["%role"] << "身份加入，剩余" << placeholders["%spotsleft"] << "个"
