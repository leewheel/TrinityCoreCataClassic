...
#include "SpellMgr.h"
#include "Spell.h"
//By leewheel 2026-07-10: 需要包含TemporarySummon.h，SummonCreature返回TempSummon*
...
bool DebugAction::Execute(Event event)
{
    ...
    else if (text.find("quest") != std::string::npos)
    {
        std::ostringstream out;
        //By leewheel 2026-08-01: 玩家可见文本中文化
        out << TravelMgr::instance().quests.size() << " 个任务 ";
    ...
    else if (text.find("rem node") != std::string::npos)
    {
        ...
        {
            //By leewheel 2026-08-01: 玩家可见文本中文化
            botAI->TellMasterNoFacing("该节点无法被移除。");
            //End By leewheel
        }
    ...
    else if (text.find("show node") != std::string::npos)
    {
        ...
        for (auto& node : nodes)
        {
            for (auto& l : *node->getLinks())
            {
                ...
                for (auto p : ppath)
                {
                    Creature* wpCreature = bot->SummonCreature(1, p.GetPositionX(), p.GetPositionY(), p.GetPositionZ(), 0, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, Milliseconds(20000));
    ...
    else if (text.find("dspell ") != std::string::npos)
    {
        ...
        for (float i = 0; i < 60; i++)
        {
            ...
            Creature* wpCreature = bot->SummonCreature(2334, botPos.GetPositionX(), botPos.GetPositionY(), botPos.GetPositionZ(), 0,
                                                       TEMPSUMMON_TIMED_DESPAWN, Milliseconds(10000));
    ...
    else if (text.find("aspell ") != std::string::npos)
    {
        ...
        for (float i = 0; i < 60; i++)
        {
            ...
            Creature* wpCreature = bot->SummonCreature(2334, botPos.GetPositionX(), botPos.GetPositionY(), botPos.GetPositionZ(), 0,
                                                       TEMPSUMMON_TIMED_DESPAWN, Milliseconds(static_cast<int64_t>(5000.0f + i * 100.0f)));
    ...
    else if (text.find("spell ") != std::string::npos)
    {
        ...
        //By leewheel 2026-07-10: TC的SendPlaySpellVisual需要Unit*和更多参数
        master->SendPlaySpellVisual(bot, spellEffect, 0, 0, 0.0f);
        //End By leewheel
        return true;
    }
            ...
            for (int32 dy = 0; dy < 10; dy++)
            {
                ...
                Creature* wpCreature =
                    bot->SummonCreature((dy == 0 && (dx == 0 || dx == 2)) ? 6 : 2, botPos.GetPositionX(), botPos.GetPositionY(),
                                        botPos.GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, Milliseconds(10000));
    ...
    else if (text.find("ispellmap") != std::string::npos)
    {
        ...
        for (int32 dx = 0; dx < 10; dx++)
        {
            for (int32 dy = 0; dy < 10; dy++)
            {
                ...
                if (wpCreature)
                {
                    //By leewheel 2026-07-10: TC不使用SMSG_PLAY_SPELL_IMPACT，用SendPlaySpellVisualKit替代
                    wpCreature->SendPlaySpellVisualKit(effect, 0, 0);
                    //End By leewheel
                }
            }
        }
        ...
        for (int32 dx = 0; dx < 10; dx++)
        {
            for (int32 dy = 0; dy < 10; dy++)
            {
                ...
                wpCreature = bot->SummonCreature((dy == 0 && (dx == 0 || dx == 2)) ? 6 : 2, botPos.GetPositionX(),
                                                 botPos.GetPositionY(), botPos.GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, Milliseconds(10000));
        ...
        for (uint32 i = 0; i < 2; i++)
            for (int32 dx = 0; dx < 10; dx++)
            {
                for (int32 dy = 0; dy < 10; dy++)
                {
                    ...
                    //By leewheel 2026-07-10: TC的SendPlaySpellVisualKit在Unit自身上调用，参数是(id, type, duration)
...
//By leewheel 2026-07-10: 重写FakeSpell使用TC的包类系统替代手动WorldPacket构建
                            ...
                            GuidVector missTargets, WorldPosition source, WorldPosition dest, bool forceDest)
{
    ...
    Unit* sendCaster = caster ? caster : truecaster;
    ...
    // SMSG_SPELL_START
    {
        ...
        castData.CasterGUID = sendCaster->GetGUID();
        castData.CasterUnit = sendCaster->GetGUID();
        castData.SpellID = spellId;
        castData.CastFlags = castFlags;
        castData.CastTime = 1000;
        ...
        {
            castData.AmmoDisplayID = 5996;
            castData.AmmoInventoryType = INVTYPE_AMMO;
        }

        WorldPacket const* data = packet.Write();
        sendCaster->SendMessageToSet(data, true);
    }
    ...
    // SMSG_SPELL_GO
    {
        ...
        if (target)
            castData.HitTargets.push_back(target);

        for (auto otherTarget : otherTargets)
            castData.HitTargets.push_back(otherTarget);

        for (auto missTarget : missTargets)
        {
            castData.MissTargets.push_back(missTarget);
            //By leewheel 2026-07-10: TC的SpellMissStatus只有Reason和ReflectStatus两个字段
            WorldPackets::Spells::SpellMissStatus missStatus;
            missStatus.Reason = SPELL_MISS_RESIST;
            missStatus.ReflectStatus = SPELL_MISS_NONE;
            castData.MissStatus.push_back(missStatus);
            //End By leewheel
        }
