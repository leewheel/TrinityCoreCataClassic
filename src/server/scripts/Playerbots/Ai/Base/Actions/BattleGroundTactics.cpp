...
//By leewheel 2026-09-06: 移植到TrinityCore-Cata，Cata核心移除各战场Zone类头文件
//BG_AV_*常量在Compat/BattlegroundAVCompat.h中补齐，此处移除WotLK的Zone类包含
#include "BattlegroundMgr.h"
//By leewheel 2026-09-06: AV节点状态查询兼容接口(Cata的AV战场脚本注册表)
#include "alterac_valley_state_compat.h"
//By leewheel 2026-09-06: IC节点/城门状态查询兼容接口(Cata的IC战场脚本注册表)
...
//By leewheel 2026-09-03 修复C4305警告：1039.819为double字面量，Position为float，补f后缀
Position const WS_GY_CAMPING_HORDE = {1039.819f, 1388.759f, 340.703f, 0.0f};
//End By leewheel
...
//原AV_CREATURE_*槽位常量(WotLK的BgCreatures数组索引)已废弃：Cata无对应槽位数组，
//队长/首领状态改经AV脚本注册表(alterac_valley_state_compat.h)查询，移动目标用坐标常量
//End By leewheel
...
//By leewheel 2026-09-06: 移植到TrinityCore-Cata，AV战术状态查询辅助
//Cata的AV战场是数据驱动脚本，无BattlegroundAV类可查GetAVNodeInfo/GetMineOwner；
//通过AV脚本注册表(alterac_valley_state_compat.h)查询节点/矿洞状态，
//并通过场景内对象搜索获取节点旗帜与首领(AV对象表索引在Cata无效)
// --- AV节点状态查询(节点ID/状态值与WotLK一致) ---
static bool AVTactics_GetNodeState(Battleground* bg, uint8 node, uint8& state, uint32& owner, bool& tower)
{
    return bg && AVCompatQueryNode(bg->GetInstanceID(), node, state, owner, tower);
}

// --- AV矿洞归属查询(0北矿/1南矿) ---
static bool AVTactics_GetMineOwner(Battleground* bg, uint8 mine, uint32& owner)
{
    return bg && AVCompatQueryMineOwner(bg->GetInstanceID(), mine, owner);
}

// --- AV队长存活查询(联盟队长=巴林达，部落队长=加尔范上尉) ---
// 通过AV脚本注册表查询，脚本内部持有队长GUID，跨图准确
static bool AVTactics_IsCaptainAlive(Battleground* bg, TeamId team)
{
    if (!bg)
        return false;

    auto const& registry = AVCompatInstanceRegistry();
    auto itr = registry.find(bg->GetInstanceID());
    if (itr == registry.end())
        return false;

    return itr->second->IsCaptainAliveCompat(team);
}
//End By leewheel
...
std::string const BGTactics::HandleConsoleCommandPrivate(WorldSession* session, char const* args)
{
        ...
        for (uint32 j = min; j <= max; j++)
        {
            ...
            for (uint32 i = 0; i < path->size(); i++)
            {
                ...
                Creature* wpCreature = player->SummonCreature(15631, waypoint.x, waypoint.y, waypoint.z, 0,
                                                              TEMPSUMMON_TIMED_DESPAWN, Milliseconds(15000));
    ...
    if (!strncmp(cmd, "showcreature=", 13))
    {
        ...
        //By leewheel 2026-09-06: 移植到TrinityCore-Cata，Cata战场无BgCreatures槽位数组，调试命令不可用
        return "showcreature is not supported on Cataclysm (no BgCreatures array)";
        //End By leewheel
    ...
    if (!strncmp(cmd, "showobject=", 11))
    {
        ...
        //By leewheel 2026-09-06: 移植到TrinityCore-Cata，Cata战场无BgObjects槽位数组，调试命令不可用
        return "showobject is not supported on Cataclysm (no BgObjects array)";
        //End By leewheel
...
bool BGTactics::Execute(Event /*event*/)
{
    ...
    BattlegroundTypeId bgType = bg->GetBgTypeID();
    if (bgType == BATTLEGROUND_RB)
        bgType = bot->GetBattleground()->GetTypeID();
...
bool BGTactics::selectObjective(bool reset)
{
    ...
    //By leewheel 2026-09-06: 移植到TrinityCore-Cata，AV战术以坐标为目标(节点旗帜在Cata无对象表索引)
    bool hasPositionObjective = false;
    Optional<uint8> selectedNodeId;
    //End By leewheel
    ...
    switch (bgType)
    {
        ...
        {
            //By leewheel 2026-09-06: 移植到TrinityCore-Cata
            //Cata无BattlegroundAV类：节点/矿洞状态改经AV脚本注册表查询(AVTactics_*辅助)，
            //移动目标改用AVNodeMovementTargets的节点坐标(不再依赖WotLK的对象表索引)
            TeamId team = bot->GetTeamId();
            ...
            // By leewheel 2026-09-06: 节点状态改经AV脚本注册表查询
            for (auto const& [nodeId, _] : defendObjectives)
            {
                uint8 nState = 0; uint32 nOwner = 0; bool nTower = false;
                if (AVTactics_GetNodeState(bg, uint8(nodeId), nState, nOwner, nTower) && nState == AV_COMPAT_POINT_DESTROYED)
                    destroyedNodes++;
            }
            // End By leewheel
            ...
            {
                // By leewheel 2026-09-06: 矿洞归属改经AV脚本注册表查询(0北矿/1南矿)
                uint8 mineType = (team == TEAM_HORDE) ? uint8(AV_SOUTH_MINE) : uint8(AV_NORTH_MINE);
                uint32 mineOwner = 0;
                if (AVTactics_GetMineOwner(bg, mineType, mineOwner) && mineOwner != uint32(team == TEAM_ALLIANCE ? ALLIANCE : HORDE))
                //End By leewheel
                {
                    ...
                    // By leewheel 2026-09-06: Cata无AV矿洞首领槽位索引，直接以矿洞坐标为移动目标
                    hasPositionObjective = true;
                }
            }
            ...
            if (!BgObjective && !hasPositionObjective && hasSnowfallRole)
            {
                // By leewheel 2026-09-06: 雪落墓地归属改经AV脚本注册表查询，以节点坐标为目标
                uint8 snowState = 0; uint32 snowOwner = 0; bool snowTower = false;
                if (AVTactics_GetNodeState(bg, uint8(BG_AV_NODES_SNOWFALL_GRAVE), snowState, snowOwner, snowTower) &&
                    snowOwner == uint32(TEAM_OTHER))
                //End By leewheel
                {
                    Position const& snowPos = AVNodeMovementTargets[uint8(BG_AV_NODES_SNOWFALL_GRAVE)].pos;
                    float rx, ry, rz;
                    bot->GetRandomPoint(snowPos, frand(5.0f, 15.0f), rx, ry, rz);
                    if (Map* map = bot->GetMap())
                    {
                        float groundZ = map->GetHeight(bot->GetPhaseShift(), rx, ry, rz);
            ...
            // --- Captain ---
            if (!BgObjective && !hasPositionObjective && urand(0, 99) < 90)
            {
                // By leewheel 2026-09-06: 队长存活改经AV脚本注册表查询；以敌方队长驻地为移动目标
                if (AVTactics_IsCaptainAlive(bg, team == TEAM_HORDE ? TEAM_ALLIANCE : TEAM_HORDE))
                {
                    Position const& captainPos = (team == TEAM_HORDE) ? AV_BOSS_WAIT_A : AV_BOSS_WAIT_H;
                    pos.Set(captainPos.GetPositionX(), captainPos.GetPositionY(), captainPos.GetPositionZ(), bot->GetMapId());
                    posMap["bg objective"] = pos;
                    hasPositionObjective = true;
                }
                //End By leewheel
            ...
            // --- Defender Logic ---
            if (!BgObjective && !hasPositionObjective && isDefender)
            {
                std::vector<uint8> contestedObjectives;
                std::vector<uint8> availableObjectives;

                // By leewheel 2026-09-06: 节点状态改经AV脚本注册表查询，候选目标以节点ID表示
                for (auto const& [nodeId, goId] : defendObjectives)
                {
                    uint8 nState = 0; uint32 nOwner = 0; bool nTower = false;
                    if (!AVTactics_GetNodeState(bg, uint8(nodeId), nState, nOwner, nTower))
                        continue;

                    if (nState == AV_COMPAT_POINT_DESTROYED)
                        continue;

                    if (nState == AV_COMPAT_POINT_ASSAULTED)
                        contestedObjectives.push_back(uint8(nodeId));
                    else
                        availableObjectives.push_back(uint8(nodeId));
                }
                //End By leewheel

                if (!contestedObjectives.empty())
                    selectedNodeId = contestedObjectives[urand(0, contestedObjectives.size() - 1)];
                else if (!availableObjectives.empty())
                    selectedNodeId = availableObjectives[urand(0, availableObjectives.size() - 1)];
            }

            // --- Enemy Boss ---
            if (!BgObjective && !hasPositionObjective && !selectedNodeId)
            {
                uint32 towersDown = 0;
                // By leewheel 2026-09-06: 节点状态改经AV脚本注册表查询
                ...
                {
                    // By leewheel 2026-09-06: 最终墓地归属改经AV脚本注册表查询
                    uint8 lastGY = uint8((team == TEAM_HORDE) ? BG_AV_NODES_FIRSTAID_STATION : BG_AV_NODES_FROSTWOLF_HUT);
                    uint8 gyState = 0; uint32 gyOwner = 0; bool gyTower = false;
                    bool ownsFinalGY = AVTactics_GetNodeState(bg, lastGY, gyState, gyOwner, gyTower) &&
                                       gyOwner == uint32(team == TEAM_ALLIANCE ? ALLIANCE : HORDE);
                    //End By leewheel

                    if (ownsFinalGY || getPlayersInArea(team, (team == TEAM_HORDE) ? AV_BOSS_WAIT_A : AV_BOSS_WAIT_H, 200.0f, false) >= 20)
                    {
                        // By leewheel 2026-09-06: 以敌方首领驻地为移动目标(Cata无AV生物槽位索引)
                        Position const& bossPos = (team == TEAM_HORDE) ? AV_BOSS_WAIT_A : AV_BOSS_WAIT_H;
                        pos.Set(bossPos.GetPositionX(), bossPos.GetPositionY(), bossPos.GetPositionZ(), bot->GetMapId());
                        posMap["bg objective"] = pos;
                        hasPositionObjective = true;
                    }
                }
            ...
            if (!BgObjective && !hasPositionObjective && !selectedNodeId)
            {
                ...
                for (auto const& [nodeId, goId] : attackObjectives)
                {
                    ...
                    // 已摧毁或已完全占领的节点跳过
                    if (nState == AV_COMPAT_POINT_DESTROYED || nOwner == uint32(team == TEAM_ALLIANCE ? ALLIANCE : HORDE))
                        continue;

                    // 被进攻中(未占领)的节点低概率作为目标
                    if (nState == AV_COMPAT_POINT_ASSAULTED && urand(0, 99) >= 1)
                        continue;

                    candidates.push_back(uint8(nodeId));
                    //End By leewheel
                ...
                if (!candidates.empty())
                    selectedNodeId = candidates[urand(0, candidates.size() - 1)];
                else if (!hasPositionObjective)
                {
            ...
            // By leewheel 2026-09-06: 移植到TrinityCore-Cata
            //对象目标(Unit)直接用其坐标；节点/坐标目标用AVNodeMovementTargets节点坐标，
            //不再依赖Cata已不存在的BG对象表索引匹配
            if (BgObjective || hasPositionObjective || selectedNodeId)
            {
                float rx, ry, rz;
                if (BgObjective)
                {
                    bot->GetRandomPoint(BgObjective->GetPosition(), frand(-2.0f, 2.0f), rx, ry, rz);
                }
                else if (selectedNodeId && AVNodeMovementTargets.count(*selectedNodeId))
                {
                    const AVNodePositionData& data = AVNodeMovementTargets[*selectedNodeId];
                    bot->GetRandomPoint(data.pos, frand(0.0f, data.maxRadius), rx, ry, rz);
                }
                else
                {
            ...
            if (!BgObjective)
            {
                ...
                for (uint8 i = 0; i < 3; ++i)
                {
                    ...
                    for (uint32 nodeId : AB_AttackObjectives)
                    {
                        ...
                        //By leewheel 2026-08-30: 对齐 NPCBots 团队配额——每节点最多 3 人前往, 避免扎堆
                        //  统计该节点附近(30码)本队人数, 达到配额则跳过此节点
                        uint32 nearbyCount = getPlayersInArea(team, go->GetPosition(), 30.0f, false);
                        if (nearbyCount >= 3)
                            continue;
                        //End By leewheel
        ...
        case BATTLEGROUND_EY:
        {
            ...
            if (bot->HasAura(BG_EY_NETHERSTORM_FLAG_SPELL))
            {
                ...
                if (bestNodeId != 0 && EY_NodePositions.contains(bestNodeId))
                {
                    ...
                    if (bestTrigger && bot->IsWithinDist3d(pos.x, pos.y, pos.z, INTERACTION_DISTANCE))
                    {
                        ...
                        WorldPackets::AreaTrigger::AreaTrigger areaTriggerPacket{WorldPacket(CMSG_AREA_TRIGGER, 4 + 1)};
                        // End By leewheel
                        areaTriggerPacket.AreaTriggerID = bestTrigger;
                        areaTriggerPacket.Entered = true;
                        areaTriggerPacket.FromClient = true;
                        bot->GetSession()->HandleAreaTriggerOpcode(areaTriggerPacket);
                        pos.Reset();
                    }
            ...
            if (!foundObjective)
            {
                for (auto const& [nodeId, _, __] : EY_AttackObjectives)
                {
                    ...
                    if (bot->IsWithinDist2d(p.GetPositionX(), p.GetPositionY(), 125.0f))
                    {
                        ...
                        rz = bot->GetMap()->GetHeight(bot->GetPhaseShift(), rx, ry, rz);
                        pos.Set(rx, ry, rz, bot->GetMapId());
                        foundObjective = true;
                    }
                }
            ...
            if (!foundObjective && isDefender && urand(0, 99) <= 80)
            {
                ...
                //By leewheel 2026-08-30: 对齐 NPCBots——EY 守旗点: 旗在基座(未被敌方拿走)时, 防守角色在旗点附近待命防止敌方拿旗
                if (!foundObjective && role == 0)  // 最小的 role 守旗点
                {
        ...
        {
            //By leewheel 2026-09-06: 移植到TrinityCore-Cata
            //Cata无BattlegroundIC类：节点/城门状态经IC脚本注册表(isle_of_conquest_state_compat.h)查询；
            //战场对象/首领改按entry就近查找(FindNearestGameObject/Creature)，替代WotLK的槽位索引
            using namespace IoCCompat;
            auto ICNodeState = [&](uint8 nodeType, uint8& state) -> bool
            {
                return IoCCompatQueryNodeState(bg->GetInstanceID(), nodeType, state);
            };
            auto ICGateDestroyed = [&](uint8 gateId) -> bool
            {
                uint8 state = 0;
                return IoCCompatQueryGateState(bg->GetInstanceID(), gateId, state) && state == IOCGATE_DESTROYED;
            };
            //End By leewheel
            ...
            {
                //By leewheel 2026-09-06: 移植到TrinityCore-Cata，城门摧毁状态改经IC脚本注册表查询(联盟前门)
                bool gateOpen = ICGateDestroyed(IOCGATE_A_FRONT);
                if (gateOpen)
                {
                    ...
                    ICNodeState(NODE_TYPE_GRAVEYARD_A, NODE_STATE_CONTROLLED_H))  // target enemy boss
                {
                    ...
                    //End By leewheel
                    {
                        if (enemyBoss->IsVisible())
                        {
                            ...
                            // TC_LOG_INFO("playerbots", "bot={} attack boss", bot->GetName());
                        }
                    }
                }
                ...
                {
                    //By leewheel 2026-09-06: 移植到TrinityCore-Cata，按entry就近查找大本营横幅
                    if (GameObject* pGO = bot->FindNearestGameObject(GO_ALLIANCE_BANNER, 250.0f))  // capture flag within keep
                    //End By leewheel
                    {
                        BgObjective = pGO;
                        // TC_LOG_INFO("playerbots", "bot={} attack keep", bot->GetName());
                    }
                }
                ...
                if (!BgObjective && !gateOpen && controlsVehicle)  // attack gates
                {
                    ...
                    //By leewheel 2026-09-06: 移植到TrinityCore-Cata，按entry就近查找联盟前门
                    if (GameObject* gate = bot->FindNearestGameObject(GO_ALLIANCE_GATE_3, 250.0f))
                    //End By leewheel
                    {
                        if (vehicleId == NPC_SIEGE_ENGINE_H)  // target gate directly if siege engine
                        {
                            BgObjective = gate;
                            // TC_LOG_INFO("playerbots", "bot={} (in siege-engine) attack gate", bot->GetName());
                        }
                        else  // target gate directly at range if other vehicle
                        {
                            ...
                            siegePos.Set(gate->GetPositionX(), gate->GetPositionY(), gate->GetPositionZ(),
                                         bot->GetMapId());
                            posMap["bg siege"] = siegePos;
                            // TC_LOG_INFO("playerbots", "bot={} (in vehicle={}) attack gate", bot->GetName(), vehicleId);
                            return true;
                        }
                    }
                }
                ...
                {
                    // Capture Refinery
                    //By leewheel 2026-09-06: 移植到TrinityCore-Cata，节点状态改经IC脚本注册表查询
                    uint8 nodeState = NODE_STATE_NEUTRAL;
                    if (ICNodeState(NODE_TYPE_REFINERY, nodeState) &&
                        nodeState != NODE_STATE_CONFLICT_H && nodeState != NODE_STATE_CONTROLLED_H)
                    //End By leewheel
                    {
                        //By leewheel 2026-09-06: 按entry就近查找精炼厂横幅
                        BgObjective = bot->FindNearestGameObject(GO_REFINERY_BANNER, 250.0f);
                        //End By leewheel
                        // TC_LOG_INFO("playerbots", "bot={} attack refinery", bot->GetName());
                    }
                }
                ...
                if (!BgObjective && !controlsVehicle && role < 3)  // Capture Docks
                {
                    //By leewheel 2026-09-06: 移植到TrinityCore-Cata，节点状态改经IC脚本注册表查询
                    uint8 nodeState = NODE_STATE_NEUTRAL;
                    if (ICNodeState(NODE_TYPE_DOCKS, nodeState) &&
                        nodeState != NODE_STATE_CONFLICT_H && nodeState != NODE_STATE_CONTROLLED_H)
                    //End By leewheel
                    {
                        //By leewheel 2026-09-06: 按entry就近查找码头横幅
                        if (GameObject* pGO = bot->FindNearestGameObject(GO_DOCKS_BANNER, 250.0f))
                        //End By leewheel
                        {
                            BgObjective = pGO;
                            // TC_LOG_INFO("playerbots", "bot={} attack docks", bot->GetName());
                        }
                    }
                }
                if (!BgObjective && !controlsVehicle && role < 6)  // Capture Hangar
                {
                    //By leewheel 2026-09-06: 移植到TrinityCore-Cata，节点状态改经IC脚本注册表查询
                    uint8 nodeState = NODE_STATE_NEUTRAL;
                    if (ICNodeState(NODE_TYPE_HANGAR, nodeState) &&
                        nodeState != NODE_STATE_CONFLICT_H && nodeState != NODE_STATE_CONTROLLED_H)
                    //End By leewheel
                    {
                        //By leewheel 2026-09-06: 按entry就近查找机库横幅
                        if (GameObject* pGO = bot->FindNearestGameObject(GO_HANGAR_BANNER, 250.0f))
                        //End By leewheel
                        {
                            BgObjective = pGO;
                            // TC_LOG_INFO("playerbots", "bot={} attack hangar", bot->GetName());
                        }
                    }
                }
                if (!BgObjective && !controlsVehicle)  // Capture Workshop
                {
                    //By leewheel 2026-09-06: 移植到TrinityCore-Cata，节点状态改经IC脚本注册表查询
                    uint8 nodeState = NODE_STATE_NEUTRAL;
                    if (ICNodeState(NODE_TYPE_WORKSHOP, nodeState) &&
                        nodeState != NODE_STATE_CONFLICT_H && nodeState != NODE_STATE_CONTROLLED_H)
                    //End By leewheel
                    {
                        //By leewheel 2026-09-06: 按entry就近查找车间横幅
                        if (GameObject* pGO = bot->FindNearestGameObject(GO_WORKSHOP_BANNER, 250.0f))
                        //End By leewheel
                        {
                            BgObjective = pGO;
                            // TC_LOG_INFO("playerbots", "bot={} attack workshop", bot->GetName());
                        }
                    }
                }
                if (!BgObjective)  // Guard point that's not fully capped (also gets them in place to board vehicle)
                {
                    ...
                    for (uint32 i = 0; i < len; i++)
                    {
                        ...
                        //By leewheel 2026-09-06: 移植到TrinityCore-Cata，节点状态+横幅entry就近查找
                        uint8 nodeState = NODE_STATE_NEUTRAL;
                        if (ICNodeState(uint8(objective.first), nodeState) && nodeState != NODE_STATE_CONTROLLED_H)
                        {
                            if (GameObject* pGO = bot->FindNearestGameObject(objective.second, 250.0f))
                            //End By leewheel
                            {
                                BgObjective = pGO;
                                // TC_LOG_INFO("playerbots", "bot={} guard point while it captures", bot->GetName());
                                break;
                            }
                        }
                    }
                ...
                if (!BgObjective)  // guard vehicles as they seige
                {
                    ...
                    if (bot->GetDistance(IC_GATE_ATTACK_POS_HORDE) < 8.0f)
                        ...
                        pos.Set(IC_GATE_ATTACK_POS_HORDE.GetPositionX() + frand(-5.0f, +5.0f),
                                ...
                                IC_GATE_ATTACK_POS_HORDE.GetPositionZ(), bot->GetMapId());
                    posMap["bg objective"] = pos;
                    // TC_LOG_INFO("playerbots", "bot={} guard vehicles as they attack gate", bot->GetName());
                    return true;
                }
            }
            ...
            {
                //By leewheel 2026-09-06: 移植到TrinityCore-Cata，城门摧毁状态改经IC脚本注册表查询(部落前门)
                bool gateOpen = ICGateDestroyed(IOCGATE_H_FRONT);
                if (gateOpen)
                {
                    PositionInfo siegePos = context->GetValue<PositionMap&>("position")->Get()["bg siege"];
                    siegePos.Reset();
                ...
                {
                    //By leewheel 2026-09-06: 移植到TrinityCore-Cata，按entry就近查找大本营横幅
                    if (GameObject* pGO = bot->FindNearestGameObject(GO_HORDE_BANNER, 250.0f))  // capture flag within keep
                    //End By leewheel
                    {
                        BgObjective = pGO;
                        // TC_LOG_INFO("playerbots", "bot={} attack keep", bot->GetName());
                ...
                {
                    // TODO: check for free vehicles
                    //By leewheel 2026-09-06: 移植到TrinityCore-Cata，按entry就近查找部落前门
                    if (GameObject* gate = bot->FindNearestGameObject(GO_HORDE_GATE_1, 250.0f))
                    //End By leewheel
                    {
                ...
                if (!BgObjective && !controlsVehicle && role == 9)  // Capture Side base
                {
                    ...
                    //By leewheel 2026-09-06: 移植到TrinityCore-Cata，节点状态改经IC脚本注册表查询
                    uint8 nodeState = NODE_STATE_NEUTRAL;
                    if (ICNodeState(NODE_TYPE_QUARRY, nodeState) &&
                        nodeState != NODE_STATE_CONFLICT_A && nodeState != NODE_STATE_CONTROLLED_A)
                    //End By leewheel
                    {
                        //By leewheel 2026-09-06: 按entry就近查找采石场横幅
                        BgObjective = bot->FindNearestGameObject(GO_QUARRY_BANNER, 250.0f);
                        //End By leewheel
                        // TC_LOG_INFO("playerbots", "bot={} attack quarry", bot->GetName());
                    }
                }
                ...
                if (!BgObjective)  // Guard point that's not fully capped (also gets them in place to board vehicle)
                {
                    ...
                    for (uint32 i = 0; i < len; i++)
                    {
                        ...
                        //By leewheel 2026-09-06: 移植到TrinityCore-Cata，节点状态+横幅entry就近查找
                        uint8 nodeState = NODE_STATE_NEUTRAL;
                        if (ICNodeState(uint8(objective.first), nodeState) && nodeState != NODE_STATE_CONTROLLED_A)
                        {
                            if (GameObject* pGO = bot->FindNearestGameObject(objective.second, 250.0f))
                            //End By leewheel
                            {
...
bool BGTactics::selectObjectiveWp(std::vector<BattleBotPath*> const& vPaths)
{
    ...
    for (auto const& path : vPaths)
    {
        ...
        // TC_LOG_INFO("playerbots", "bot={}\t{:6.1f}\t{:4.1f}\t{:4.1f}\t{}", bot->GetName(), pathScore,
    ...
    // TC_LOG_INFO("playerbots", "{} bot={} path={}", (bot->GetTeamId() == TEAM_HORDE ? "HORDE" : "ALLIANCE"),
    ...
    for (ObjectGuid const guid : closeObjects)
    {
        ...
        switch (bgType)
        {
            ...
            case BATTLEGROUND_IC:
            {
                ...
                // By leewheel 2026-07-09: TC需要用局部SpellCastTargets变量传给prepare
            ...
            case BATTLEGROUND_WS:
            {
                if (dist < INTERACTION_DISTANCE)
                {
                    ...
                    if (isWsBaseFlag)
                    {
                        ...
                        {
                            // By leewheel 2026-07-09: 修复最令人头疼的解析
                            WorldPackets::AreaTrigger::AreaTrigger areaTriggerPacketH{WorldPacket(CMSG_AREA_TRIGGER, 4 + 1)};
                            // End By leewheel
                            areaTriggerPacketH.AreaTriggerID = BG_WS_TRIGGER_HORDE_FLAG_SPAWN;
                            areaTriggerPacketH.Entered = true;
                            areaTriggerPacketH.FromClient = true;
                            bot->GetSession()->HandleAreaTriggerOpcode(areaTriggerPacketH);
                        }
                        else
                        {
                            // By leewheel 2026-07-09: 修复最令人头疼的解析
                            WorldPackets::AreaTrigger::AreaTrigger areaTriggerPacketA{WorldPacket(CMSG_AREA_TRIGGER, 4 + 1)};
                            // End By leewheel
                            areaTriggerPacketA.AreaTriggerID = BG_WS_TRIGGER_ALLIANCE_FLAG_SPAWN;
                            areaTriggerPacketA.Entered = true;
                            areaTriggerPacketA.FromClient = true;
                            bot->GetSession()->HandleAreaTriggerOpcode(areaTriggerPacketA);
                        }
                        return true;
                    }
                    ...
                    // 拾取旗帜
                    WorldPackets::GameObject::GameObjUse gameObjUsePacket1{WorldPacket(CMSG_GAME_OBJ_USE, 8)};
                    gameObjUsePacket1.Guid = go->GetGUID();
                    bot->GetSession()->HandleGameObjectUseOpcode(gameObjUsePacket1);
            ...
            {  // Handle Netherstorm flag capture requiring a channel
                if (dist < INTERACTION_DISTANCE)
                {
                    ...
                    // 拾取掉落的旗帜
                    WorldPackets::GameObject::GameObjUse gameObjUsePacket2{WorldPacket(CMSG_GAME_OBJ_USE, 8)};
                    gameObjUsePacket2.Guid = go->GetGUID();
                    bot->GetSession()->HandleGameObjectUseOpcode(gameObjUsePacket2);
...
bool BGTactics::IsLockedInsideKeep()
{
    ...
    if (closestPortal)
    {
        ...
        {
            WorldPackets::GameObject::GameObjUse gameObjUsePacket3{WorldPacket(CMSG_GAME_OBJ_USE, 8)};
            gameObjUsePacket3.Guid = closestPortal->GetGUID();
            bot->GetSession()->HandleGameObjectUseOpcode(gameObjUsePacket3);
            return true;
        }
        else
        {
    ...
    if (target)
    {
        ...
        if (losBlocked)
        {
            ...
            if (path.GetPathType() != PATHFIND_NOPATH)
            {
                ...
                botAI->TellMasterNoFacing("重新走位以摆脱视野限制的目标！");
                //End By leewheel
...
bool ArenaTactics::moveToCenter(Battleground* bg)
{
    ...
    switch (bg->GetBgTypeID())
    {
        case BATTLEGROUND_BE:
            // By leewheel 2026-07-09: TC的WorldSafeLocsEntry需要通过Loc成员访问位置
            if (bg->GetTeamStartPosition(bot->GetBGTeam() == ALLIANCE ? TEAM_ALLIANCE : TEAM_HORDE)->Loc.GetPositionY() < 240)
            // End By leewheel
            {
                if (Preference == 3)
        ...
        case BATTLEGROUND_RL:
            // By leewheel 2026-07-09: TC的WorldSafeLocsEntry需要通过Loc成员访问位置
            if (bg->GetTeamStartPosition(bot->GetBGTeam() == ALLIANCE ? TEAM_ALLIANCE : TEAM_HORDE)->Loc.GetPositionY() < 1600)
            // End By leewheel
            {
                if (Preference == 3)
        ...
        case BATTLEGROUND_NA:
            // By leewheel 2026-07-09: TC的WorldSafeLocsEntry需要通过Loc成员访问位置
            if (bg->GetTeamStartPosition(bot->GetBGTeam() == ALLIANCE ? TEAM_ALLIANCE : TEAM_HORDE)->Loc.GetPositionY() < 2870)
            // End By leewheel
            {
                if (Preference == 3)
...
//By leewheel 2026-08-30: 进战场强制PVP装备检查——老大要求进入战场必须是PVP装备, 解控饰品是必需品。
//  NPCBots 原版仅50%概率要求战场内韧性装(bot_ai.cpp _isItemFitForGeneratedBot), 且无解控饰品专项逻辑;
//  此动作比原版更强: 进战场即检查, 缺解控饰品(42292)则强制从缓存装备到 TRINKET1 槽。
bool BgEnsurePvpGearAction::isUseful()
{
    // 只在战场/竞技场内执行
    ...
    // 人类(人人平等)/亡灵(亡灵意志)种族技能与饰品共享CD, 无需饰品
    if (bot->getRace() == RACE_HUMAN || bot->getRace() == RACE_UNDEAD_PLAYER)
        return false;
    ...
    for (uint8 slot : { EQUIPMENT_SLOT_TRINKET1, EQUIPMENT_SLOT_TRINKET2 })
    {
        ...
        if (!trinket)
            continue;
        //By leewheel 2026-08-30: TC 343 物品法术在 Effects 向量中, Spells[] 为空字段
        for (ItemEffectEntry const* effect : trinket->GetTemplate()->Effects)
...
bool BgEnsurePvpGearAction::Execute(Event /*event*/)
{
    if (!isUseful())
        return false;

    // 从 PVP 解控饰品缓存(已按物等降序)选第一个可装备的
    uint32 const level = bot->GetLevel();
    for (uint32 itemId : PlayerbotFactory::GetCcBreakTrinketCache())
    {
        ItemTemplate const* proto = sObjectMgr->GetItemTemplate(itemId);
        if (!proto)
            continue;
        //By leewheel 2026-09-03 修复C4018警告：GetBaseRequiredLevel返回int32，与uint32比较显式转换
        if (static_cast<uint32>(proto->GetBaseRequiredLevel()) > level)
            continue;
        //End By leewheel
        if (bot->CanUseItem(proto) != EQUIP_ERR_OK)
            continue;

        // 检查两个饰品槽位是否已装同一饰品, 已装则无需处理
        for (uint8 slot : { EQUIPMENT_SLOT_TRINKET1, EQUIPMENT_SLOT_TRINKET2 })
        {
            Item* oldItem = bot->GetItemByPos(INVENTORY_SLOT_BAG_0, slot);
            if (oldItem && oldItem->GetEntry() == itemId)
                return true;
        }

        // 腾出 TRINKET1 槽: 旧饰品移入背包(TC无CMSG_AUTOSTORE_BAG_ITEM, 用WPPCompat)
        if (Item* oldItem = bot->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_TRINKET1))
        {
            //By leewheel 2026-08-30: 复用 WPPCompat 把旧饰品移入背包
            WPPCompat::AutoStoreBagItem(bot->GetSession(), oldItem->GetBagSlot(), oldItem->GetSlot(), NULL_BAG);
            //End By leewheel
            if (bot->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_TRINKET1))
                return false;  // 未能腾出槽位
        }

        //By leewheel 2026-08-30: TC的CanEquipNewItem/EquipNewItem签名——直接装备新饰品
        uint16 dest;
        if (bot->CanEquipNewItem(EQUIPMENT_SLOT_TRINKET1, dest, itemId, true) != EQUIP_ERR_OK)
            continue;
