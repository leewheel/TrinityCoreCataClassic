...
float CastTimeMultiplier::GetValue(Action* action)
{
    ...
    if (/*targetHealth < sPlayerbotAIConfig.criticalHealth && */ dynamic_cast<CastSpellAction*>(action))
    {
        ...
        //By leewheel 2026-07-10: TC中CalcCastTime接受Spell*而非Player*，先取基础值再手动应用玩家施法时间修正
        int32 castTime = spellInfo->CalcCastTime(nullptr);
        bot->ModSpellCastTime(spellInfo, castTime, nullptr);
        //End By leewheel
