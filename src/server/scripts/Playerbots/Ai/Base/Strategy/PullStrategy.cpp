...
//By leewheel 2026-07-10: 添加Engine.h以解决未定义类型Engine的错误
...
PullStrategy* PullStrategy::Get(PlayerbotAI* botAI)
{
    ...
    //By leewheel 2026-07-10: TC中GetStrategy在Engine上而非PlayerbotAI上
    if (Engine* engine = botAI->GetEngine(BOT_STATE_NON_COMBAT))
    {
        if (PullStrategy* strategy = dynamic_cast<PullStrategy*>(engine->GetStrategy("pull")))
        {
    ...
    if (Engine* engine = botAI->GetEngine(BOT_STATE_COMBAT))
        return dynamic_cast<PullStrategy*>(engine->GetStrategy("pull"));
    //End By leewheel
    return nullptr;
}
