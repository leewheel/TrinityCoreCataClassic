...
{
    //By leewheel 2026-08-23: 塞斯元素英文名改 entry(冰霜19204/暗影19206/奥术19205/火焰19203,
    //原英文名查找在本项目中文DB环境不生效, 标记静默失败)
    ...
    Position const& pillarCenter, float botAngle, float distToPillar)
{
    ...
    //By leewheel 2026-08-18: 移植 brighton-chi the-lab 7453e2d8(测试分支警告修复)——用 CastStop 替代 InterruptSpell
    bot->CastStop();
    //End By leewheel
    return MoveTo(
        SETH_MAP_ID, moveX, moveY, bot->GetPositionZ(), false, false,
    ...
    Position const& pillarCenter, float distToPillar)
{
    ...
    //By leewheel 2026-09-03 修复C4305警告：M_PI为double字面量，统一改float常量
    constexpr float angularStep = float(M_PI) / 8.0f;
    constexpr float angularDeadzone = 0.105f;  // ~6°

    float const botAngle = pillarCenter.GetAngle(bot);
    float const targetAngle = pillarCenter.GetAngle(ikiss) + float(M_PI);
