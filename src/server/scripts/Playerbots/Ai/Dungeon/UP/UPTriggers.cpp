    ...
    for (auto i = objects.begin(); i != objects.end(); ++i)
    {
        ...
        if (unit && unit->GetEntry() == NPC_BREATH_TRIGGER)
        {
            ...
            for (Unit::AuraApplicationMap::const_iterator itr = Auras.begin(); itr != Auras.end(); ++itr)
            {
                ...
                //By leewheel 2026-09-03 修复C4389警告：SPELL_FREEZING_CLOUD宏返回uint16枚举引用(模板T推断为uint16)，
                //GetId()返回uint32，比较前显式转换
                if (aura && aura->GetId() == static_cast<uint32>(SPELL_FREEZING_CLOUD))
                //End By leewheel
                {
                    return true;
                }
