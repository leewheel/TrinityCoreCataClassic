...
{
    Unit* boss = AI_VALUE2(Unit*, "find target", "24201");
...
bool IngvarSmashReturnAction::Execute(Event /*event*/)
{
    ...
    float distance = bot->GetExactDist2d(boss->GetPosition());
    //By leewheel 2026-07-10: TC使用GetCombatReach()替代GetMeleeReach()
    return Move(bot->GetAngle(boss), distance + bot->GetCombatReach());
    //End By leewheel
}
