...
#include "ACTriggers.h"
#include "ACActions.h"
//By leewheel 2026-08-24: the-lab 新增标准库头(std::find/distance 需 <algorithm>/<iterator>)
#include <algorithm>
#include <iterator>
#include <vector>
//By leewheel 2026-08-30: 补 using namespace EncounterHelpers——GetStepToPosition 等在该命名空间（上游同）
//End By leewheel
...
bool ShirrakRangedKeepDistanceAction::Execute(Event /*event*/)
{
    ...
    //By leewheel 2026-09-03 修复C4305警告：M_PI为double字面量，constexpr float/float初始化须为float常量
    constexpr float arcSpan = float(M_PI) / 2.0f;
    float arcCenter = float(M_PI);
    //End By leewheel
