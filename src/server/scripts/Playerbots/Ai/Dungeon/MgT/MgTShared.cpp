...
//业务对标: AC azerothcore-wotlk modules/mod-playerbots src/Ai/Dungeon/MgT/MgTShared.cpp
//End By leewheel
...
//By leewheel 2026-09-05: 项目规则——bot策略里boss名一律用entry(塞林·火心24723/凯尔萨斯·逐日者24664)
constexpr char const* SELIN_ENTRY = "24723";
constexpr char const* KAEL_ENTRY  = "24664";
//End By leewheel
                             ...
                             std::vector<Unit*>& out)
{
    ...
    //By leewheel 2026-09-05: TC343的GetCreatureListWithEntryInGrid只接受单个uint32 entry(Object.h:724模板),
    //无AC的多entry重载;按entry逐个收集,语义与上游一致(entries互不相同,不会产生重复)
...
bool StopsACast(uint32 spellId)
{
    ...
    //By leewheel 2026-09-05: AC的SPELL_INTERRUPT_FLAG_INTERRUPT为0x04位;TC343的SpellInterruptFlags是enum class
    //且0x04位命名为Stun,未暴露INTERRUPT名。本项目DB2数据为3.3.5时代语义,按位值0x04直接比对保持上游判定不变
    static constexpr SpellInterruptFlags AC_INTERRUPT_FLAG_INTERRUPT = static_cast<SpellInterruptFlags>(0x04);
    if (info->InterruptFlags.HasFlag(AC_INTERRUPT_FLAG_INTERRUPT) &&
    ...
    {
        if (info->GetEffects()[i].Effect == SPELL_EFFECT_INTERRUPT_CAST)
            return true;

        if (info->GetEffects()[i].Effect != SPELL_EFFECT_APPLY_AURA)
            continue;

        if (info->GetEffects()[i].ApplyAuraName == SPELL_AURA_MOD_SILENCE ||
            info->GetEffects()[i].ApplyAuraName == SPELL_AURA_MOD_STUN)
            return true;
    }
...
{
    return GET_PLAYERBOT_AI(bot)->GetAiObjectContext()->GetValue<Unit*>("find target", SELIN_ENTRY)->Get();
}

Unit* GetKaelthas(Player* bot)
{
    return GET_PLAYERBOT_AI(bot)->GetAiObjectContext()->GetValue<Unit*>("find target", KAEL_ENTRY)->Get();
}
