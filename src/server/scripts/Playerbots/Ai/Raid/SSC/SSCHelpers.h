/* 毒蛇神殿 机器人策略 */
//By leewheel 2026-09-04: 上游d86674ef/c31f2280/ec1d7d1c——全量对齐上游HEAD:
//  命名空间 SerpentShrineCavernHelpers→SscHelpers; enum 改 enum class SscSpells/SscNpcs/SscItems + Id() 取值;
//  位置常量改 inline(移入头文件); 删 NPC_GREYHEART_SPELLBINDER(上游随 LeotherasTheBlindTargetSpellbindersAction 一并删除);
//  新增 Trash 段: 毒水池危害缓存/查询助手(TOXIC_POOL_RADIUS=25 实际危害半径, 检索半径+2y 余量, HAZARD_CACHE_INTERVAL=500ms);
//  Leotheras 四查函返回类型 Unit*→Creature*; FindNearestCreature 调用去第三参(默认true); GetFirstAliveUnitByEntry 已被上游删除
//End By leewheel
