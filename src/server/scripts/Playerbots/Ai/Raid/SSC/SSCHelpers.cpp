...
//  命名空间→SscHelpers; spell/entry/item 全部 Id(enum class) 化; 位置常量上移到头文件(此处删除定义);
//  SHIELD_GENERATOR_DB_GUIDS 定义上移到头文件; 新增 Trash 段毒水池缓存查询函数;
//  传递者遴选逻辑重构(统一空指针/死亡/去重continue + 老四号传递者改 IsAssistHealOfIndex 0——上游修正);
//  FindNearestCreature 去第三参 true(默认); GetLeotheras* 返回 Creature*
//End By leewheel
