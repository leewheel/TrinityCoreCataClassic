...
bool UnderbogColossusEscapeToxicPoolAction::Execute(Event /*event*/)
{
    ...
    if (radius <= 0.0f && sInfo)
    {
        for (int e = 0; e < MAX_SPELL_EFFECTS; ++e)
        {
            ...
            auto const& eff = sInfo->GetEffects()[e];
...
//By leewheel 2026-09-04: 上游——GetFirstAliveUnitByEntry 已删, 改 FindNearestCreature(20y)
...
//By leewheel 2026-09-04: 上游——GetFirstAliveUnitByEntry 已删, 改 FindNearestCreature(75y)
...
bool TheLurkerBelowRunAroundBehindBossAction::Execute(Event /*event*/)
{
    ...
    constexpr float safeArc = float(M_PI) / 2.0f; //By leewheel 2026-09-03 修复C4305警告

    if (!PlayerbotAI::IsMainTank(bot) &&
        std::fabs(Position::NormalizeOrientation(relativeAngle - float(M_PI))) > safeArc / 2.0f) //By leewheel 2026-09-03 修复C4305警告
    {
        float tangentAngle = botAngle + (relativeAngle > float(M_PI) ? -0.1f : 0.1f); //By leewheel 2026-09-03 修复C4305警告
    ...
    {
        float behindAngle = lurker->GetOrientation() + float(M_PI) + frand(-0.5f, 0.5f) * safeArc; //By leewheel 2026-09-03 修复C4305警告
...
bool TheLurkerBelowSpreadRangedInArcAction::Execute(Event /*event*/)
{
    ...
    if (it == lurkerRangedPositions.end())
    {
        ...
        constexpr float arcSpan = 2.0f * float(M_PI) / 3.0f; //By leewheel 2026-09-03 修复C4305警告
...
bool TheLurkerBelowTanksPickUpAddsAction::Execute(Event /*event*/)
{
    ...
    for (auto guid : attackers)
    {
        ...
        //By leewheel 2026-09-04: 上游——entry 引用 Id 化(合并为单条件)
...
//By leewheel 2026-09-04: 上游——删除 LeotherasTheBlindTargetSpellbindersAction(标记术 Binder 逻辑随 GetFirstAliveUnitByEntry 一并移除)
...
bool LeotherasTheBlindDemonFormTankAttackBossAction::Execute(Event /*event*/)
{
    ...
    //By leewheel 2026-09-04: 上游——返回类型 Unit*→Creature*
...
{
    //By leewheel 2026-09-04: 上游——返回类型 Unit*→Creature*; spell 引用 Id 化
...
bool FathomLordKarathressMisdirectBossesToTanksAction::Execute(Event /*event*/)
{
    ...
    if (hunterIndex == 0)
    {
        ...
        //By leewheel 2026-08-23: the-lab 更新为单参数 GetGroupAssistTank API
...
bool LadyVashjPhase1SpreadRangedInArcAction::Execute(Event /*event*/)
{
    ...
    constexpr float arcCenter = float(M_PI) / 2.0f; // North //By leewheel 2026-09-03 修复C4305警告
    constexpr float arcSpan = float(M_PI); // 180° //By leewheel 2026-09-03 修复C4305警告
    constexpr float arcStart = arcCenter - arcSpan / 2.0f;
...
bool LadyVashjAssignPhase2AndPhase3DpsPriorityAction::Execute(Event /*event*/)
{
    ...
    if (bot->GetPositionZ() - platformZ > 2.0f)
    {
        ...
        //By leewheel 2026-08-18: 移植 brighton-chi the-lab 61e3c186(污浊核心拾取者目标/战斗修复)——同地图瞬移不触发读条,保持战斗状态
...
bool LadyVashjLootTaintedCoreAction::Execute(Event /*event*/)
{
    ...
    *packet << elemental->GetGUID();  // Loot object GUID
    //By leewheel 2026-08-15: LootListID是1-based(服务端req.LootListID-1取槽)，传0会下溢255拾取失败，
    //污染之核固定落在槽0→LootListID=1
    *packet << uint8(coreIndex + 1);  // LootListID(1-based)
    //End By leewheel
...
bool LadyVashjPassTheTaintedCoreAction::Execute(Event /*event*/)
{
    ...
    //By leewheel 2026-09-04: 上游——spell/item 引用 Id 化
    if (!bot->HasAura(Id(SscSpells::SPELL_FEAR_WARD)))
        bot->AddAura(Id(SscSpells::SPELL_FEAR_WARD), bot);
...
bool LadyVashjPassTheTaintedCoreAction::UseCoreOnNearestGenerator(const uint32 instanceId)
{
    ...
    //By leewheel 2026-09-04: 上游——const 位置规范化 GeneratorInfo const*
    ...
    //By leewheel 2026-08-30: 修复——TC 343 物品法术在 Effects 中, Spells[] 恒为空, 旧代码读不到虚空核心技能
    for (ItemEffectEntry const* effect : core->GetTemplate()->Effects)
    {
    ...
    //By leewheel 2026-08-14: TC的HandleUseItemOpcode需要WorldPackets::Spells::UseItem而非WorldPacket
    ...
    useItem.Cast.Target.Unit = generator->GetGUID();
...
//By leewheel 2026-09-04: 上游——签名 const 位置规范化
