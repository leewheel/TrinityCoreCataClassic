...
bool RazuviousUseObedienceCrystalAction::Execute(Event /*event*/)
{
    ...
    if (Unit* charm = bot->GetCharm())
    {
        ...
        //By leewheel 2026-07-27: TC使用GetCurrentMovementGeneratorType替代GetMotionSlotType
        if (charm->GetMotionMaster()->GetCurrentMovementGeneratorType(MOTION_SLOT_ACTIVE) == MAX_MOTION_TYPE)
        //End By leewheel
        {
    ...
    else
    {
        ...
        if (diff == RAID_DIFFICULTY_10MAN_NORMAL)
        {
            ...
            for (auto i = npcs.begin(); i != npcs.end(); i++)
            {
                ...
                Creature* creature = bot->GetNPCIfCanInteractWith(*i, UNIT_NPC_FLAG_SPELLCLICK, UNIT_NPC_FLAG_2_NONE); // By leewheel 2026-07-08
                if (!creature)
                    continue;
