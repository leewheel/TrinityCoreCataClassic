...
static constexpr float RAZORGORE_CONE_ARC = float(M_PI); //By leewheel 2026-09-03 修复C4305警告：M_PI为double字面量
static constexpr float RAZORGORE_BACK_FUZZ_ARC = float(M_PI_4); //By leewheel 2026-09-03 修复C4305警告
...
bool BwlVaelastraszMoveAwayAction::MoveAlongFleeDirection(const Unit* boss, float fleeX, float fleeY)
{
    ...
    for (bool requireLos : {true, false})
    {
        ...
        static constexpr float angleOffsets[] = {0.0f, float(M_PI_4), -float(M_PI_4), float(M_PI_2), -float(M_PI_2)}; //By leewheel 2026-09-03 修复C4305警告
