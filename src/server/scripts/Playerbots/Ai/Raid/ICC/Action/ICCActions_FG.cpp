/*
 * 鍐板啝鍫″瀿 - 鑵愰潰锛團estergut锛夋満鍣ㄤ汉鍔ㄤ綔瀹炵幇
 * 澶勭悊鑳冭儉/鎭惰嚟姘斾綋鍙犲眰瑙勯伩銆佸潶鍏嬭疆鎹㈢瓑鏈哄埗
 */
...
bool IccFestergutSporeAction::Execute(Event /*event*/)
{
    ...
    // In position 鈥?let combat rotation run (multiplier blocks movement so bot stays put)
    return false;
}
                                                          ...
                                                          Position const& spreadRangedPos)
{
    ...
    // Goo overlap override 鈥?checked before hasSpore so non-spored bots also redirect.
