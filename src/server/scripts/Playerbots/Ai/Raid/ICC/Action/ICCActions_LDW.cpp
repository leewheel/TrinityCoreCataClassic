/*
 * 鍐板啝鍫″瀿 - 浜¤濂冲＋锛圠ady Deathwhisper锛夋満鍣ㄤ汉鍔ㄤ綔瀹炵幇
 * 澶勭悊鏆楀奖鐏惧巹璧颁綅銆佽繙绋嬪垎鏁ｃ€佽繎鎴樼偣鍚嶇瓑鏈哄埗
 */
...
{
    Unit* boss = AI_VALUE2(Unit*, "find target", "38490");
    if (!boss)
        return false;
        ...
        if (isInfected)
        {
            ...
            if (targetBot)
            {
                ...
                if (bot->GetExactDist2d(targetBot) > 1.0f)
                {
                    ...
                    float angleOffset = float(guidLow % 628) / 100.0f;  // Random angle between 0 and 2蟺
