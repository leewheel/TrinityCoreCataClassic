...
#include "SpellPackets.h" //By leewheel 2026-08-14: TargetLocation闇€瑕?#include "Playerbots.h"
#include "RtiValue.h"
#include "Vehicle.h"
...
bool IccGunshipEnterCannonAction::EnterVehicle(Unit* vehicleBase, bool moveIfFar)
{
    ...
    //By leewheel 2026-08-14: TC鐨凥andleCancelMountAuraOpcode闇€瑕乄orldPackets::Spells::CancelMountAura
    WorldPacket cancelMountAuraPacket(CMSG_CANCEL_MOUNT_AURA);
    WorldPackets::Spells::CancelMountAura cancelMountAura(std::move(cancelMountAuraPacket));
    bot->GetSession()->HandleCancelMountAuraOpcode(cancelMountAura);
    //End By leewheel
...
bool IccGunshipRocketJumpAction::Execute(Event /*event*/)
{
    ...
    if (botAI->IsAssistTank(bot))
    {
        ...
        // No adds 鈥?return to hold position if drifted
    ...
    // Otherwise return to friendly ship 鈥?captain one-shots if tanked too long.
    if (isMainTank)
    {
...
bool IccGunshipRocketJumpAction::ExitCannonIfSeated()
{
    ...
    //By leewheel 2026-08-14: TC鐨凥andleRequestVehicleExit闇€瑕乄orldPackets::Vehicle::RequestVehicleExit
    WorldPacket requestVehicleExitPacket(CMSG_REQUEST_VEHICLE_EXIT);
    WorldPackets::Vehicle::RequestVehicleExit requestVehicleExit(std::move(requestVehicleExitPacket));
    bot->GetSession()->HandleRequestVehicleExit(requestVehicleExit);
    //End By leewheel
    return true;
}
...
bool IccGunshipRocketJumpAction::UseRocketPack(Position const& destination, bool walkIfOutOfRange)
{
    ...
    //By leewheel 2026-09-03 修复C4189警告：castCount/glyphIndex为AC手工构造包时代遗留死变量，
    ...
    //By leewheel 2026-08-14: TC鐨凥andleUseItemOpcode闇€瑕乄orldPackets::Spells::UseItem
    ...
    useItem.Slot = slot;
    useItem.CastItem = itemGuid;
    useItem.Cast.SpellID = spellId;
    useItem.Cast.SendCastFlags = castFlags;
    useItem.Cast.Target.Flags = TARGET_FLAG_DEST_LOCATION;
    useItem.Cast.Target.DstLocation = WorldPackets::Spells::TargetLocation{ ObjectGuid::Empty, TaggedPosition<Position::XYZ>(Position(destination.GetPositionX(), destination.GetPositionY(), destination.GetPositionZ())) };
...
bool IccGunshipRocketPackSetupAction::AcquireRocketPack()
{
    ...
    //By leewheel 2026-08-14: TC鐨凥andleGossipHelloOpcode闇€瑕乄orldPackets::NPC::Hello
    WorldPacket gossipHelloPacket(CMSG_GOSSIP_HELLO);
    WorldPackets::NPC::Hello gossipHello(std::move(gossipHelloPacket));
    gossipHello.Unit = zafod->GetGUID();
    bot->GetSession()->HandleGossipHelloOpcode(gossipHello);
    //End By leewheel

    //By leewheel 2026-08-14: TC鐨凥andleGossipSelectOptionOpcode闇€瑕乄orldPackets::NPC::GossipSelectOption
    WorldPacket gossipSelectPacket(CMSG_GOSSIP_SELECT_OPTION);
    WorldPackets::NPC::GossipSelectOption gossipSelect(std::move(gossipSelectPacket));
    gossipSelect.GossipUnit = zafod->GetGUID();
    gossipSelect.GossipID = ZAFOD_GOSSIP_MENU_ID;
    gossipSelect.GossipOptionID = ZAFOD_GOSSIP_OPTION;
