/*
 * 鍐板啝鍫″瀿 - 鐑傞潰锛圧otface锛夋満鍣ㄤ汉鍔ㄤ綔瀹炵幇
 * 澶勭悊灏忚蒋娉ュ悎骞躲€佸ぇ杞偿杈愬皠鍙犲眰銆佸潶鍏嬭疆鎹㈢瓑鏈哄埗
 */
...
bool IccRotfaceTankPositionAction::PositionMainTankAndMelee(Unit* boss, Unit*)
{
    ...
    if (botAI->IsMainTank(bot) && boss && boss->GetVictim() == bot)
    {
        ...
        if (!bossInPosition)
        {
            // Step 2y in the direction from boss toward center 鈥?tank leads, boss follows
...
bool IccRotfaceGroupPositionAction::HandleOozeTargeting()
{
    ...
    // Find assist tank currently kiting a Big Ooze 鈥?preferred merge point
