...
#include "SpellPackets.h" //By leewheel 2026-08-14: TargetLocation需要
...
bool IccSindragosaFrostBeaconAction::TryDropTombFlares(Unit const* boss)
{
    ...
    if (bot->GetItemCount(itemId, false) == 0)
        bot->StoreNewItemInBestSlots(itemId, 10, ItemContext::NONE);
    ...
    //By leewheel 2026-09-03 修复C4189警告：castCount为AC手工构造包时代遗留死变量，TC经UseItem结构体传递无对应字段，删除
    //By leewheel 2026-08-30: 修复——TC 343 物品法术在 Effects 中, Spells[] 恒为空, 旧代码读不到奥术火炮信号弹技能
    uint32 spellId = 0;
    for (ItemEffectEntry const* effect : flare->GetTemplate()->Effects)
    {
    ...
    //By leewheel 2026-09-03 修复C4189警告：glyphIndex为AC手工构造包时代遗留死变量，删除
    uint8 const castFlags = 0;
    //End By leewheel

    //By leewheel 2026-08-14: TC的HandleUseItemOpcode需要WorldPackets::Spells::UseItem
    ...
    useItem.Cast.Target.DstLocation = WorldPackets::Spells::TargetLocation{ ObjectGuid::Empty, TaggedPosition<Position::XYZ>(Position(chosenPos->GetPositionX(), chosenPos->GetPositionY(), chosenPos->GetPositionZ())) };
    ...
    bool myZoneAllProtected = false;
    {
        ...
        if (myZoneAllProtected)
        {
            for (Creature* pet : pets)
            {
                ...
                {
                    //By leewheel 2026-08-14: TC无CharmInfo::SetPlayerReactState, 上面pet->SetReactState已覆盖
                    pet->GetMotionMaster()->MoveFollow(bot, PET_FOLLOW_DIST,
                                                       Creature_GetFollowAngle(pet)); //By leewheel 2026-08-14: TC无Creature::GetFollowAngle，用全局helper
