/*
 * 鍐板啝鍫″瀿 - 鏅磾濉炲痉鏁欐巿锛圥rofessor Putricide锛夋満鍣ㄤ汉鍔ㄤ綔瀹炵幇
 * 澶勭悊鍙樺紓鐤剧梾/绐掓伅姣掓皵鍙犲眰瑙ｆ硶銆佽蒋娉ョ摱涓㈡幏绛夋満鍒? */
...
bool IccPutricideGrowingOozePuddleAction::Execute(Event /*event*/)
{
    ...
    if (botAI->IsMainTank(bot))
    {
        ...
        if (boss && boss->IsAlive() && boss->GetVictim() == bot && !boss->HealthBelowPct(35))
        {
            ...
            {
                // Centroid of near puddles 鈫?seed direction is boss-from-centroid.
...
Position IccPutricideGrowingOozePuddleAction::CalculateSafeMovePosition(Unit* closestPuddle)
{
    ...
    for (int i = 0; i < numAnglesToTest; ++i)
    {
        ...
        if (insidePuddle)
        {
            ...
            // bot-relative radial. i=0 鈫?straight outward from puddle center.
...
bool IccPutricideVolatileOozeAction::Execute(Event /*event*/)
{
    ...
    if (botAI->IsMelee(bot) && !botAI->IsMainTank(bot))
    {
        ...
        // the ooze directly 鈥?the stack target will end up here anyway.
    ...
    if (botAI->IsRanged(bot) || botAI->IsHeal(bot))
    {
        ...
        // whether any other group member is already stacking near it 鈥?if
...
bool IccPutricideGasCloudAction::HandleGaseousBloatMovement(Unit* gasCloud)
{
    ...
    if (blockedAtStart >= 3)
    {
        ...
        for (int i = 0; i < 32; ++i)
        {
            ...
            // Count long-range openness (25f rays) 鈥?reject positions near walls.
            int farFreeDirs = 0;
            for (int k = 0; k < 8; ++k)
            {
        ...
        // angle with the most local openness regardless of cloud direction 鈥?        // even briefly stepping toward the cloud is preferable to standing
    ...
    // Normal search: scan 32 angles 脳 distance increments, score each valid position.
...
bool IccPutricideAvoidMalleableGooAction::Execute(Event /*event*/)
{
    ...
    if (!boss->HealthBelowPct(35))
    {
        ...
        if (botInDanger)
        {
            ...
            // Sample ring: multiple radii 脳 multiple angles. Prefer minimum
...
bool IccPutricideAbominationAction::TryRegurgitate(Unit* abo, Unit* target)
{
    ...
    //By leewheel 2026-07-27: TC的Spell::prepare参数为const SpellCastTargets&，直接传targets对象名即可
    //By leewheel 2026-09-03 修复C4189警告并修复功能bug：原注释行尾乱码把spell->prepare(targets)调用吞进注释，
    //导致变形瘟疫兽的呕吐法术从未释放且Spell对象泄漏，按AC原版恢复调用
...
bool IccPutricideAbominationAction::TryEatOoze(Unit* abo, Unit* puddle)
{
    ...
    // In range of puddle 鈥?hold position until it dies, even on cooldown
