...
bool IccBpcEmpoweredVortexAction::MaintainRangedSpacing()
{
    ...
    // Find safe angle 鈥?try assigned slot first, then nudge away from vortices
...
bool IccBpcEmpoweredVortexAction::HandleEmpoweredVortexSpread()
{
    ...
    // If locked in position, don't move 鈥?allow combat/healing
    ...
    // Close enough to slot 鈥?lock position
...
bool IccBpcBallOfFlameAction::Execute(Event /*event*/)
{
    ...
    // Hunters excluded 鈥?they can DPS from range without needing to soak
