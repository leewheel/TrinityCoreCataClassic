...
bool IccBqlGroupPositionAction::HandleGroupPosition(Unit* boss, Aura* frenzyAura, Aura* shadowAura)
{
    ...
    if (bossAirborne)
    {
        // Bloodbolt Whirl is heavy raid-wide damage 鈥?pop personal defensive CD to survive
        ...
        for (auto const& guid : members)
        {
            ...
            if (!sPlayerbotsMgr.GetPlayerbotAI(player))
                continue;
            // Frenzied biters roam freely to reach their bite target 鈥?exclude from slot pool
        ...
        static AirSlot const allSlots[] = {
            // Inner ring r=10f, 8 slots at 45掳 鈥?chord 7.65f
            ...
            // Ring 2 r=18f, 15 slots at 24掳 鈥?chord 7.5f
            ...
            // Ring 3 r=26f, 20 slots at 18掳 鈥?chord 8.1f
            ...
            // Outer ring r=34f, 30 slots at 12掳 鈥?chord 7.1f
        ...
        // Pick a new slot 鈥?hunters start from outer ring going in, others from inner going out
        if (myAssignedSlot < 0)
        {
        ...
        if (myAssignedSlot < 0)
        {
            ...
            // No safe slot available 鈥?if standing in shadow, flee away from nearest shadow
            if (botInShadow)
            {
        ...
        else
        {
                ...
                bot->GetExactDist2d(candidateX, candidateY) > moveGate)
            {
                MoveTo(bot->GetMapId(), candidateX, candidateY, candidateZ, false, false, false, true,
                       prio, true, false);
                // Still moving to slot 鈥?block combat so we actually get there
                return true;
            }
        }
        // At slot (or fleeing shadow) 鈥?let combat rotation attack/heal
        return false;
    }
    ...
    // Ground phase ranged positioning 鈥?persistent slot assignment, only affected bots reassign
    if (isRanged && !isAirPhase)
    {
        ...
        // Collect every OTHER bot's remembered slot as "reserved" 鈥?each bot owns its own
        ...
        // Step 2: need a new slot 鈥?pick first safe slot not reserved by others
        if (myAssignedSlot < 0)
        {
        ...
        if (myAssignedSlot < 0)
        {
            // No safe unreserved slot 鈥?fall back to melee. Forget my slot so others can use it.
        ...
        {
            // No safe slot 鈥?flee to lowest-GUID melee bot until a safe slot frees up
...
bool IccBqlVampiricBiteAction::MoveTowardsTarget(Player* target)
{
    ...
    // Don't gate movement on LOS 鈥?if blocked, take a step toward target so LOS
