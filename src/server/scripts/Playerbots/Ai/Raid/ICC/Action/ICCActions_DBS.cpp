/*
 * 鍐板啝鍫″瀿 - 浜¤鑰呭コ澹紙Deathbringer Saurfang锛夋満鍣ㄤ汉鍔ㄤ綔瀹炵幇
 * 澶勭悊鍧﹀厠浣嶇疆銆佸壇鍧﹀厠鎻存姢銆佽鍏戒紭鍏堢骇绛夋満鍒? */
...
bool IccDbsTankPositionAction::PositionInRangedFormation()
{
    ...
    if (myAssignedSlot < 0)
    {
        ...
        for (GroupReference* itr = group->GetFirstMember(); itr != nullptr; itr = itr->next())
        {
            ...
            //By leewheel 2026-07-27: TC鐨凣etRawValue()鏃犲弬杩斿洖std::vector<uint8>锛岄渶浼犵储寮?鍙杣int64
            uint64 g = member->GetGUID().GetRawValue(0);
            //End By leewheel
