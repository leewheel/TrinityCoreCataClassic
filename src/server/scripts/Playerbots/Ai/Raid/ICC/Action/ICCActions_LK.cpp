...
// AoE taunt 鈥?returns true if a spell was cast
...
bool IccLichKingNecroticPlagueAction::Execute(Event /*event*/)
{
    ...
    // No alive Horror 鈥?stop moving so healers can dispel immediately.
    ...
    // Already in delivery range 鈥?hold position for dispel
...
bool IccLichKingWinterAction::TryMoveToPosition(float targetX, float targetY, float targetZ, bool forced)
{
        ...
        bot->IsWithinLOS(targetX, targetY, targetZ))
    {
        ...
        // Path crosses defile or shadow trap 鈥?fall through to arc-stepping logic
    }
    ...
    // No defile nearby 鈥?safe to move directly to the target.
...
bool IccLichKingWinterAction::HandleMeleePositioning()
{
    ...
    // not yet under MT control. Once engaged, trust flank logic 鈥?no retreats.
    ...
    // Adds further out are not under tank control 鈥?engaging them puts bots
    ...
    // within FLANK_SETTLE_DIST of it, stay put and attack 鈥?no per-tick step
    // (prevents back-and-forth jitter near the slot).
    static constexpr float FLANK_SETTLE_DIST = 2.0f;

    // Position behind the current melee target using mainTank鈫抋dd vector
    if (mainTank && mainTank->IsAlive())
    {
    ...
    {
        float(M_PI),           // 180掳 directly behind
        float(M_PI) * 0.75f,   // 135掳 left-rear flank
        float(M_PI) * 1.25f,   // 225掳 right-rear flank
        float(M_PI) * 0.5f,    // 90掳  left flank
        float(M_PI) * 1.5f,    // 270掳 right flank
        float(M_PI) * 0.875f,  // 157掳 closer left-rear
    };
...
bool IccLichKingWinterAction::HandleRangedPositioning()
{
    ...
    // tick of the inbound phase 鈥?otherwise we cancel the bot's own movement
    ...
    if (farFromAnchor)
    {
        ...
        TryMoveToPosition(targetPos.GetPositionX(), targetPos.GetPositionY(), PLATFORM_Z, true);
        // Skip target acquisition while inbound 鈥?Attack() would cancel movement.
        return false;
    }
    ...
    // No spheres 鈥?engage skull-marked add or nearest valid add
...
bool IccLichKingWinterAction::HandleAssistTankAddManagement(Unit*, Position const* frostPos)
{
    ...
    // adds are on us 鈥?walk back to frost position
    if (!addsOnUs.empty())
    {
    ...
    if (!targetAdd)
    {
        ...
        // No loose adds 鈥?idle at frost pos but allow lower-priority actions to run
        return false;
    }
...
bool IccLichKingWinterAction::HandlePetManagement()
{
    ...
    if (botAI->IsHeal(bot))
    {
        if (ci->GetCommandState() != COMMAND_FOLLOW)
        {
            pet->AttackStop();
            pet->CastStop();
            pet->GetMotionMaster()->MoveFollow(bot, PET_FOLLOW_DIST, Creature_GetFollowAngle(pet)); // By leewheel 2026-07-26: TC鏃燙reature::GetFollowAngle锛岀敤鍏ㄥ眬helper
    ...
    if (pet->IsAIEnabled()) // By leewheel 2026-07-26: TC鐨処sAIEnabled鏄柟娉曢渶鍔犳嫭鍙?
    {
...
{
    // Being carried by a Val'kyr 鈥?no actions possible
    ...
    // Heroic cheat buffs 鈥?apply to all group members
    ...
    // Val'kyr marking owns skull while any Val'kyr is actively grabbing 鈥?skip here.
    if (Group* group = bot->GetGroup())
    {
        ...
        if (!anyValkyrGrabbing)
        {
            ...
            // During Vile Spirit windows, melee can't reach the spirits 鈥?mark boss on cross
            ...
            if (boss && boss->HealthBelowPct(40) && !IccBossHasRemorselessWinter(boss))
            {
                ...
                // Cross marker is no longer used for vile windows 鈥?clear if set
...
bool IccLichKingAddsAction::HandleHeroicNonTankPositioning(Difficulty diff, Unit* terenas)
{
    ...
    // Stack on main tank 鈥?3-yard threshold avoids micro-jitter
...
bool IccLichKingAddsAction::HandleAssistTankAddManagement(Unit* boss, Difficulty diff)
{
    ...
    // No adds at all 鈥?stay at hold position
    ...
    //   Tier 1 鈥?Shambling Horror
    //   Tier 0 鈥?Ghoul / Raging Spirit / other collectible add
    ...
    // Current target is a valid add 鈥?keep it unless a higher-tier add exists
    if (currentTier >= 0)
    {
        if (currentTier < 1)
        {
    ...
    {
        // Current target is dead/invalid 鈥?pick a new one.
    ...
    if (isHeroic && !addsOnUs.empty())
    {
        ...
        if (mainTank && mainTank->IsAlive())
        {
            ...
            // dot > 0.3: Shambling is facing toward MT 鈥?reposition to turn it away
...
bool IccLichKingAddsAction::HandleDefileMechanics(Unit* boss, Difficulty diff)
{
    ...
    // Boss casting Defile 鈥?only the targeted player runs out. Target is
    ...
    {
        //By leewheel 2026-08-01: 鐜╁鍙鏂囨湰涓枃鍖?
        botAI->Yell("污染落在 " + target->GetName() + " 身上了——快移到边缘！");
        //End By leewheel
...
bool IccLichKingAddsAction::HandleValkyrMechanics(Difficulty diff)
{
    ...
    // No active valkyrs 鈥?reset bots still targeting an excluded valkyr
...
bool IccLichKingAddsAction::HandleVileSpiritMechanics()
{
    ...
    if (sharedSlot >= 0 && sharedSlot < 3)
    {
        ...
        if (defileHit)
        {
            // Defile is fatal 鈥?move regardless of old spirits
            sharedSlot = -1;
        }
        else if (spiritHit)
        {
    ...
    if (botAI->IsAssistTank(bot))
    {
        ...
        // Find spirit closest to the slot (not to bot) 鈥?that's the spirit
        ...
        if (chaseTarget)
        {
            ...
            {
                // Spirit too far from slot 鈥?clamp chase point to leash radius
    ...
    // 鈥?they may not know the strategy, so a bot owns the role.
    ...
    // The harvested player must stay alive 鈥?flee movement breaks range for
    ...
    if (!botAI->IsTank(bot) && !bossCastingHarvest)
    {
        ...
        // Either condition is enough 鈥?proximity catches spirits that haven't
        ...
        if (chaser)
        {
            ...
            // No MT 鈥?fall through to leash logic
        }
    }
