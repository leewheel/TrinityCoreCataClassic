...
{
//By leewheel 2026-08-23: the-lab 更新——GetGroupMainTank 缩为单参数, fearWard 提升为 constexpr
...
bool AttumenTheHuntsmanHandlePhaseTwoAction::StackBehindAttumen(Unit* attumen)
{
    ...
    float const orientation = attumen->GetOrientation() + float(M_PI); //By leewheel 2026-09-03 修复C4305警告
...
{
    //By leewheel 2026-08-22: 随从名改为 entry(原英文名依赖 creature 名称→entry 缓存, 名称汉化后会落空)
...
{
    Unit* flare = AI_VALUE2(Unit*, "find target", "17096");
...
{
    //By leewheel 2026-08-22: 目标名改为 entry(恶魔之链17248/基尔雷克17229/邪蹄15688, 原英文名依赖名称缓存)
...
bool NetherspiteAvoidBeamAndVoidZoneAction::Execute(Event /*event*/)
{
    ...
    {
        float const angle = i * float(M_PI) / 18.0f; //By leewheel 2026-09-03 修复C4305警告
...
bool PrinceMalchezaarEnfeebledBotAvoidHazardAction::Execute(Event /*event*/)
{
    ...
    {
        float const angle = (2 * float(M_PI) * i) / numAngles; //By leewheel 2026-09-03 修复C4305警告
...
bool NightbaneGroundPhaseTanksPositionBossAction::Execute(Event /*event*/)
{
    ...
    if (deltaAB < 0.0f)
        deltaAB += 2.0f * float(M_PI); //By leewheel 2026-09-03 修复C4305警告

    float arcStart = 0.0f;
    float arcEnd = 0.0f;
    if (deltaAB < float(M_PI)) //By leewheel 2026-09-03 修复C4305警告
    {
        arcStart = thetaB;
        arcEnd = thetaA + 2.0f * float(M_PI); //By leewheel 2026-09-03 修复C4305警告
    }
    else
    {
        arcStart = thetaA;
    ...
    if (thetaN < arcStart)
        thetaN += 2.0f * float(M_PI); //By leewheel 2026-09-03 修复C4305警告
    else if (thetaN >= arcStart + 2.0f * float(M_PI)) //By leewheel 2026-09-03 修复C4305警告
        thetaN -= 2.0f * float(M_PI); //By leewheel 2026-09-03 修复C4305警告
...
bool NightbaneGroundPhaseCoordinateRangedMovementAction::MoveRangedLeaderToSafeSpot()
{
    ...
    for (uint8 i = 0; i <= numSteps; ++i)
    {
        ...
        {
            float const angle = j * float(M_PI) / 18.0f; //By leewheel 2026-09-03 修复C4305警告
...
bool NightbaneControlPetAggressionAction::Execute(Event /*event*/)
{
    ...
    //By leewheel 2026-08-21: 移植 brighton-chi e92a52db..2eaa83f3——状态切换成功后返回true(动作视为生效)
...
bool NightbaneTeleportBackToTerraceAction::Execute(Event /*event*/)
{
    ...
    //By leewheel 2026-08-18: 移植 brighton-chi the-lab 2670a809+7288c89f——改用 NearTeleportTo(同地图内瞬移不触发读条),
    //并校验落地高度作为成功标志(Nightbane落地阶段失败时回退重试)
    bot->NearTeleportTo(
