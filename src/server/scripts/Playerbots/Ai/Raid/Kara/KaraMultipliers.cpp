...
float AttumenTheHuntsmanWaitForDpsMultiplier::GetValue(Action* action)
{
    ...
    if (it == attumenDpsWaitTimer.end() || getMSTimeDiff(it->second, now) < dpsWaitMs)
        return 0.0f;
...
float TheCuratorDisableTankAssistMultiplier::GetValue(Action* action)
{
    ...
    if (AI_VALUE2(Unit*, "find target", "15691"))
        return 0.0f;
...
float NetherspiteWaitForDpsMultiplier::GetValue(Action* action)
{
    ...
    if (it == netherspiteDpsWaitTimer.end() || getMSTimeDiff(it->second, now) < dpsWaitMs)
        return 0.0f;
...
float NightbaneDisablePetsMultiplier::GetValue(Action* action)
{
    ...
    if (nightbane->GetPositionZ() > NIGHTBANE_FLIGHT_Z) // PetAttackAction
        return 0.0f;
...
float NightbaneWaitForDpsMultiplier::GetValue(Action* action)
{
    ...
    if (it == nightbaneDpsWaitTimer.end() || getMSTimeDiff(it->second, now) < dpsWaitMs)
        return 0.0f;
...
float NightbaneDisableMovementMultiplier::GetValue(Action* action)
{
    ...
    //By leewheel 2026-09-04 对齐上游: timer 未落时视为仍在飞行阶段, 阻塞跟随保持集合;
    //原写法放行跟随, 与上游"保守阻塞"语义相反
