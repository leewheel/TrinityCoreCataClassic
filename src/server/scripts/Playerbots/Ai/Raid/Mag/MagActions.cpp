...
Player* mainTank = GetGroupMainTank(bot);
    //By leewheel 2026-08-21: 移植 brighton-chi e92a52db..2eaa83f3——主坦克死亡时不再误导/打稳固射击
...
{
    constexpr float angleStep = float(M_PI) / 8.0f; //By leewheel 2026-09-03 修复C4305警告

    //By leewheel 2026-08-21: 移植 brighton-chi e92a52db..2eaa83f3——碎石/烈焰各查询一次供循环复用,
    //避免每个候选角度都重新遍历动态对象与最近物体列表
    Position debris;
    ...
    for (float angle = 0.0f; angle < 2.0f * float(M_PI); angle += angleStep) //By leewheel 2026-09-03 修复C4305警告
    {
...
bool MagtheridonMoveOutOfDebrisAction::FindSafePosition(Position& outPos)
{
    ...
    constexpr float angleStep = float(M_PI) / 12.0f; //By leewheel 2026-09-03 修复C4305警告
...
bool MagtheridonManageTimersAndAssignmentsAction::Execute(Event /*event*/)
{
    ...
    if (IsMagtheridonActive(magtheridon))
    {
        ...
        //By leewheel 2026-08-21: 移植 brighton-chi e92a52db..2eaa83f3——顶部落石(30%血)会把
        //Blast Nova等定时技能推迟18秒;落石标记改为集合
