...
float MagtheridonUseManticronCubeMultiplier::GetValue(Action* action)
{
    ...
    //By leewheel 2026-08-22: 随上游(9e8e7718..70b60954)重构——计时器存在即压制移动(点击者进入等待/点击阶段)。
    //上游代码在计时器不存在时对 end() 迭代器解引用(UB), 此处修正为返回1.0f不压制
...
float MagtheridonWaitToAttackMultiplier::GetValue(Action* action)
{
    ...
    //By leewheel 2026-08-22: 随上游(9e8e7718..70b60954)重构等待判定
...
float MagtheridonControlTankActionsMultiplier::GetValue(Action* action)
{
    ...
    //By leewheel 2026-08-22: 随上游(9e8e7718..70b60954)调整判定顺序与通道者检查
