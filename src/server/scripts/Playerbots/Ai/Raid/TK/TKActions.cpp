...
//By leewheel 2026-08-23: RaidBossHelpers.h 已重构为 Util/EncounterHelpers.h(TK 用其动态对象查询), 其余 include 按需
#include "SpellPackets.h" //By leewheel 2026-08-14: TC的UseItem封包需要
...
{
    //By leewheel 2026-08-24: 移植 brighton-chi c9a1a088——重置逻辑简化为统一清空4个实例状态
    //(原版本逐个查boss后清, 新版本一次清空, 依赖trigger保证无战斗)
...
//By leewheel 2026-09-04: 对齐上游b8304144——fear ward改由通用牧师策略处理，删除风暴要塞主坦fear ward动作
//End By leewheel

//By leewheel 2026-08-24: 移植 brighton-chi c9a1a088——坦克站位基类(MoveToTankPosition, 用于虚空/顾问/凯尔)
    ...
    Unit* target, Position const& position, float tolerance, bool shouldAttack)
{
    ...
    //By leewheel 2026-09-04: 上游4f9815d1——步进计算改用共享助手 EncounterHelpers::GetStepToPosition
    //(含后退步判定: 持仇+近战范围内+目标点在背向, 步长 2.25/3.5 码)
    float moveX;
    float moveY;
    bool backwards;
...
{
    constexpr float searchRadius = 40.0f;
    //By leewheel 2026-09-04: 上游4f9815d1——GetFlamePatches 移到 TKHelpers(带 bot 参)
    std::vector<Unit*> flamePatches = TkHelpers::GetFlamePatches(bot, searchRadius);
    //End By leewheel
...
// (GetFlamePatches 已移到 TKHelpers——By leewheel 2026-09-04 上游4f9815d1)
...
{
    constexpr float searchStep = float(M_PI) / 12.0f; //By leewheel 2026-09-03 修复C4305警告
    ...
    for (uint8 i = 0; i <= numDistSteps; ++i)
    {
        ...
        for (uint8 j = 0; j < numAngles; ++j)
        {
            ...
            //By leewheel 2026-09-04: 上游4f9815d1——IsPathSafe 增加 start 参(显式路径起点)
...
//By leewheel 2026-09-04: 上游4f9815d1——IsPathSafe 增加 start 参(路径起点由调用方传入, 不再固定用 bot 位置)
...
//By leewheel 2026-08-21: 移植 brighton-chi 2eaa83f3——索兰莉安恐惧守卫动作已并入通用 TempestKeepCastFearWardOnMainTankAction
//End By leewheel
...
bool KaelthasSunstriderMisdirectAdvisorsToTanksAction::Execute(Event /*event*/)
{
    ...
    {
        //By leewheel 2026-08-23: 保留 entry(#20063 = 技师特络尼库斯/Master Engineer Telonicus), 单参数 GetGroupAssistTank
        advisorTarget = AI_VALUE2(Unit*, "find target", "20063");
        tankTarget = GetGroupAssistTank(bot, 0);
        //End By leewheel
    }

    if (!IsAdvisorActive(advisorTarget) || advisorTarget->HasUnitFlag(UNIT_FLAG_NOT_SELECTABLE))
        return false;
...
//By leewheel 2026-08-21: 移植 brighton-chi 0040b523——主坦/副坦接位动作合并为统一 MeleeTanksPositionAdvisors
...
//By leewheel 2026-08-21: 移植 brighton-chi 615c78d2——RangedBotsDisperse 改传阶段数字
...
bool KaelthasSunstriderAssignAdvisorDpsPriorityAction::Execute(Event /*event*/)
{
    ...
    //End By leewheel
    //By leewheel 2026-08-21: 移植 brighton-chi 58deda97——Capernian坦必须是术士
...
{
    //By leewheel 2026-08-23: 顾问英文名改 entry(星术师卡波妮娅20062/首席技师塔隆尼库斯20063/萨古纳尔男爵20060,
    //原英文名查找在本项目中文DB环境不生效, 静默返回空导致满血判定失效)
...
bool KaelthasSunstriderLootLegendaryWeaponsAction::LootWeapon(uint32 weaponEntry, uint32 itemId)
{
    ...
    //By leewheel 2026-08-14: TC中CMSG_LOOT_ITEM=HandleAutostoreLootItemOpcode，格式：uint32 Count + (ObjectGuid + uint8)*Count + bool IsSoftInteract(bit)
    WorldPacket* packet = new WorldPacket(CMSG_LOOT_ITEM);
    *packet << uint32(1);  // Count
    *packet << weapon->GetGUID();  // Loot object GUID
    //By leewheel 2026-08-15: LootListID是1-based(服务端req.LootListID-1取槽)，传0会下溢255拾取失败，
    //凯尔萨斯传奇武器固定落在槽0→LootListID=1
    *packet << uint8(weaponIndex + 1);  // LootListID(1-based)
    //End By leewheel
...
bool KaelthasSunstriderLootLegendaryWeaponsAction::EquipLegendaryWeapon(uint32 itemId)
{
    ...
    if (proto->GetInventoryType() == INVTYPE_RANGED)
    {
        dstSlot = EQUIPMENT_SLOT_RANGED;
    }
    else if (proto->GetInventoryType() == INVTYPE_SHIELD ||
        proto->GetInventoryType() == INVTYPE_WEAPONOFFHAND)
    {
    ...
    if (dstSlot == EQUIPMENT_SLOT_OFFHAND)
    {
        ...
        if (mhItem && mhItem->GetTemplate()->GetInventoryType() == INVTYPE_2HWEAPON)
    ...
    bool const oldIs2H = alreadyEquipped->GetTemplate()->GetInventoryType() == INVTYPE_2HWEAPON;
    bool const newIs2H = proto->GetInventoryType() == INVTYPE_2HWEAPON;

    bot->SwapItem(srcPos, dstPos);

    // Changing between a 2H and a 1H leaves a stale offhand behind.
    if (((oldIs2H && !newIs2H && proto->GetInventoryType() != INVTYPE_SHIELD) ||
    ...
    auto const scanSlots = [&](uint8 bag, uint8 start, uint8 end)
    {
        for (uint8 slot = start; slot < end; ++slot)
        {
            ...
            uint8 const invType = itemProto->GetInventoryType();
...
bool KaelthasSunstriderUseLegendaryWeaponsAction::UseEquippedItemWithPacket(Item* item)
{
    ...
    //By leewheel 2026-08-30: 修复——TC 343 物品法术在 Effects 向量(ItemEffectEntry)中, Spells[] 恒为空,
    //  旧代码读不到传说武器(埃索达长弓/相位壁垒等)的使用技能, 传说武器技能从未生效
    ...
    //By leewheel 2026-08-14: TC的HandleUseItemOpcode需要WorldPackets::Spells::UseItem而非WorldPacket(用变量避免most vexing parse)
    ...
    useItem.Cast.Target.Flags = TARGET_FLAG_UNIT;
...
bool KaelthasSunstriderBreakMindControlAction::Execute(Event /*event*/)
{
    ...
    switch (bot->getClass())
    {
        ...
        case CLASS_DEATH_KNIGHT:
            spell = "blood strike";
            break;
        //By leewheel 2026-09-04 修复: 恢复上游 default 保护——圣骑士/德鲁伊/法系等非白名单职业
        //落空时 spell 仍为 nullptr, 直接 CanCastSpell(nullptr,...) 存在 UB/崩溃风险
        default:
            return false;
    }
...
bool KaelthasSunstriderSpreadOutInMidairAction::HoverAndSpread()
{
    ...
    //By leewheel 2026-08-14: TC无MOTION_SLOT_CONTROLLED/GetMotionSlotType，用当前运动类型+Clear等效
    if (mm->GetCurrentMovementGeneratorType() != NULL_MOTION_TYPE)
        mm->Clear();
    //End By leewheel
