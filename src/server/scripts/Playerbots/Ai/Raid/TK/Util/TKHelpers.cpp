...
//By leewheel 2026-09-04: 上游4f9815d1——GetFlamePatches 从 TKActions 成员函数移到 TKHelpers(带 bot 参)
...
//By leewheel 2026-08-23: the-lab(7cdc26e8 time→getMSTime) —— 配合 uint32 计时
std::unordered_map<uint32, uint32> advisorDpsWaitTimer;

//By leewheel 2026-08-21: 移植 brighton-chi 615c78d2——凯尔萨斯阶段读取助手。
//Boss 不存在或 dynamic_cast 失败(非本镜像类)均返回 PHASE_NONE,是模块内唯一的阶段读取点
...
//By leewheel 2026-08-21: 移植 brighton-chi 77ff8ec4——神器武器死亡GUID查找(走值缓存, 避免每帧全图扫描)
...
//By leewheel 2026-08-21: TC 343 的 GetCreatureListWithEntryInGrid 仅支持单 entry 重载,
    //逐 entry 遍历收集(与原实现行为一致); 2026-08-23: 半径改用 the-lab 常量 KAELTHAS_ROOM_SEARCH_DISTANCE
    GuidVector guids;
    for (uint32 weaponEntry : weaponEntries)
    {
        std::list<Creature*> weapons;
        bot->GetCreatureListWithEntryInGrid(weapons, weaponEntry, KAELTHAS_ROOM_SEARCH_DISTANCE);
