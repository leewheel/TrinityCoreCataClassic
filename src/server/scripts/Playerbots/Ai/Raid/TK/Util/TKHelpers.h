...
//By leewheel 2026-08-21: 移植 brighton-chi 615c78d2——GetKaelthasPhase 需要 Creature 前置声明
class Creature;
class Item;
class Player;
class PlayerbotAI;
...
//By leewheel 2026-09-04: 上游4f9815d1——GetFlamePatches 从 TKActions 移入(带 bot 参)
...
//By leewheel 2026-08-21: 移植 brighton-chi 615c78d2——凯尔萨斯阶段枚举从 TKKaelthasBossAI.h 迁入本头文件。
//阶段读取统一走 GetKaelthasPhase()(内部对 boss_kaelthas 做 dynamic_cast 镜像转换),
//这是全模块唯一的转换点,核心 boss_kaelthas 类变更时只需改这一处。
//Boss 不存在或转换失败统一返回 PHASE_NONE。
enum KTPhases
{
    PHASE_NONE           = 0,
...
//End By leewheel
//By leewheel 2026-08-21: 移植 brighton-chi 50bdd1c8——DPS等待计时未启动哨兵常量(改 uint32 与 getMSTime 配套)
...
//By leewheel 2026-08-21: 移植 brighton-chi 50bdd1c8——凤凰蛋查找助手
Creature* GetPhoenixEgg(Player* bot);
//End By leewheel

extern std::unordered_map<uint32, uint32> advisorDpsWaitTimer;

//By leewheel 2026-08-21: 移植 brighton-chi 615c78d2——凯尔萨斯阶段读取助手(取代散落的 dynamic_cast)
...
//By leewheel 2026-08-23: the-lab 已删除 IsAnyLegendaryWeaponDead 逻辑(改用 KAELTHAS_ROOM_SEARCH_DISTANCE 的 GetCreatureListWithEntryInGrid)
...
//By leewheel 2026-09-04 修复: 删除重复的GetPhoenixEgg声明(251行已有一份, 移植时重复添加)
//Creature* GetPhoenixEgg(Player* bot);
