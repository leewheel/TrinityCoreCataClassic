...
//By leewheel 2026-08-14: Uld reset——脱战清理全局状态
bool UlduarResetEncounterStatesAction::Execute(Event /*event*/)
{
    return RazorscaleBossHelper::ClearInstanceState(bot);
}
//End By leewheel
...
bool FlameLeviathanEnterVehicleAction::EnterVehicle(Unit* vehicleBase, bool moveIfFar)
{
    ...
    //By leewheel 2026-08-14: TC的HandleCancelMountAuraOpcode需要WorldPackets::Spells::CancelMountAura(WorldPacket&&构造)
...
bool RazorscaleHarpoonAction::Execute(Event /*event*/)
{
    ...
    {
        //By leewheel 2026-08-14: TC的HandleGameObjectUseOpcode需要WorldPackets::GameObject::GameObjUse(WorldPacket&&构造)
        WorldPacket useItemPacket(CMSG_GAME_OBJ_USE);
        WorldPackets::GameObject::GameObjUse usePacket(std::move(useItemPacket));
        usePacket.Guid = closestHarpoon->GetGUID();
        bot->GetSession()->HandleGameObjectUseOpcode(usePacket);
        //End By leewheel
    }

    {
        //By leewheel 2026-08-14: TC的HandleGameobjectReportUse需要WorldPackets::GameObject::GameObjReportUse(WorldPacket&&构造)
        WorldPacket reportItemPacket(CMSG_GAMEOBJ_REPORT_USE);
        WorldPackets::GameObject::GameObjReportUse reportPacket(std::move(reportItemPacket));
        reportPacket.Guid = closestHarpoon->GetGUID();
...
bool ThorimMarkDpsTargetAction::Execute(Event /*event*/)
{
    ...
    if (botAI->IsMainTank(bot))
    {
        ...
        Unit* evoker = AI_VALUE2(Unit*, "find target", "32924");
...
bool MimironShockBlastAction::Execute(Event /*event*/)
{
    ...
    else
    {
        ...
        for (float delta = 0; delta <= float(M_PI) / 2; delta += float(M_PI) / 8) //By leewheel 2026-09-03 修复C4305警告
        {
...
bool VezaxShadowCrashAction::Execute(Event /*event*/)
{
    ...
    float angleIncrement = float(M_PI) / 10; //By leewheel 2026-09-03 修复C4305警告
...
bool YoggSaronMoveToEnterPortalAction::Execute(Event /*event*/)
{
    ...
    if (bot->GetRaidDifficulty() == RAID_DIFFICULTY_10MAN_NORMAL)
...
bool YoggSaronUsePortalAction::Execute(Event /*event*/)
{
    ...
    //By leewheel 2026-08-14: TC的HandleSpellClick返回void，调用后返回true
...
bool YoggSaronIllusionRoomAction::SetIllusionRtiTarget(YoggSaronTrigger yoggSaronTrigger)
{
    ...
    if (botAI->HasCheat(BotCheatMask::raid))
    {
        ...
        //By leewheel 2026-08-14: TC的Unit::DealDamage仅8参数
