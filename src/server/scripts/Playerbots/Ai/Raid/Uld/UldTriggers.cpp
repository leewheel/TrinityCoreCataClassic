...
//By leewheel 2026-08-14: Uld reset trigger
bool UlduarBotIsNotInCombatTrigger::IsActive()
{
...
bool ThorimGauntletPositioningTrigger::IsActive()
{
    ...
    if (raidDifficulty == RAID_DIFFICULTY_10MAN_NORMAL)
    {
        requiredDpsQuantity = 3;
        requiredHealerQuantity = 1;
    }
    else if (raidDifficulty == RAID_DIFFICULTY_25MAN_NORMAL)
    {
...
{
    Unit* boss = AI_VALUE2(Unit*, "find target", "33432");
...
bool YoggSaronMoveToExitPortalTrigger::IsActive()
{
    ...
    if (brain->HasUnitState(UNIT_STATE_CASTING))
    {
        ...
        {
            uint32 castingTimeLeft = induceMadnessSpell->GetTimer();
