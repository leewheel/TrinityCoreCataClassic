...
//By leewheel 2026-08-23: RaidBossHelpers.h 已重构并入 Util/EncounterHelpers.h(GetInstancePlayerGuids 随迁)
#include "EncounterHelpers.h"
//End By leewheel

//By leewheel 2026-08-23: GetInstancePlayerGuids 现位于 EncounterHelpers 命名空间
...
//By leewheel 2026-08-14: 实例级状态清理——清空本实例所有bot的换位时间
bool RazorscaleBossHelper::ClearInstanceState(Player* bot)
{
    bool erased = false;
    for (ObjectGuid const& guid : GetInstancePlayerGuids(bot))
    {
        if (_lastRoleSwapTime.erase(guid) > 0)
            erased = true;
    }
    // _harpoonCooldowns按墙钟5秒自然过期, 无需主动清理
    return erased;
}
//End By leewheel
