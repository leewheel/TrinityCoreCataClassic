...
/*By leewheel 2026-08-18: 对齐 brighton-chi the-lab HEAD(e92a52db)——
  ...
  Archimonde 地狱火(Doomfire)完整体系(燃烧/危险/控制/场半径、月神之佑、动态物体缓存)、
  MELEE_RANGE_INSET/BOSS_ENGAGED_HEALTH_PCT/DEATH_AND_DECAY_MELEE_CONTROL_RADIUS 常量、
  Kaz'rogal 接战血量、IsKazrogalManaUser 声明。*/
...
// 缓存间隔与默认 AiPlayerbot.ReactDelay 一致
inline constexpr uint32 HAZARD_CACHE_INTERVAL = 100;
inline constexpr float HAZARD_SEARCH_MARGIN = 2.0f;
// 远程危害规避触发止于危害边缘。FleePosition() 以固定 AiPlayerbot.FleeDistance 步长移动
// (默认5码), 可能停在危害外最多5码处。该余量用于乘数: 覆盖这5码并留一点, 防止
// ReachSpellAction 在bot逃出危害后把它拉回危害内
...
//By leewheel 2026-08-30: 删除本地重复定义——BOSS_ENGAGED_HEALTH_PCT 统一由 Util/EncounterHelpers.h 提供，
//  此前本地在此重复定义导致与 EncounterHelpers.h 同时 include 时 C2872 符号不明确（上游此处无此定义）
//End By leewheel
struct RangedGroups
{
...
//By leewheel 2026-08-30: 补回 GetRangedMembers 声明（上游 HyjalHelpers.h 有此声明，本地移植时遗漏导致 HyjalActions.cpp C3861）
//End By leewheel
...
//By leewheel 2026-08-30: 补回 INFERNAL_RANGED_ENGAGE_DISTANCE 常量——超过此距离远程不打地狱火（本地遗漏）
//End By leewheel
...
//By leewheel 2026-08-30: 对齐上游签名——GetLooseInfernal/GetNearestInfernal/GetInfernalTargetingBot 改为单参(Player*)，
//  内部自行 GET_PLAYERBOT_AI(bot)，与 AC 业务对标一致（本地旧双参签名与上游调用不匹配导致编译错误）
Unit* GetLooseInfernal(Player* bot);
Unit* GetNearestInfernal(Player* bot);
Unit* GetInfernalTargetingBot(Player* bot);
//By leewheel 2026-08-30: 补回 GetInfernalToAttack 声明——远程bot应攻击的最近地狱火（上游有，本地遗漏导致 HyjalTriggers.cpp C3861）
//End By leewheel
...
// 这一组留出了"逃离的魔法阈值"与"回队伍的魔法阈值"之间的间隔，避免在两者之间来回横跳
...
//By leewheel 2026-08-30: 补回 GetKazrogalImmunitySpell 声明（上游 HyjalHelpers.h 有此声明，本地遗漏导致 HyjalTriggers.cpp C3861）
//End By leewheel
uint32 GetKazrogalImmunitySpell(Player* bot);

// Azgalor
// 15y radius + 1.95y 最大玩家 CombatReach。与死亡凋零同理，CombatReach 叠加到范围源于 AC 的
// 一处 bug
...
//By leewheel 2026-08-30: 对齐上游签名——IsDoomguardTank 改单参(Player*)，内部无需 botAI
...
// 缓存持有的每个存活地狱火尾迹位置，供需要权衡整个场地的调用方，而不只是问附近有无
...
// 当前正在施放的气爆(如有)。读条开始时记录, 超时自行失效——回答"是否在施放中",
// 而不是"是否曾施放过"。结果通过出参 airBurst 返回
