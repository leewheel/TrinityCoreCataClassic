...
{
    constexpr float searchStep = float(M_PI) / 8.0f; //By leewheel 2026-09-03 修复C4305警告：M_PI为double字面量，改float常量
...
bool TeronGorefiendPositionRangedOnBalconyAction::Execute(Event /*event*/)
{
    ...
    constexpr float arcSpan = 2.0f * float(M_PI) / 5.0f; //By leewheel 2026-09-03 修复C4305警告
...
bool MotherShahrazRunAwayToBreakFatalAttractionAction::Execute(Event /*event*/)
{
    ...
    else
    {
        ...
        const float angle = frand(0.0f, 2.0f * float(M_PI)); //By leewheel 2026-09-03 修复C4305警告
...
{
    constexpr float searchStep = float(M_PI) / 16.0f; //By leewheel 2026-09-03 修复C4305警告
    constexpr float minDistance = 2.0f;
    constexpr float distanceStep = 1.0f;

    float backwardsAngle = Position::NormalizeOrientation(bot->GetOrientation() + float(M_PI)); //By leewheel 2026-09-03 修复C4305警告

    Position bestPos;
    float bestAngleDiff = float(M_PI) * 2.0f; //By leewheel 2026-09-03 修复C4305警告
    ...
    Unit* illidan, Group* group)
{
    ...
    constexpr float arcSpan = float(M_PI); //By leewheel 2026-09-03 修复C4305警告
    const float arcCenter = illidan->GetOrientation() + float(M_PI); //By leewheel 2026-09-03 修复C4305警告
    ...
    Unit* illidan, Group* group)
{
    ...
    constexpr float forbiddenArc = (2.0f / 3.0f) * float(M_PI); //By leewheel 2026-09-03 修复C4305警告
    constexpr float allowedArc = (4.0f / 3.0f) * float(M_PI); //By leewheel 2026-09-03 修复C4305警告
...
bool IllidanStormrageDestroyHazardsAction::Execute(Event /*event*/)
{
    ...
    if (!entries.empty())
    {
        //By leewheel 2026-08-14: TC的GetCreatureListWithEntryInGrid仅支持单个entry，逐entry收集
        for (uint32 entry : entries)
            bot->GetCreatureListWithEntryInGrid(hazards, entry, searchRadius);
        //End By leewheel
    }
...
bool IllidanStormrageHandleAddsCheatAction::Execute(Event /*event*/)
{
    ...
    else
    {
        ...
        if (shadowDemon && shadowDemon->GetHealthPct() > 25.0f)
        {
            ...
            //By leewheel 2026-08-14: TC的Unit::DealDamage仅8参数(无isLethal/redirect等)
