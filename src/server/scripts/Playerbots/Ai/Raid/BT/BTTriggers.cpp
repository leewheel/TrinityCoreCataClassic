...
bool SupremusPullingBossOrChangingPhaseTrigger::IsActive()
{
    ...
    const uint32 elapsed = getMSTimeDiff(it->second, now);

    // Active during first 10 seconds, or during 60-70, 120-130, etc.
    return (elapsed < 10 * IN_MILLISECONDS) ||
           ((elapsed % (60 * IN_MILLISECONDS)) < 10 * IN_MILLISECONDS &&
            elapsed >= 60 * IN_MILLISECONDS);
}
