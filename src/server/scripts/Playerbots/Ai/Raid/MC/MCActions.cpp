...
//By leewheel 2026-08-23: McMoveFromLava 用 Map::GetLiquidStatus 判断岩浆(TC)
#include "Map.h"
//End By leewheel
...
//By leewheel 2026-09-03 修复C4305警告：6个double字面量初始化float Position，补f后缀
...
bool McMoveFromLavaAction::Execute(Event /*event*/)
{
    ...
    if (Group* group = bot->GetGroup())
    {
        for (GroupReference* ref = group->GetFirstMember(); ref; ref = ref->next())
        {
            ...
            //By leewheel 2026-08-23: TC 无 LiquidData.Flags, 用 Map::GetLiquidStatus(Magma) 判断成员是否身处岩浆
            LiquidData liquid;
            if (bot->GetMap()->GetLiquidStatus(
                    member->GetPhaseShift(), member->GetPositionX(), member->GetPositionY(),
                    member->GetPositionZ(), map_liquidHeaderTypeFlags::Magma, &liquid) != LIQUID_MAP_NO_WATER)
                continue;
            //End By leewheel
