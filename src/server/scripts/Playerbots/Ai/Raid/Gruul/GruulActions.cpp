...
{
    //By leewheel 2026-09-04: 上游52e295d2/8baf63da——最小距离常量化(Blast Wave半径15y+2y避让), 距离判定改精确
    Position const& position = KROSH_TANK_POSITION;
    float const distanceKroshToPosition = krosh->GetExactDist2d(position);
    float const minDistance = KROSH_BLAST_WAVE_SAFE_DISTANCE;
    ...
    bot->CastStop();
    return MoveAway(krosh, minDistance - currentDistance);
    //End By leewheel
}
...
bool HighKingMaulgarMoonkinTankAttackKigglerAction::Execute(Event /*event*/)
{
    ...
    //By leewheel 2026-09-04: 上游52e295d2——奥爆安全距离常量化(30y半径+2y避让), 距离判定改精确
...
bool HighKingMaulgarAssignDpsPriorityAction::Execute(Event /*event*/)
{
    ...
    //By leewheel 2026-09-05: 上游1c632ddf+e0b3b375——Maulgar兜底查找改为最后分支, 且先判IsRanged再查Krosh(避免近战白查)
    Unit* target = nullptr;
    Unit* krosh = nullptr;
    if (Unit* blindeye = AI_VALUE2(Unit*, "find target", "18836"))
    {
        target = blindeye;
    }
...
bool HighKingMaulgarRunAwayFromWhirlwindAction::Execute(Event /*event*/)
{
    ...
    //By leewheel 2026-09-04: 上游52e295d2——旋风安全距离常量化(8y半径+8*sqrt(2)避让), 距离判定改精确
...
//By leewheel 2026-09-04: 上游8baf63da——改名 HighKingMaulgarBackAwayFromKroshAction(语义: 从克罗什背后退开)
...
{
    //By leewheel 2026-09-04: 上游3b0ee7f7——魔犬改走1秒缓存值(排序规范化保证全团配对一致)
    ...
    //By leewheel 2026-09-04: 上游7df92554——std::find定位术士序号
...
{
    //By leewheel 2026-09-04: 上游52e295d2——粉碎安全距离常量化(20y半径+2y避让)
