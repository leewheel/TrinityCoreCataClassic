...
bool FelmystAvoidDemonicVaporAction::MoveAwayFromVapor(bool unrestricted)
{
    ...
    //By leewheel 2026-09-04: 上游70808114——蒸汽危害半径13.5→13.0, 距离判定改精确 GetExactDist2d
...
{
    //By leewheel 2026-09-04: 上游70808114——跟随距离2.0→5.0, 距离判定改精确 GetExactDist2d
...
void FelmystAvoidDemonicVaporAction::AnnounceFlightLeader(Player* leader)
{
    std::string const text = PlayerbotTextMgr::instance().GetBotTextOrDefault(
        //By leewheel 2026-09-04: 中文化——fallback 与 ai_playerbot_texts 中文行保持一致
        "felmyst_flight_leader",
        "[NAME] 现在是飞行阶段领队。飞行阶段所有人都要集合到 [NAME] 身边。",
...
bool FelmystMoveToSafeFogLaneAction::Execute(Event /*event*/)
{
    ...
    //By leewheel 2026-09-04: 上游70808114——雾目的地改双函数: 穿雾用bot自身位置投影, 第三趟后落地接近用菲米丝位置投影
    Position destination;
...
bool FelmystManageLandingDpsTimerAction::Execute(Event /*event*/)
{
    ...
    //By leewheel 2026-08-21: 移植 brighton-chi 348091f8——直接取实例id, 不再绕道Map
