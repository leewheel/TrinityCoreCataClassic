...
{
    //By leewheel 2026-09-04: 上游70808114——跟随改名 FELMYST_RIGHT/LEFT_LANDING_POSITION
...
{
    //By leewheel 2026-09-04: 上游70808114——跟随改名 FELMYST_CENTER_GROUND_REFERENCE/FELMYST_RIGHT/LEFT_LANDING_POSITION
    ...
    Player* bot, Position const& anchorDestination, Position& destination)
{
    ...
    //By leewheel 2026-08-21: 移植 brighton-chi 348091f8——用坐标分量判断距离, 避免Position临时对象
...
//By leewheel 2026-09-04: 上游70808114——雾走位重构: 候选点网格化(垂向净距×沿向侧偏)按距离排序后取首个合法点;
//   TryGetFelmystFogSafeDestination 拆为 TryGetFelmystFogCrossingDestination(穿雾)/TryGetFelmystLandingApproachDestination(落地接近);
//   蒸汽净距常量 FOG_DESTINATION_VAPOR_CLEARANCE, 距离判定改 GetExactDist2d
namespace
{
    ...
    Player* bot, FogLane lane, Position const& projectFrom, Position& destination)
{
    ...
    //By leewheel 2026-08-21: 移植 brighton-chi 348091f8——活跃雾气时取最短路线到另一条安全lane;
    //第三趟结束后调用方改传菲米丝位置, 让bot贴着lane终点并行跑, 为落地做准备
...
{
    //By leewheel 2026-09-04: 上游70808114——跟随改名 FELMYST_TANK_POSITIONS
...
//By leewheel 2026-08-21: 移植 brighton-chi 348091f8——远程站位分配不再存map, 改为按GUID序数
//当场计算(治疗三堆轮换, 远程DPS左右交替), 免去跨tick的状态清理
...
//By leewheel 2026-08-21: 移植 brighton-chi 348091f8——扫雾趟tracker拆分为独立函数,
//避免TryGetFelmystFogOfCorruptionStageState为取窗口而误触第三趟逻辑
namespace
{
...
bool IsFelmystAirPhaseTargetSuppressed(Unit* felmyst)
{
    ...
    // 空中开局拉怪阶段保留近战目标的HP阈值
    //By leewheel 2026-08-21: 移植 brighton-chi f2add941——统一用拉怪完成判定常量
...
bool TryGetFelmystFogOfCorruptionStageState(Unit* felmyst, FogOfCorruptionState& state)
{
    ...
    //By leewheel 2026-08-21: 移植 brighton-chi 348091f8——只推进扫雾趟tracker, 不取窗口
...
Player* GetFelmystEncapsulateTarget(Player* bot)
{
    ...
    for (GroupReference* ref = group->GetFirstMember(); ref; ref = ref->next())
    {
        ...
        //By leewheel 2026-09-04: 上游70808114——距离判定改精确 GetExactDist
...
Player* GetFelmystCharmedTarget(Player* bot, Unit* felmyst)
{
    ...
    for (GroupReference* ref = group->GetFirstMember(); ref; ref = ref->next())
    {
        ...
        //By leewheel 2026-09-04: 上游70808114——30.0f 改用全局 RANGED_ABILITY_REACH
