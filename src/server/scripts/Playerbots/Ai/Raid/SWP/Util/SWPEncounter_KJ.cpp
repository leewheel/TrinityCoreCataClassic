...
//By leewheel 2026-08-29: 引入 mod-playerbots 新提交——KJ 手部控制: 搜索附近的欺骗者之手
...
bool TryGetKiljaedenRangedSlotPosition(uint8 slotIndex, Position& position)
{
    ...
    float const angleOffset = GetCenteredArcSlotAngleOffset(localSlotIndex, slotCount, float(M_PI)); //By leewheel 2026-09-03 修复C4305警告
...
void EnsureKiljaedenRangedAssignments(Player* bot)
{
    ...
    //By leewheel 2026-09-04: 上游1e110a5f——跟随改名 KILJAEDEN_RANGED_ASSIGNMENT_REBUILD_INTERVAL_MS
...
Player* FindBestKiljaedenDragonClusterTarget(Player* bot, Unit* dragon, uint32 spellId)
{
    ...
    for (GroupReference* ref = group->GetFirstMember(); ref; ref = ref->next())
    {
             ...
             otherRef; otherRef = otherRef->next())
        {
            ...
            //By leewheel 2026-09-04: 上游1e110a5f——跟随改名 KILJAEDEN_DRAGON_CLUSTER_RADIUS
        ...
        //By leewheel 2026-09-04: 上游1e110a5f——跟随改名 KILJAEDEN_DRAGON_MIN_CLUSTER_SIZE
...
//By leewheel 2026-08-27: 对齐 the-lab 19196110——移除 HasAtLeastThreeBotTanks(不再要求3个机器人坦克), 由 KiljaedenAssignHandsOfTheDeceiverAction 改为 GetGroupAssistTank/GetGroupMainTank 逐个判定
//End By leewheel
