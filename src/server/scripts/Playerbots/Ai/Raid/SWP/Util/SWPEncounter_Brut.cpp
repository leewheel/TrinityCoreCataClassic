/* 太阳之井高地 机器人策略 */
/* 移植来源: brighton-chi mod-playerbots the-lab SWPEncounter_Brut.cpp 移植适配 TC 框架 */
...
//By leewheel 2026-08-21: 移植 brighton-chi 798a1da0/3c8d60e0——布胖遭遇状态(按副本实例id区分)
...
// 每个有站位的bot每tick都会调用Ensure系列函数, 它们扫描同一团队并构建相同的全团结果,
// 因此每个重建窗口内第一个调用者干活, 其余读取其遗留结果。跳过是安全的, 因为调用方
// 会重读map, 把缺失条目当作"本tick未定位"而不是错误。
...
// 万一所有槽位占满则重叠站位(TBC虽不待见近战, 20个位置仍基本不可能占满)
...
// 万一所有槽位占满则重叠站位(14个位置, TBC不待见近战, 基本不可能)
        assignments[member->GetGUID()] =
    ...
    uint8 rangedIndex, float radius, Position& position)
{
    ...
    constexpr float degreeToRadian = float(M_PI) / 180.0f; //By leewheel 2026-09-03 修复C4305警告
