...
//By leewheel 2026-09-04: 移植上游——IsEncounterInProgress需要InstanceScript
    ...
    float& stepX, float& stepY, float& stepZ)
{
    ...
    //By leewheel 2026-09-03 修复C4189警告：升级后的兼容层Map_CanReachPositionAndGetValidCoords已内置
    //坡度校验(IsWalkableClimb)无法外部开关，此处按AC原版语义 failOnSlopes=(candidateZ > botZ)，
    //仅上坡执行完整校验，平地/下坡只做碰撞校验跳过坡度检查
    ...
    //By leewheel 2026-08-23: AC 的 Map::CanReachPositionAndGetValidCoords 在 TC 不存在,
    // 改用项目 Playerbots.h 的兼容封装(碰撞校验+坡度校验)
    //By leewheel 2026-09-03 修复C4189警告：按AC原版语义 failOnSlopes 仅上坡为true——
    //上坡走完整兼容封装(碰撞+坡度)，平地/下坡手动只做碰撞校验(对应AC第9参数传false跳过坡度检查)
    //By leewheel 2026-09-04 对齐上游修复: 两个分支的碰撞校验起点都必须是机器人坐标——
    //原写法上坡分支把候选点同时当起点和终点(零长度射线, 恒无碰撞命中), 平地/下坡分支只在
    //候选点竖直列内探测, 都不检测机器人到候选点之间的墙体, 避害走位会径直撞进障碍
    bool canReach = false;
    if (failOnSlopes)
    {
        canReach = Map_CanReachPositionAndGetValidCoordsTwoPoint(
            bot->GetMap(), bot, botX, botY, botZ, candidateX, candidateY, candidateZ);
    }
    else
    {
        // 平地/下坡：从机器人坐标整段执行TC的碰撞校验，跳过坡度检查
        float startX = botX, startY = botY, startZ = botZ;
        canReach = bot->GetMap()->CheckCollisionAndGetValidCoords(
            bot, startX, startY, startZ, candidateX, candidateY, candidateZ, true);
    }
    if (!canReach)
    {
        return false;
...
//'facing'为可选参数供tank使用——传所坦怪物允许倒退走位，传nullptr则普通前进
...
std::vector<Position> GetDynamicObjectPositions(Player* bot, float searchRadius, uint32 spellId)
{
    ...
    //By leewheel 2026-08-23: the-lab 原版用 Acore:: 命名空间(AC), TC 为 Trinity:: —— 修命名空间
...
//By leewheel 2026-08-23: 原 RaidBossHelpers.cpp 的实现(RaidBossHelpers 重构后并入 EncounterHelpers)
// 收集当前地图内全部玩家的 GUID — 用于战斗结束时按实例清理 GUID-keyed 状态
