...
int32 SpellEffectInfo::CalcValueAsInt(WorldObject const* caster /*= nullptr*/, SpellEffectValue const* basePoints /*= nullptr*/, Unit const* target /*= nullptr*/, float* variance /*= nullptr*/) const
{
    return int32(CalcValue(caster, basePoints, target, variance));
}

SpellEffectValue SpellEffectInfo::CalcValue(WorldObject const* caster /*= nullptr*/, SpellEffectValue const* bp /*= nullptr*/, Unit const* target /*= nullptr*/, float* variance /*= nullptr*/) const
{
    SpellEffectValue basePoints = CalcBaseValue(caster, target);
...
SpellEffectValue SpellEffectInfo::CalcBaseValue(WorldObject const* caster, Unit const* target) const
{
    if (Scaling.Coefficient != 0.0f)
    {
        float value = 0.0f;
...
SpellEffectValue SpellEffectInfo::CalcPointsPerResource(WorldObject const* caster) const
{
    SpellEffectValue value = 0.0;
    Unit const* unitCaster = Object::ToUnit(caster);
    ...
    if (unitCaster)
    {
        if (Scaling.Coefficient != 0.0f)
        {
            ...
            float scaledAmount = GetSpellScalingColumnForClass(sSpellScalingGameTable.GetRow(unitCaster->GetLevel()), scalingClass);
            scaledAmount *= _spellInfo->GetSpellScalingMultiplier(unitCaster->GetLevel(), false) * Scaling.ResourceCoefficient;
            value = scaledAmount;
        }
    }

    if (std::fabs(value) < 0.00000023841858 && PointsPerResource != 0.0f)
        return PointsPerResource;
    ...
    Optional<uint32> numNonDiminishedTargetsValueHolderSpell, Optional<SpellEffIndex> numNonDiminishedTargetsValueHolderEffect)
{
    ...
    if (maxTargetsValueHolderEffect)
    {
        ...
        else
        {
            ...
            int32 expectedValue = int32(valueHolder.CalcBaseValue(nullptr, nullptr));
